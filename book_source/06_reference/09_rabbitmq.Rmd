...
5. The target model service, which is listening on its dedicated queue, hears this message and runs its `APPLICATION`, which is `job.sh`, in the directory indicated by the message.
   If model execution was successful, the model service posts an OK message to the `pecan` queue.
   
6. The `executor` container, meanwhile, is waiting to hear this OK message on its queue.
   Once it hears this message, it advances to the post-execution parts of the workflow.
