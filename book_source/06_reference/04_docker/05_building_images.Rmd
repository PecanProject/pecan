...
For general use, it is sufficient to use the pre-built PEcAn images hosted on [Docker Hub](https://hub.docker.com/r/pecan/).
...
- `pecan/executor` -- Rebuild if:
  - You built a new version of `pecan/base` (on which `pecan/executor` depends) and/or, `pecan/depends` (on which `pecan/base`)
...
- `pecan/web` -- Rebuild if you modified any of the following:
  ...
  - NOTE: Because changes to this code are applied instantly (i.e. do not require compilation or installation), a more effective way to do local development is to mount the `web/` or other relevant folders as a volume onto the `pecan/web` container (see [local development guide below](#docker-local-devel)).
