...
- `format`: R list object that is the output of the function  `query.format.vars` (see function description [here] and example below)
...
The data for this example has already been entered in to the database. To add new data go to [this useful site].
...
1. Create a connection to the BETY database. This can be done using R function 
   ...
   where the complete path to the `config.php` is specified. See [here] for creating the `config.php` file. 

2. Look up the inputs record for the data in BETY. 

   ![Input_ID_name](images/Input_ID_name.png)
