...
* If you are new to GitHub or to PEcAn, start with the one-time set-up instructions under [Before any work is done](#before-any-work-is-done). Also see the excellent tutorials and references in the [References]) section at the the bottom of this page.
...
### PEcAn Project and  GitHub: https://github.com/organizations/PecanProject
...
We follow branch organization laid out on this page: http://nvie.com/posts/a-successful-git-branching-model](http://nvie.com/posts/a-successful-git-branching-model)/

In short, there are three main branches you must be aware of:
Develop - Main Branch conataining the latest code
Master - Branch containing the latest stable code
release/vX.X.X - Named branches containing code specific to a release.
...
* milestones (Big picture, "Epic"): contains many issues, organizaed by release.
...
**Each feature should be in it’s own branch** (for example each issue is a branch, names of branches are often the issue in a bug tracking system).
...
If you would like to make changes to a release branch, you must follow a differetn workflow, as the release branhc will contain the latest code on develop and must remain seperate.
