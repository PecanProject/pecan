...
4. **Initialize the PEcAn database and data images**. Navigate to the directory you saved `docker-compose.yml` to and use the following `docker-compose` commands to download all the data PEcAn needs to start working. For more on how they work, see our [Docker topical pages](#pecan-docker-quickstart-init).
    ...
    a. Create and start the PEcAn database container (without any data)
        ...
        ```
        Creating pecan-postgres-1 Started
        ```
...
5. **Start the PEcAn stack**. Assuming all of the steps above completed successfully, start the full stack by running the following shell command:
    ...
    - File browser (minio) -- http://pecan.localhost/minio/ <!-- ERS: Doesn't work on docker installation -->
    - RabbitMQ management console (for managing queued processes) -- http://pecan.localhost/rabbitmq/ <!-- ERS: Doesn't work on docker installation -->
    - Traefik, webserver showing maps from URLs onto their respective containers -- http://pecan.localhost/traefik/ <!-- ERS: Doesn't work on docker installation -->
