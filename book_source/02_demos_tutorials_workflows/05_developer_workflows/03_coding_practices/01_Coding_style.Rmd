...
* The `Suggests` field can be used to declare dependencies on packages that make your package more useful but are not completely essential. By default R will not install these when your package is installed (users can change this using the `dependencies` argument of `install.packages`), and you are not allowed to import functions from them into your package's namespace. Again from the [R extensions manual](https://cran.r-project.org/doc/manuals/R-exts.html#Package-Dependencies):
