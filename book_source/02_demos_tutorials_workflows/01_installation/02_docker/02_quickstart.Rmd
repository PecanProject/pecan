...
[Setup PEcAn using docker-compose](#pecan-setup-compose)
...
- `--env FQDN=docker` -- the Fully Qualified Domain Name, this is the same value as specified in the `docker-compose.yml` file (for the web, monitor and executor containers). This will link the data files to the name in the machines table in BETY.
