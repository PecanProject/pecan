...
## Remote data module
Remote data module retrieves remote sensing data from MODISTools and Google Earth Engine (with plans to support AppEEARS in near future). The downloaded data can be used for preforming further analysis in PEcAn.
...
```
3. **Authenticate GEE**. The GEE API needs to be authenticated using your credentials (one time step). The credentials will be stored locally on your system. This can be done by,
```bash
