...
In addition to met2model.MODEL, PEcAn also supports the following additional input conversions:

- `veg2model.MODEL` - handles vegetation initial conditions. Supports both pool-based and cohort-based models
- `write.events.MODEL` - handles management events (still in early development)
- soil physical parameters (e.g., texture, hydraulics, thermodynamics) - there is not yet a convention for stand-alone converters, existing models currently read the `soil.nc` file from `settings$run$inputs$soil_physics` as part of their `write.configs`. This file is generated by `PEcAn.data.land::soil2netcdf`
