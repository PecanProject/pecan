...
- `revision` -- A unique identifier for the model version of `binary`. Format is model-specific, but many models expect a semantic version (eg "2.12.3") and rely on version comparisons (e.g. `revision >= "2.0"`) to match PEcAn configuration behavior to model capabilities.
...
- `options` -- Additional configuration passed on to the model.
  Depending on the model these might be passed as a set of command flags (e.g. `<options><infile>path/to/file</infile><quiet>FALSE</quiet><options` becomes `./path/to/binary --no-quiet --inputfile=path/to/file`),
  entries in a configuration file (e.g. `<options><LOG_FILE>path/to/file</LOG_FILE><QUIET>1</QUIET><options` becomes `LOG_FILE = path/to/file` and `QUIET = 1`),
  or a more complicated transformation (e.g. a model that accepts some options as command flags and other as lines in a configuration file might take `<options><flags>...</flags><settings>...</settings></options>`).
  To avoid confusion, we recommend that the names in `model$options` be *identical* to the names the model recognizes: If the model expects `LONG_COMPLICATED_OPTION_NAME = "third_verbose_option"`, write that into the XML instead of making write.config.MODEL translate from `model$options$complic_opt = 3".`
...
See the [PEcAn Models](pecan-models.html) reference for detailed documentation on all supported models.
...
The `<met>` tag specifies the meteorological forcing data used to drive the model simulation.

Users must define either:

- `source` – the name of a supported meteorological dataset, or  
- `path` – the location of a pre-processed meteorological file.
...
- `id` – Database ID of the meteorological dataset.
- `source` – Name of the meteorological product (e.g., `CRUNCEP`, `AmerifluxLBL`).
- `path` – Direct path to an existing meteorological file.
- `output` – Model format to convert the meteorological data into.

If `source` is provided, PEcAn will automatically retrieve and process the dataset.  
If `path` is provided, PEcAn will use the file directly.
