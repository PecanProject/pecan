...
<state.data.assimilation>
  ...
  <Obs_Prep>
   ...
   <MODIS_LAI>
    <var_name>LAI</var_name>
    <outdir>/projectnb/dietzelab/dongchen/All_NEON_SDA/test_OBS</outdir>
    <search_window>30</search_window>
    <export_csv>TRUE</export_csv>
   </MODIS_LAI>
   <SMAP_SMP>
    <var_name>SoilMoistFrac</var_name>
    <outdir>/projectnb/dietzelab/dongchen/All_NEON_SDA/test_OBS</outdir>
    <search_window>30</search_window>
    <export_csv>TRUE</export_csv>
...
* **Landtrendr_AGB** : [optional] The naming convention of this section is based on "DataSource_AbbreviationOfData", if you want to proceed with the SDA_Obs_Assembler function and generate obs.mean and obs.cov of the landtrendr AGB data, you need to specify it in a similar style.
* **var_name** : [optional] The exact variable name used in the SDA workflow. it's worth noting that it's not the abbreviation of the observation (for example, AGB, SMP, or SoilC), instead, it's the name of the state variables (for example, AbvGrndWood, SoilMoistFrac, or TotSoilCarb).
* **AGB_input_dir** : [optional] This tag is only for Landtrendr_AGB_prep function, which points to the directory where the pre-downloaded AGB files existed.
* **allow_download** : [optional] This tag is only for Landtrendr_AGB_prep function, which decide if we want to proceed to download the AGB files (that will take a while, and a lot of space) if some of them are detected to be empty.
* **outdir** : [optional] This tag is for every observation preparation function, which points to the directory where the csv file associated with each data stream should be generated, read, or updated.
* **export_csv** : [optional] This tag is for every observation preparation function except Soilgrids_SoilC_prep function, which decide if we want to export the csv file to the outdir. Normally the outdir should be given in order to set export_csv as TRUE.
* **search_window** : [optional] This tag is used for LAI and SMAP observation preparation functions, which defines the search window within which the closest observation is selected.
* **update_csv** : [optional] This tag is used for SMAP preparation function only, which decide if we want to update the CSV file. Normally, we will only set it to be TRUE if we redownload the SMAP_GEE csv file from Google Earth Engine, the tutorial can be found in the SMAP_SMP_prep function itself.
* **timestep** : [optional] This tag defines the time unit and how many unit time per each time step. It currently only support year or day as the time unit.
