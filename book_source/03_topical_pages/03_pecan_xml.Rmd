...
<state.data.assimilation>
  ...
  <Obs_Prep>
   <AGB>
    <Var>AbvGrndWood</Var>
    <AGB_dir>/projectnb/dietzelab/dongchen/Multi-site/download_500_sites/AGB</AGB_dir>
    <Time_Step>
     <unit>year</unit>
     <num>1</num>
    </Time_Step>
    <Allow_download>TRUE</Allow_download>
    <Out_dir>/projectnb/dietzelab/dongchen/All_NEON_SDA/test_OBS</Out_dir>
...
* **Obs_Prep** : [required] You will need to specify required arguments for different var prep functions (LAI_prep, AGB_prep, SMP_prep, and the SoilC_prep functions). For example, you will need to specify the Var_name (LAI), OutDir, Search_Window, NCore, Time_Step, and Export_CSV under the Obs_Prep/LAI section to make the LAI_prep function work. You will also need to specify the start and end date directly under the Obs_Prep section. More details can be found in prep functions themselves.
