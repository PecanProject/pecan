...
Although JSON is the official PEcAn format for management events, PEcAn supports more efficient storage of large collections of event data in parquet format. Such data can be converted to the JSON format with `data.land::event_parquet_to_json`. Data for each type of managmeent event must be stored in its own parquet file (e.g., you _cannot_ combine data for leaf on and leaf off events in a `phenology.parquet` file). Each parquet file must have columns `date`, `site_id`, and additional columns for event-specific data (e.g., `method` and `amount_mm` for irrigation). For ensemble uncertainty analysis, events may also (optionally, but highly recommended) include an `event_member_id` column (as a character) that uniquely identifies each ensemble member for downstream uncertainty analysis. (This is distinct from PEcAn's ensemble ID because PEcAn's ensemble code treats all management events as a single input type. The `event_member_id` for each event type is therefore used to extend PEcAn's ability to attribute sensitivity or uncertainty to specific event types. See `data.land::event_parquet_to_json` docs for additional details).
