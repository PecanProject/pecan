...
- Using the PEcAn web interface (LINK) 
- By hand using the `pecan.xml` (LINK)

### The Benchmarking Shiny App

The entire benchmarking process can be done through the Benchmarking R Shiny app. 

When the model run has completed, navigate to the workflow visualization Shiny app. (LINK)
...
- To view interactive results, naviage to the 
...
The `pecan.xml` has an _optional_ benchmarking section. 

Below are all the tags in the benchmarking section explained. Many of these field are filled in automatically during the benchmarking process when using the benchmarking shiny app. 

The only time one should edit the benchmarking section by hand for performing clone runs. See documentation [below](#CloneRun)
