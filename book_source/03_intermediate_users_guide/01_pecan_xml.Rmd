...
2. [PEcAn Folders]
3. [Database Access]
4. [BETY Configuration](#bety-database-configuration)
5. [Brown Dog](#brown-dog-configuration)
6. [PFT Selection](#pft-selection)
7. [Meta Analysis](#meta-analysis)
8. [Ensemble Runs](#ensemble-runs)
9. [Sensitivity Runs](#sensitivity-runs)
10. [Model Setup](#model-setup)
11. [Run Setup](#run-setup)
12. [State Data Assimilation](#state-data-assimilation-tags)
13. [Parameter Data Assimilation]
14. [Benchmarking]

## Info
The information section, tagged `info`, contains information specific to your run. They are not critical settings, so any missing values will not break your run. 

An example of this section looks like:
```xml
<info>
  <notes>Test_Run</notes>
  <userid>99000000036</userid>
  <username>guestuser</username>
  <date>2018/09/05 16:21:45 +0000</date>
 </info>
```

The `<notes></notes>` tag will be filled in by the web GUI if you provide notes, or you can add notes yourself within these tags. We suggest adding notes that help identify your run and a brief description of what the run is for.

The `<userid></userid>` and `<username></username>` section is filled in from the GUI if you are signed in. If you are not using the GUI, add the user name and ID you are associated with that exists within the BETY database.
...
The following tag is used to configure the output folder used by PEcAn.
...
New format (starting with PEcAn 1.5.3)
```xml
<database>
	<bety>
		<user>bety</user>
...
* **write** : [optional] should any results be written back to the database, setting this to TRUE (the default) will write any results back into the database. (since PEcAn 1.5.3)

For first time installers, check your `config.php` in the web directory of pecan. These settings are derived from there. If you are editing the pecan.xml by hand, check the `config.php` file for settings.

For other database drivers these parameters will change. See the driver documentation in R for the right parameters.

The fia tag correspond to connecting to the FIA database through PEcAn. Stability not ensured as of 1.5.3.
