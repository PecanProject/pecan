...
- `PEcAn.<MODEL>::write.config.<MODEL>` - Write model-specific configuration files
...
- `quantiles` : [optional] Quantiles of parameter distributions at which the model should be evaluated when running sensitivity analysis. Values must be between 0 and 1.
- `sigma` : [optional] The number of standard deviations (i.e. "Z-score") to use in the analysis. Basically, this is an alternative way to specify quantiles. For instance, `<sigma>1</sigma>` corresponds to the quantile associated with 1 standard deviation greater than the mean (i.e. 0.681). Note that we _do not automatically add the quantile associated with `-sigma`_ -- i.e. if you want +/- 1 standard deviation, then you must include both `<sigma>1</sigma>` _and_ `<sigma>-1</sigma>`.
