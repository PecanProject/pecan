...
**NOTE: Instructions are only tested for Ubuntu 16.04 on the VM.**
...
First step is to install Tomcat 8 and configure it.
...
To customize the thredds server for your installation edit the file in /home/caray/thredds/threddsConfig.xml. For example the following file is included in the VM.
...
For example to update the catalog with the latest data, run the following command from the root crontab.

```
0 * * * * /home/carya/pecan/scripts/thredds.sh > /home/carya/thredds/catalog.xml && service tomcat8 restart
```
