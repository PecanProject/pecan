...
create_status_page <- function(config_file, file_prefix = "status", delta = 3600) {
  ...
  for (i in seq_len(nrow(pecan_nodes))) {
    ...
    if (RCurl::url.exists(version_url)) {
      ...
      if (length(version) == 4) {
        ...
        if ((hostname %in% nodes) && (nodes[[hostname]]["schema"] != schema)) {
          msg <- paste(
            Sys.time(), "SCHEMA UPDATE DETECTED ON NODE", x$sync_host_id, hostname,
    ...
    if (RCurl::url.exists(log_url)) {
      ...
      for (row in log) {
        ...
        if (!is.na(sync.cols[7])) {
          sync[[as.character(sync.cols[7])]] <- list(
            id = sync.cols[7],
    ...
    nodes[[as.character(x$sync_host_id)]] <- list(
      sync_host_id = x$sync_host_id,
      sync_url = x$sync_url,
      hostname = hostname,
  ...
  xlim <- grDevices::extendrange(sapply(nodes, function(x) {
    x$lon
  }), f = 1)
  ylim <- grDevices::extendrange(sapply(nodes, function(x) {
    x$lat
  }), f = 1)
