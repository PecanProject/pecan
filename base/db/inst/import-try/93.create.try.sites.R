...
latlon.unique[, c("site.Latitude", "site.Longitude") := list(
...
try.latlon.merge <- try.latlon.cast[latlon.unique[, list(
  latlon.id, try.site.id,
...
try.dat <- try.dat[try.latlon.merge[, list(
  ObservationID, latlon.id, try.site.id,
  site.Latitude, site.Longitude
)]]
