...
priordupe <- function(parent.pft.name = NULL,
  ...
  parent.pft.id <- db.query(query = paste("select id from pfts where name = ", parent.pft.name, ";"),
                            con = con)

  ## create new pft
  db.query(
    query = paste(
      "insert into pfts set definition = ", newpftdefn,
      " name = ", new.pft.name, ";"
    ),
    con = con
  )
  new.pft.id <- db.query(query = paste("select id from pfts where name =",
                                       new.pft.name,";"), con = con)

  old.species.id <- db.query(query = paste("select specie_id from pfts_species where pft_id =",
                                     parent.pft.id, ";"), con = con)
  new.pfts_species <- c(pft_id = new.pft.id, specie_id = unique(old.species.id))

  db.query(
    query = paste(
      "insert into pfts_species set pft_id = ", new.pfts_species$pft_id,
      "specie_id = ", new.pfts_species$specie_id, ";"
    ),
    con = con
  )

  old.priors <-  db.query(query = paste("select prior_id from pfts_priors where pft_id =",
                                        parent.pft.id, ";"), con = con)
  new.pfts_priors <- c(pft_id = new.pft.id,
                       prior_id = unique(old.priors))
  db.query(
    query = paste(
      "insert into pfts_priors set pft_id = ", new.pfts_priors$pft_id,
