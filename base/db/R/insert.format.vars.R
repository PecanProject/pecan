  ...
  if(!is.null(formats_variables)){
    for(i in 1:nrow(formats_variables)){
      ...
      if(suppress == FALSE){
        ...
        var_id_test <- dplyr::tbl(con, "variables") %>% dplyr::select(.data$id) %>% dplyr::filter(.data$id %in% !!formats_variables[[i, "variable_id"]]) %>% dplyr::collect(.data$id)
