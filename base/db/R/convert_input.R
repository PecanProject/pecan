...
##'
##' \code{convert_input} is a relatively generic function that applies the function \code{fcn} and inserts a record of it into the database.
##' It is primarily designed for converting meteorological data between formats and can be used on observed data, forecasts, and ensembles of forecasts.
...
##'   Currently only \code{host$name} is used by \code{convert_input}, but whole list is passed to other functions
