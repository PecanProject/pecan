...
start.model.runs <- function(settings, write = TRUE, stop.on.error = TRUE) {
  ...
  for (run in run_list) {
    ...
    if (is_rabbitmq) {
      ...
      out <- start_qsub(
        run = run, qsub_string = settings$host$qsub, rundir = settings$rundir,
    ...
    if (!is_local) {
      for (run in run_list) {
        if (run %in% job_modellauncher) {
          ...
          PEcAn.remote::remote.copy.to(settings$host, file.path(
            settings$rundir,
            format(run, scientific = FALSE)
          ), settings$host$rundir, delete = TRUE)
        }
      }
    }
    if (is_qsub) {
      ...
      out <- start_serial(
        run = run, host = settings$host, rundir = settings$rundir, host_rundir = settings$host$rundir,
