  ...
  for (trait in traits) {
    for (quantile in quantiles) {
      ...
      for (var in seq_along(variables)) {
        ...
        out.tmp <- read.output(
          runid = run.id, outdir = file.path(outdir, run.id),
      ...
      for (quantile.str in quantiles.str) {
        if (quantile.str != MEDIAN) {
          ...
          if (!is.null(con)) {
            ...
            PEcAn.DB::db.query(paste0(
              "INSERT INTO runs (model_id, site_id, start_time, finish_time, outdir, created_at, ensemble_id, parameter_list) values ('",
              settings$model$id, "', '",
              settings$run$site$id, "', '",
              settings$run$start.date, "', '",
              settings$run$end.date, "', '",
            ...
            run.id <- PEcAn.DB::db.query(paste0(
              ...
              now, "' AND parameter_list='", paramlist, "'"
            ), con = con)[["id"]]

            # associate posteriors with ensembles
            for (pft in defaults) {
              PEcAn.DB::db.query(paste0(
                "INSERT INTO posteriors_ensembles (posterior_id, ensemble_id, created_at, updated_at) values (",
                pft$posteriorid, ", ",
                ensemble.id, ", '",
                now, "', '", now,
                "');"
            ...
            if (!is.null(inputs)) {
              for (x in inputs) {
                PEcAn.DB::db.query(paste0(
                  "INSERT INTO inputs_runs (input_id, run_id, created_at) ",
