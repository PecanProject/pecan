...
convert.input <-
  function(input.id,
    ...
    PEcAn.logger::logger.debug(paste(
      "Convert.Inputs", fcn, input.id, host$name, outfolder, formatname,
    ...
    PEcAn.logger::logger.info(paste(
      "start CHECK Convert.Inputs", fcn, input.id, host$name, outfolder,
    ...
    if (forecast) {
      ...
      for (i in seq_len(ensemble)) {
        ...
        existing.dbfile[[i]] <- PEcAn.DB::dbfile.input.check(
          siteid = site.id,
          mimetype = mimetype,
        ...
        if (nrow(existing.dbfile[[i]]) > 0) {
          ...
          existing.machine <- PEcAn.DB::db.query(paste0(
          ...
          machine <- PEcAn.DB::db.query(paste0(
          ...
          if (existing.machine$id == machine$id) {
            if (overwrite) { # If the files are on the current machine, check to see if we should overwrite them.
              ...
              files.to.delete <- c(files.to.delete, as.list(PEcAn.remote::remote.execute.R(paste0(
                "list.files('",
      ...
      if (!is.null(input.args$dbfile.id)) {
        existing.dbfile <- PEcAn.DB::dbfile.input.check(
          siteid = site.id,
          mimetype = mimetype,
          formatname = formatname,
          parentid = input.id,
      ...
      if (nrow(existing.dbfile) > 0) {
        ...
        if (overwrite) {
          ...
          files.to.delete <- PEcAn.remote::remote.execute.R(paste0(
            "list.files('",
    ...
    if (write) {
      ...
      for (i in 1:length(result)) { # Master for loop
        ...
        if (insert.new.file && id_not_added) {
          dbfile.id <- PEcAn.DB::dbfile.insert(
          ...
          new_entry <- PEcAn.DB::dbfile.input.insert(
            in.path = dirname(result[[i]]$file[1]),
            in.prefix = result[[i]]$dbfile.name[1],
            siteid = site.id,
            startdate = start_date,
