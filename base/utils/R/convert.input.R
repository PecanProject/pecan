##' 
##' @section Description:
##' convert.input is a relatively generic function that applies the function fcn and inserts a record of it into the database.
##' 
##' In order to minimize downloading and storing duplicate data, convert.input first checks to see if a given file is already in the
##' database before applying the function fcn.
##' 
##' convert.input handles forecast data as well as observed data.  In addition, convert.input handles forecasts that are ensembles of predictions.
##' 
...
##' @param formatname met specific format name
##' @param mimetype met specific file format
...
##' @param write Write new file records to the database? (logical)
##' @param format.vars
##' @param overwrite If a file already exists, create a fresh copy? (logical) Passed allong to fcn
##' @param exact.dates For non-forecast files, convert.input attempts to minimize
##' @param allow.conflicting.dates Should overlapping years ignore download optimizations and exist as separate files? (logical)
##' @param insert.new.file 
##' @param pattern A regular expression to search 
##' @param forecast Is the data product a forecast? (logical)
