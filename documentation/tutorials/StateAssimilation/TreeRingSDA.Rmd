...
<state.data.assimilation>
  ...
  <state.variables>
    ...
    <variable>LeafC</variable>
    <variable>SoilMoistFrac</variable>
    <variable>SWE</variable>
    <variable>Litter</variable>
    <variable>Snow</variable>
  </state.variables>
  <spin.up>
    <start.date>1979/01/01</start.date>
    <end.date>1979/12/31</end.date>
  </spin.up>
...
* In the PEcAn History, go to your PDA run and open ```pecan.PDA.POST.xml``` (the one saved AFTER you finished the PDA)

* Cut-and-paste the PDA ```<pfts>``` block into the SDA settings file

* Save the file as ```pecan.SDA.xls```
...
* If you have not done so already, clone (new) or pull (update) the PalEON Camp2016 repository
 ...
 + To pull: ```cd Camp2016; git pull git@github.com:PalEON-Project/Camp2016.git master```
