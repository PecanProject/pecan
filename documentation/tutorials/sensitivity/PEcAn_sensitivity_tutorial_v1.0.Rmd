...
You can find this file (*pecan.xml*) in the *~/output* folder within the specific run directory (e.g., *~/output/PEcAn_99000000002/pecan.xml*), where *99000000002* is the BETY database id of the workflow that called all the ensemble and sensitivity runs of SIPNET. 
...
####A. First, let’s look at the sensitivity analysis output, and set up our variables
...
Finally, open up a connection to the bety database and create a settings object from your xml, just like in the modelVSdata tutorial. 
```{r, eval = FALSE}
settings<-read.settings("/fs/data2/output/PEcAn_1000007698/pecan.CONFIGS.xml")
...
```{r,eval=FALSE}
arun <- read.output(runids[1], paste(settings$outdir, "out", runids[1], sep="/"), start.year= start.year, end.year= end.year,"NEE", dataframe = TRUE)
...
In the earlier tutorial, you also compared NEE to the ensemble model run. Here we will do the same except we include each sensitivity run. 
