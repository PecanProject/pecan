...
RunDir<-'/fs/data2/output//PEcAn_1000007678'

#Open a connection to the bety database
...
```{r, eval=FALSE}
load("/fs/data2/output/PEcAn_1000007678/sensitivity.samples.1000014062.Rdata")
load("/fs/data2/output/PEcAn_1000007678/sensitivity.output.1000014062.NPP.2004.2004.Rdata")
names(sa.samples)
...
```{r,eval=FALSE}
arun<-read.output()
#arun <- read.output(runids[1], paste("~/output", RunDir, "out", runids[1], sep="/"), start.year= 2004, end.year= 2004, c('time',"NEE"), dataframe = TRUE)
arun <- read.output(runids[1], paste( RunDir, "out", runids[1], sep="/"), start.year= 2004, end.year= 2004, c('time',"NEE"), dataframe = TRUE)
plot(arun$time,arun$NEE)
```

####B. Now let’s bring in the actual observations
Recall reading Ameriflux NEE in the earlier tutorial. Read in an Ameriflux csv file, which is downloadable from Ameriflux. Here *722* is the database site ID which allows bety to assign the correct location of the data. The *722* may be a different number in your system depending on the site you chose and the database contents. Also note *US-NR1*, which refers to the Fluxnet site identifier for Niwot Ridge.
```{r,eval=FALSE}
...
In the earlier tutorial, you also compared NEE to the ensemble model run. Here we will do the same except for each sensitivity run. Recall that we made a scatter plot. 

Since we alinged the data, we got a dataframe with the variables we requested in a ```{r}NEE.m``` and a ```{r}NEE.o``` format. The ```{r}.o``` is for observations, and the ```{r}.m ``` is for model. The posix column allows for easy plotting along a timeseries.  
```{r,eval=FALSE}
...
arun<-read.output(runids[r], paste( RunDir, "out", runids[1], sep="/"), start.year= 2004, end.year= 2004, c('time',"NEE"), dataframe = TRUE)
plottable<-align_data(model.calc = arun, obvs.calc = obs, align_method = "match_timestep", var= "NEE")
...
```{r,eval=FALSE}
unc <- flux.uncertainty(plottable$NEE.o,QC=rep(0,17520)) #Note to self: Not sure this is really the best object to use here. 
