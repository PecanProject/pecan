...
site<-PEcAn.DB::query.site(site.id = 772,bety$con) #site.id is set to Niwot Ridge 

obs<-PEcAn.benchmark::load_data(data.path = File_Path, format= File_format, time.row = File_format$time.row,  site = site, start_year = 2004, end_year = 2004) #This will throw an error that not all of the units can be converted. That's ok, as the units of the varibles of interest (NEE) are being converted. 
...
```{r,eval=FALSE}
plot(obs$NEE,arun$NEE)
abline(0,1,col="red")
```

And remember the formula for RMSE:
```{r,eval=FALSE}
sqrt(mean((obs$NEE-arun$NEE)^2,na.rm = TRUE))	
```
...
arun <- read.output(runids[r],paste("~/output",RunDir,"out",runids[r],sep="/"),2004,2004,"NEE")
rmses[r] <- (sqrt(mean((obs$NEE-arun$nee)^2,na.rm = TRUE)))
}
```
...
```{r,eval=FALSE}
unc <- flux.uncertainty(arun$nee,QC=rep(0,17520)) #Note to self: Not sure this is really the best object to use here. 
