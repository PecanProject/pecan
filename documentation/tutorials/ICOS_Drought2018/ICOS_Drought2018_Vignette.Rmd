...
europe_cropped <-
        world_map %>% dplyr::filter((
          world_map$lat >= 30 & world_map$lat <= 75 &
...
ggplot(europe_cropped, aes(x = long, y = lat)) +
  ...
  geom_point(data=icos_d2018, aes(x=Longitude, y=Latitude))
...
end_date   <- "2018-01-01"
PEcAn.data.atmosphere::download.ICOS(sitename, outfolder, start_date, end_date)
