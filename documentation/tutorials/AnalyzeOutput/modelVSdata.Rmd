...
There are two ways to access runs from pecan. One is from a settings object generated from an xml file, and the other is to query the database using the run ID, site ID, and output files manually. These ID's can all be found in the run README file. Both are shown below, so only run one.   

**From an XML file**
```{r}
## Read in the xml

settings<-PEcAn.settings::read.settings("/fs/data2/output/PEcAn_99000000001/pecan.CONFIG.xml")
...
File_path<- settings$run$inputs$met$path
...
site.id<-772
File_Path<-"/fs/data3/tmccabe/AMF_US-NR1_BASE_HH_9-1.csv" #This is a path to where the observations are stored. If you have your own observations, change to cusom file path. 
