...
```
library(PEcAn.utils)
out <- read.output(99000000001, "output/PEcAn_99000000001/out/99000000001/" ,2006,2006,c("time","NEE"))
plot(out$time,out$NEE,type='l',ylab="kg ha-1 yr-1",xlab="day of year")
```
...
Here 0-722 is the database site ID, a folder that was created when you selected the site and PEcAn downloaded the Ameriflux NetCDF (.nc) file from the Ameriflux website. 
...
```
modNEE <- ud.convert(out$NEE,"kg ha-1 yr-1","ug m-2 s-1")/12.0 #12 g C per mol
```
