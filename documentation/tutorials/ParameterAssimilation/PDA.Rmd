...
* Set up and run a PDA in PEcAn using model emulation technique, assimilating NEE data from Sylvania
...
##   Search by Ameriflux ID (e.g. US-Syv)
...
datadir     <- "/home/carya/output/dbfiles/Ameriflux_site_0-622/US-Syv." 
...
NEEo <- ud.convert(obs$NEE,"ug m-2 s-1","kg m-2 s-1")*12.0  ## convert units
```
...
run.ensemble.analysis(plot.timeseries=TRUE, observations=NEEo, window=48, variable="NEE")
```
...
For the purposes of this demo, choose three to five parameters that contribute high uncertainty to model output and/or seem like good choices for some other rational reason. 
...
In this block, use the ```<param.names><param>``` tags to identify the parameters you’ve chosen for PDA (it's up to you to choose the number of parameters you want to constrain, then you can set the ```<n.knot>``` to be >= 10 per parameter you choose, e.g. 60 knots for 3 parameters).  Here, you need to use PEcAn’s standard parameter names, which are generally not the same as what’s printed on your variance decomposition graph. To find your parameters look at the row names in the ```prior.distns.csv``` file for each PFT under the PFT pulldown menu. Insert the variable name (exactly, and case sensitive) into the ```<param>``` tags of the XML code below. 
...
<pecan>                  <-- just paste the <assim.batch> block below right after them. 
  <assim.batch>
    ...
    <iter>10000</iter>
    <param.names>
     <param>YOUR_PARAM_1</param>              <-- FILL IN
     <param>YOUR_PARAM_2</param>              <-- FILL IN
     <param>YOUR_PARAM_3</param>              <-- FILL IN
    </param.names>
    <jump>
     <adapt>100</adapt>
     <adj.min>0.1</adj.min>
    ...
    <inputs>
     <file>
      <path>~/output/dbfiles/Ameriflux_site_0-622/US-Syv.2004.nc</path>   <-- FILL IN
      <format>Ameriflux.L2</format>
      <input.id>1000000463</input.id>                                     <-- FILL IN
...
saveXML(listToXml(settings, "pecan"), file=file.path(settings$outdir, 'pecan.PDA.POST.xml'))
```
...
When the PDA is finished, a number of outputs are automatically produced that are either the same as or similar to posterior outputs that we’ve seen before. These are located in the ```PEcAn_[workflow_id]/pft/*``` output directory and are identified by ```pda.[workflow_id]``` in the filenames:

* posteriors.pda.*.pdf shows the posterior distributions resulting from your PDA
*	trait.mcmc.pda.*.Rdata contains all the parameter samples contained in the PDA posterior 
*	mcmc.pda.*.Rdata is essentially the same thing in a different format
*	mcmc.diagnostics.pda.*.pdf shows trace plots and posterior densities for each assimilated parameter, as well as pairs plots showing all pairwise parameter correlations. 
...
The PDA you ran has automatically produced an updated XML file that includes the posterior id to be used in the next round of runs. Locate this file in your run directory and load the file for the post-pda ensemble/sensitivity analysis:
