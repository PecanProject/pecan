...
prePDA_ensemble_output_file <- file.path(outdir,paste0("PEcAn_",workflow_id, "/ensemble.ts.", settings$ensemble$ensemble.id, ".AbvGrndWood.1709.2009.Rdata"))
load(prePDA_ensemble_output_file)

# calculate CI
pre_pda_ens <- ensemble.ts[["AbvGrndWood"]]
...
```{r eval=FALSE}
 ...
 postPDA_ensemble_output_file <- file.path(outdir,paste0("PEcAn_", workflow_id, "/ensemble.ts.", postPDA.settings$ensemble$ensemble.id, ".AbvGrndWood.1709.2009.Rdata"))
 load(postPDA_ensemble_output_file)
 post_pda_ens <- ensemble.ts[["AbvGrndWood"]]
 ...
 post_pda <- t(apply(post_pda_ens, 1, function(x) {
        tapply(x, rep(1:(length(x)/window + 1), each = window)[1:length(x)], 
               mean, na.rm = TRUE)}))
 fobs <- tapply(AGBo, rep(1:(length(AGBo) / window + 1), 
                          each = window)[1:length(AGBo)], mean, na.rm = TRUE)
 
 
 # save the comparison plots to pdf
 pdf(file.path(outdir,paste0("PEcAn_",workflow_id),"model.data.comparison1.pdf"), onefile=T,paper='A4r', height=15, width=20) 
 ...
 plot(1709:2009,pre_pda[1,], ylim = c(ymin, ymax), lwd = 2, xlab = "time", ylab = "NEE", main = "pre-PDA vs post-PDA", type = "n",xlim=c(1709,2015))
 prepoly <- 1709:2009
 polygon(c(prepoly, rev(prepoly)),c(preCI[3,], rev(preCI[1,])),col='khaki',border=NA)
 ...
 points(x = 1998:2014,y = fobs, pch = ".", col= adjustcolor("purple",alpha.f=0.7))
