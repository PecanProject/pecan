...
# PRIVATE FUNCTIONS
# ----------------------------------------------------------------------
data.fetch2 <- function(var, data, start, end, values, fun=mean) {
	...
	# aggregate the data
	val <- aggregate(data[[useme]][indices], by=aggrlist, FUN=fun)$x
...
# 
data.fetch <- function(var, data, start, end, values, fun=mean) {
	...
	if (var == "time") {
		...
		PLANT_RESP  <- data.fetch2("AVG_PLANT_RESP", data, start, end, values, fun);
		HTROPH_RESP <- data.fetch2("AVG_HTROPH_RESP", data, start, end, values, fun);
		val         <- (PLANT_RESP + HTROPH_RESP) * umol2gc
		attr(val, "lbl") <- "unknown"
		return(val)
	} else if (var == "NPP") {
		GPP         <- data.fetch2("AVG_GPP", data, start, end, values, fun);
		PLANT_RESP  <- data.fetch2("AVG_PLANT_RESP", data, start, end, values, fun);
		...
		return(data.fetch2(var, data, start, end, values, fun));
	}
}
...
plot.hdf5 <- function(year, yvar, xvar='time', width=800, height=600, filename, settings) { 
	...
	# find the Tower file
	h5file <- list.files(settings$run$host$outdir, full.names=TRUE,pattern=paste('.*-T-', year, '-.*.h5', sep=''))[1]
	data <- hdf5load(h5file, load = FALSE)
	
	# compute variables
	xval_mean <- data.fetch(xvar, data=data, start=start_day, end=end_day, values=values_day, fun=mean)
	xval_max  <- data.fetch(xvar, data=data, start=start_day, end=end_day, values=values_day, fun=max)
	xval_min  <- data.fetch(xvar, data=data, start=start_day, end=end_day, values=values_day, fun=min)
	yval_mean <- data.fetch(yvar, data=data, start=start_day, end=end_day, values=values_day, fun=mean)
	yval_max  <- data.fetch(yvar, data=data, start=start_day, end=end_day, values=values_day, fun=max)
	yval_min  <- data.fetch(yvar, data=data, start=start_day, end=end_day, values=values_day, fun=min)
