...
query.format.vars <- function(input.id, con, format.id) {
  ...
  mimetype <- db.query(paste("SELECT * from  mimetypes where id = ", f$mimetype_id), 
                       con)[["type_string"]]
  ...
  if (nrow(fv) > 0) {
    ...
    vars_full <- merge(fv, merge(vars, bety_mstmip, by = "bety_name", all.x = TRUE), 
                       by = "variable_id", all = TRUE)  # not sure if all=TRUE is appropriate here or not.
