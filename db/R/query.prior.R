...
query.priors <- function(pft, trstr = NULL, out = NULL, con = NULL, ...) {
  ...
  query.text <- paste("select variables.name, distn, parama, paramb, n", "from priors", 
                      "join variables on priors.variable_id = variables.id", "join pfts_priors on pfts_priors.prior_id = priors.id", 
                      "join pfts on pfts.id = pfts_priors.pft_id", "where pfts.id = ", pft)
  if (is.null(trstr) || trstr == "''") {
    query.text <- paste0(query.text, ";")
  } else {
