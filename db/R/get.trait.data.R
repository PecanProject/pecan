...
##' @export
##'
get.trait.data.pft <- function(pft, modeltype, dbfiles, dbcon, forceupdate = TRUE, 
  ...
  write.csv(ldply(trait.data), file = file.path(pft$outdir, "trait.data.csv"), 
            row.names = FALSE)
...
get.trait.data <- function(pfts, modeltype, dbfiles, database, forceupdate, trait.names = NULL) {
  ...
  result <- lapply(pfts, get.trait.data.pft, modeltype, dbfiles, dbcon, 
                   forceupdate, trait.names)
  db.close(dbcon)
