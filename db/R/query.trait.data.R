...
query.yields <- function(trait = "yield", spstr, extra.columns = "", con = NULL, ...) {
  query <- paste("select
          ...
          where specie_id in (", 
                 spstr, ");", sep = "")
  if (!trait == "yield") {
    query <- gsub(");", paste(" and variables.name in ('", trait, "');", sep = ""), 
                  query)
  }
...
arrhenius.scaling.traits <- function(data, covariates, temp.covariates, new.temp = 25, 
  ...
  if (nrow(covariates) > 0) {
    ...
    covariates <- do.call(rbind, lapply(temp.covariates, 
...
rename.jags.columns <- function(data) {
  ...
  # Subset data frame
  selected <- subset(transformed, 
                     select = c("Y", "n", "site", "trt", "ghs", "obs.prec", "se", "cite"))
...
query.trait.data <- function(trait, spstr, con = NULL, update.check.only = FALSE, ...) {
  ...
  ### Set small sample size for derived traits if update-checking only. Otherwise use
  ### default.
  if (update.check.only) {
    sample.size <- 10
  } else {
  ...
  if (trait == "Vcmax") {
    ...
    #########################    SLA    ############################
    ## convert LMA to SLA
    data <- rbind(data, derive.traits(function(lma) { 1 / lma }, 
                                      query.data("LMA", 
                                                 spstr, 
                                                 con = con, 
                                                 store.unconverted = TRUE), 
                                      sample.size = sample.size))
    ...
    #########################    LEAF TURNOVER    ############################
    ## convert LMA to SLA
    data <- rbind(data, derive.traits(function(leaf.longevity) { 1 / leaf.longevity },
                                      query.data("Leaf Longevity", 
                                                 spstr, 
                                                 con = con,
                                                 store.unconverted = TRUE), 
                                      sample.size = sample.size))
    ...
    data <- rbind(data, derive.traits(function(leafN) { 48 / leafN }, 
                                      query.data("leafN", 
                                                 spstr, 
                                                 con = con, 
                                                 store.unconverted = TRUE),
  ...
  if (nrow(result) == 0) {
    ...
    ## print(result)
    if (!update.check.only) {
      print(paste0("Median ", trait, " : ", 
                   round(median(result$mean, na.rm = TRUE), digits = 3)))
      print("---------------------------------------------------------")
    }
