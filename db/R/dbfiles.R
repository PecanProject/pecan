...
dbfile.input.insert <- function(in.path, in.prefix, siteid, startdate, enddate, mimetype, formatname, parentid=NA, con, hostname=fqdn(), allow.conflicting.dates=FALSE) {
  ...
  if (nrow(existing.input) > 0) {
    ...
    startdate <- lubridate::force_tz(lubridate::as_date(startdate), 'GMT')
    enddate <- lubridate::force_tz(lubridate::as_date(enddate), 'GMT')
    existing.input$start_date <- lubridate::force_tz(lubridate::as_date(existing.input$start_date), 'GMT')
    existing.input$end_date <- lubridate::force_tz(lubridate::as_date(existing.input$end_date), 'GMT')
