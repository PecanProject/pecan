...
sda.enkf.multisite <- function(settings, 
        ...
        for (i in length(settings$run$inputs$met$path)) {
          ...
          settings$run$inputs$met$path[[i]] <- do.call(
            ...
            args = list(
              ...
              outpath = paste("/projectnb/dietzelab/dongchen/Multi-site/SDA/met",settings$run$site$id,sep = ""),
              overwrite =F
            )
          )
  ...
  for(t in sim.time){
    ...
    #tryCatch({
      ...
      source("/projectnb/dietzelab/dongchen/Multi-site/Write_ensemble_config.R")
      if (control$debug) browser()
      ...
      reads <- out.configs %>%
        `class<-`(c("list")) %>%
        furrr::future_map2(settings,function(configs,settings) {
          # Loading the model package - this is required bc of the furrr
          library(paste0("PEcAn.",settings$model$type), character.only = TRUE)
