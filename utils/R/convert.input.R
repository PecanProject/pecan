...
convert.input <- function(input.id, outfolder, formatname, mimetype, site.id, start_date, 
  ...
  if (nrow(existing.dbfile) > 0) {
    ...
    start_date <- lubridate::force_tz(as_date(start_date), "UTC")
    end_date   <- lubridate::force_tz(as_date(end_date), "UTC")
    existing.input$start_date <- lubridate::force_tz(as_date(existing.input$start_date), 
                                          "UTC")
