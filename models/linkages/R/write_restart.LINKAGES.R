...
write_restart.LINKAGES <- function(outdir, runid, start.time, stop.time,
  ...
  if (FALSE) {
    distance.matrix <- rbind(
      c(0, 1, 4, 3, 2, 6, 5, 8, 7, 9, 10, 11, 12, 13, 14),
  ...
  for (j in seq_len(sum(ntrees))) {
    ...
    spp.biomass.params <- biomass_spp_params(
  ...
  for (s in seq_len(nspec)) {
    ...
    b_calc[s] <- sum(biomass_function(dbh.temp[nl:nu],
      spp.biomass.params = spp.biomass.params
    )) * (1 / 833) * 0.48 # changing units to be kgC/m^2
    ...
    bMax <- 200
    for (j in nl:nu) {
      dbh.temp[j] <- optimize(merit, c(1, bMax),
        b_obs = b_obs[j],
        spp.biomass.params = spp.biomass.params
      )$minimum
    }
  ...
  if (RENAME) {
    file.rename(
      file.path(settings$rundir, runid, "linkages.restart.Rdata"),
      file.path(settings$rundir, runid, paste0(start.time, "linkages.restart.Rdata"))
  ...
  do.call(write.config.LINKAGES,
    args = list(
      trait.values = new.params, settings = settings, run.id = runid,
  ...
  if (RENAME) {
    file.rename(
      file.path(outdir, runid, "linkages.out.Rdata"),
