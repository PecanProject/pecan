...
# R Code to convert NetCDF CF met files into NetCDF CTSM met files.

##' met2model wrapper for CTSM
##' 
##' @title met2model for CTSM
##' @export
...
met2model.CTSM <- function(in.path, in.prefix, outfolder, start_date, end_date, lst = 0, lat, lon, 
                            overwrite = FALSE, verbose = FALSE, ...) {
  
  # General Structure- CTSM Uses Netcdf so we need to rename vars, split files from years into months, and generate the header file
  # Get Met file from inpath.
  # Loop over years (Open nc.file,rename vars,change dimensions as needed,close/save .nc file)
  # close
  # defining temporal dimension needs to be figured out. If we configure CTSM to use same tstep then we may not need to change dimensions  
  ...
  PEcAn.logger::logger.info("Done with met2model.CTSM")
