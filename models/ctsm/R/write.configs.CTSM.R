...
##-------------------------------------------------------------------------------------------------#
##' Writes config files for use with CTSM.
##'
##' @name write.config.CTSM
##' @title Write CTSM configuration files
...
##-------------------------------------------------------------------------------------------------#
write.config.CTSM <- function(defaults, trait.values, settings, run.id){
   ...
   domain.default <- system.file("domain.lnd.1x1pt-brazil_navy.090715.nc",package="PEcAn.CTSM")
   ...
   surf.default <- system.file("surfdata_1x1_brazil_16pfts_Irrig_CMIP6_simyr2000_c171214.nc",package = "PEcAn.CTSM")
   ...
   if(!is.null(settings$run$inputs$met)){
     ...
     datm <- readLines(con=system.file("datm_atm_in.template",package = "PEcAn.CTSM"),n=-1)
     ...
     ## DATM STREAM MET
     met <- readLines(con=system.file("datm.streams.txt.PEcAn_met.template",package = "PEcAn.CTSM"),n=-1)
...
# create launch script (which will create symlink)
   if (!is.null(settings$model$jobtemplate) && file.exists(settings$model$jobtemplate)) {
     ...
     jobsh <- readLines(con=system.file("template.job", package = "PEcAn.CTSM"), n=-1)
   }
...
# create job.sh
   ...
   if (!is.null(settings$model$compset)) {
     ...
     #WRONG COMPSET. PLEASE CORRECT
     compset <- "I2000Clm50CTSMGs"
   }
...
#   
   ...
   write_params_CTSM(trait.values = trait.values, settings = settings, run.id = run.id)   
