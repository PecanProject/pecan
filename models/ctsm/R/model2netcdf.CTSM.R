...
##' @name model2netcdf.CTSM
##' @title Code to convert CTSM netcdf output into into CF standard
##'
##' @param outdir Location of CTSM model output
##' 
##' @examples  
##' \dontrun{
##' example.output <- system.file("case.clm2.h0.2004-01-01-00000.nc",package="PEcAn.CTSM")
##' model2netcdf.CTSM(outdir="~/")
##' }
##' 
##' @export
##'
##' @author Michael Dietze, Shawn Serbin
model2netcdf.CTSM <- function(outdir) {
    ...
    var_update <- function(out,oldname,newname,newunits=NULL,long_name=NULL){
      if (oldname %in% ncin_names) {
        ...
        ## correct way to "skip" and output variables that may be missing in the HLM-CTSM output?
        PEcAn.logger::logger.info(paste0("HLM-CTSM variable: ", oldname," not present. Skipping conversion"))
      }
      return(out)
    }
    ...
    for (year in unique(years)) {
        ...
        if (length(ysel) > 1) {
            PEcAn.logger::logger.warn("PEcAn.CTSM::model2netcdf.CTSM does not currently support multiple files per year")
        }
        
        fname <- files[ysel[1]]
        oname <- file.path(dirname(fname), paste0(year, ".nc"))
        PEcAn.logger::logger.info(paste("model2netcdf.CTSM - Converting:",  fname, "to", oname))
        ncin <- ncdf4::nc_open(fname, write = TRUE)
        ncin_names <- names(ncin$var)                               # get netCDF variable names in HLM-CTSM output
        
        
        ## CTSM time is in multiple columns, create 'time'
        mcdate <- ncdf4::ncvar_get(ncin, "mcdate")                  # current date (YYYYMMDD)
        if (length(mcdate)==1) {
          ## do we need to bother converting outputs where CTSM provides only a single timepoint for a date?
        ...
        # matches CTSM internal time var.
        ...
        ## time variable based on internal calc, nc$dim$time is the CTSM output time
