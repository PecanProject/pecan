...
# finalize
if (wind) {
  output_met_driver <- cbind.data.frame(
...
write.csv(output_met_driver,
  file = file.path(met_output_path, paste0(
    site_name, "_NGEETropics_", met_yr_subset[1], "_",
    met_yr_subset[2], "_UTC.csv"
  )), row.names = F
)
### output XML file
