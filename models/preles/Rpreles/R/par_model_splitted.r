...
## Modified by Mikko Peltoniemi 23.4.2014
##      1. made random term optional
##      2, Speed-up by splitting calculation to two pieces, dPAR0 generated f1
##         needs to be calculated only once for grid cell
## 
 
dPAR0 <- function(LAT,DOY){
...
dPAR1 <- function(f1, VPD, radmodel=1){
            ...
            if (radmodel==1 | radmodel== 11) {
                if(length(ind1)>0) eps[ind1] <-
                    rnorm(length(VPD[ind1]),mean=moments_un[1],sd=moments_un[2])
                if(length(ind2)>0) eps[ind2] <-
                    rnorm(length(VPD[ind2]),mean=moments_ov[1],sd=moments_ov[2])
                if(length(ind3)>0) eps[ind3] <-
                    rnorm(length(VPD[ind3]),mean=moments_un[1],sd=(moments_un[2]/2))
                if(length(ind4)>0) eps[ind4] <-
                    rnorm(length(VPD[ind4]),mean=moments_ov[1],sd=(moments_ov[2]/2))
            }

            ## Mikko: note that taking random term fixed may cause different GPP response
            ## in photosynthesis model, as effect of PAR is non-linear.
            if (radmodel == 2 | radmodel== 12) { 
