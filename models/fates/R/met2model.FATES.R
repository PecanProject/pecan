...
met2model.FATES <- function(in.path,in.prefix,outfolder,start_date, end_date, lst=0,lat,lon,..., overwrite=FALSE,verbose=FALSE){
  ...
  #General Structure- FATES Uses Netcdf so for now just need to rename vars.(Many not is CF standard. Need to Check that out)
...
#       
  ...
  ## extract variables. These need to be read in and converted to FATES standards
...
#print("Done with met2model.FATES")
