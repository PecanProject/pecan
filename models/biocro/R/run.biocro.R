...
run.biocro <- function(lat, lon, metpath, soil.nc = NULL, config = config, coppice.interval = 1) {
  ...
  for (i in seq_along(years)) {
    ...
    result.yeari.withmet <- merge(
      x = result.yeari.hourly,
  ...
  daily.results.inter <- dplyr::select(
    daily.results.initial, "year...1",
    "doy...2", "Stem", "Leaf",
    "Root", "AboveLitter", "BelowLitter",
    "Rhizome", "Grain", "LAI",
    "tmax", "SoilEvaporation",
    "CanopyTrans", "precip",
    "tmin", "tavg"
  )
  ...
  annual.results.initial <- dplyr::bind_cols(
    dplyr::summarize_at(
      .tbl = daily_grp,
      .vars = c(
        "Stem", "Leaf", "Root", "AboveLitter", "BelowLitter",
        "Rhizome", "Grain"
      ),
      .fun = max
    ),
  ...
  annual.results.inter <- dplyr::select(
    annual.results.initial, .data$year...1,
  ...
  col_order <- c(
    "year", "Stem", "Leaf", "Root", "AboveLitter", "BelowLitter",
