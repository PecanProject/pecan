...
model2netcdf.BIOCRO <- function(result, genus = NULL, outdir, lat = -9999, lon = -9999) {
  ...
  x <- ncdf4::ncdim_def("lat", "degrees_north",
                 vals =  as.numeric(lat),
                 longname = "station_latitude",
                 unlim = TRUE) 
  y <- ncdf4::ncdim_def("lon", "degrees_east",
                 vals = as.numeric(lon),
  ...
  for(yeari in unique(result$Year)){
      ...
      t <- ncdf4::ncdim_def("time", "days since 1700-01-01", 
    ...
    if(file.exists(ncfile)){
      ...
      nc <- ncdf4::nc_create(filename = file.path(outdir, paste0(yeari, ".nc")),
                      vars = vars)      
