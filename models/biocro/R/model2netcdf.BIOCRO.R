...
model2netcdf.BIOCRO <- function(result, genus = NULL, outdir, lat = -9999, lon = -9999) {
  ...
  x <- ncdim_def("latitude", "degrees_east", vals = as.numeric(lat), longname = "station_latitude", 
    unlim = TRUE)
  y <- ncdim_def("longitude", "degrees_north", vals = as.numeric(lon), longname = "station_longitude", 
    unlim = TRUE)
  ...
  for (yeari in unique(result$Year)) {
    ...
    c2biomass <- 0.4
    vars <- list(TotLivBiom = mstmipvar("TotLivBiom", x, y, t), RootBiom = mstmipvar("RootBiom", 
      x, y, t), StemBiom = mstmipvar("StemBiom", x, y, t), Yield = mstmipvar("Yield", 
      x, y, t), Evap = mstmipvar("Evap", x, y, t), TVeg = mstmipvar("TVeg", 
      x, y, t), LAI = mstmipvar("LAI", x, y, t))
    
    k <- ud.convert(1, "Mg/ha", "kg/m2")/c2biomass
    
    RR <- with(R, list(TotLivBiom = k * (Leaf + Root + Stem + Rhizome + Grain), 
      RootBiom = k * Root, StemBiom = k * Stem, Yield = Stem, Evap = ud.convert(SoilEvaporation + 
        CanopyTrans, "Mg/ha/h", "kg/m2/s"), TVeg = ud.convert(CanopyTrans, 
        "Mg/ha/h", "kg/m2/s"), LAI = LAI))
    
    ncfile <- file.path(outdir, paste0(yeari, ".nc"))
    if (file.exists(ncfile)) {
      nc <- nc_open(ncfile, write = TRUE)
    } else {
      nc <- nc_create(filename = file.path(outdir, paste0(yeari, ".nc")), vars = vars)
      ncatt_put(nc, 0, "description", "This is an output from the BioCro Crop model generated by the model2netcdf.BIOCRO.R function in the PEcAn.BIOCRO package; see github.com:PecanProject/pecan/wiki for more information")
    }
    
    varfile <- file(file.path(outdir, paste(yeari, "nc", "var", sep = ".")), 
      "w")
