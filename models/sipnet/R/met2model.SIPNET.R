...
met2model.SIPNET <- function(in.path, in.prefix, outfolder, start_date, end_date,
  ...
  if (year.fragment) { # in.prefix is not guaranteed to contain the file extension.
    ...
    # This function is supposed to process netcdf files,
    # so we'll search for files with the extension .nc and use those first.
  ...
  for (year in start_year:end_year) {
    ...
    if (file.exists(old.file)) {
      ...
      # If using whole-year data, the appropriate length in seconds is
      # fetched; otherwise, it is assumed that the length of time provided in
      # the time dimension of the input file is correct.
      if (year.fragment) {
    ...
    ## filter out days not included in start or end date if not a year fragment.
    ## (This procedure would be nonsensible for a year fragment,
    ## as it would filter out all of the days.)
    if (year == start_year && !year.fragment) {
      # extra days length includes the start date
      extra.days <- length(
        as.Date(paste0(start_year, "-01-01")):as.Date(start_date)
      )
      if (extra.days > 1) {
        PEcAn.logger::logger.info("Subsetting SIPNET met to match start date")
        # subtract to include start.date, add to exclude last half hour of day before
        start.row <- ((extra.days - 1) * 86400 / dt) + 1
    ...
    if (year == end_year && !year.fragment) {
      if (year == start_year) {
        ...
        # extra days length includes the end date
        extra.days <- length(
          as.Date(end_date):as.Date(paste0(end_year, "-12-31"))
        )
        if (extra.days > 1) {
          PEcAn.logger::logger.info("Subsetting SIPNET met to match end date")
    ...
    if (year.fragment) { # gets correct DOY for fragmented years
      ...
      doy.start <- doy.start.index[ifelse(
        lubridate::hour(start_date) == 0,
        1,
        lubridate::hour(start_date) / (24 / (86400 / dt))
      )]
      doy.end <- doy.end.index[ifelse(
        lubridate::hour(end_date) == 0,
        1,
        lubridate::hour(end_date) / (24 / (86400 / dt))
      )]
      # check to see if doy matches with downloaded data dims,
      # if not last time is removed
      if (length(doy) != n) {
      ...
      if (year.fragment) { # gets correct DOY for fragmented years using start date, time since start date and end date
        doy.seq <- as.Date(seq(
          from = start_date + sec[1],
          to = end_date,
          length.out = length(sec)
        ))
