...
met2model.SIPNET <- function(in.path, in.prefix, outfolder, start_date, end_date,
  ...
  for (year in start_year:end_year) {
    ...
    if (file.exists(old.file)) {
      ...
      pres <- ncdf4::ncvar_get(nc,'air_pressure') ## should be used in order to calculate accurate VPD and VPD soil
      ...
      if (!is.numeric(VPD)) {
        #use air pressure data provided by nc files, divided by 100 because qair2rh function only calculate percentage, not 0-1
