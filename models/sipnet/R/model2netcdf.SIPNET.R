...
model2netcdf.SIPNET <- function(outdir, sitelat, sitelon, start_date, end_date) {
  ...
  for (y in years){
   ...
   ## ***** Need to dynamically update the UTC offset here *****
    ...
    var <- list()
    var[[1]]  <- mstmipvar("Year", lat, lon, t, NULL)
    var[[2]]  <- mstmipvar("FracJulianDay", lat, lon, t, NULL)
    var[[3]]  <- mstmipvar("GPP", lat, lon, t, NULL)
    var[[4]]  <- mstmipvar("NPP", lat, lon, t, NULL)
    var[[5]]  <- mstmipvar("TotalResp", lat, lon, t, NULL)
    var[[6]]  <- mstmipvar("AutoResp", lat, lon, t, NULL)
    var[[7]]  <- mstmipvar("HeteroResp", lat, lon, t, NULL)
    var[[8]]  <- mstmipvar("NEE", lat, lon, t, NULL)
    #var[[9]]  <- mstmipvar("CarbPools", lat, lon, t, NULL)
    var[[9]]  <- mstmipvar("AbvGrndWood", lat, lon, t, NULL)
    var[[10]]  <- mstmipvar("LeafC", lat, lon, t, NULL)
    var[[11]]  <- mstmipvar("TotLivBiom", lat, lon, t, NULL)
    var[[12]]  <- mstmipvar("TotSoilCarb", lat, lon, t, NULL)
    var[[13]]  <- mstmipvar("TVeg", lat, lon, t, NULL)
    var[[14]]  <- mstmipvar("SoilMoist", lat, lon, t, NULL)
    var[[15]]  <- mstmipvar("SoilMoistFrac", lat, lon, t, NULL)
    var[[16]]  <- mstmipvar("SWE", lat, lon, t, NULL)
