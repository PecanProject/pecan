...
model2netcdf.SIPNET <- function(outdir, sitelat, sitelon, start_date, end_date, delete.raw, revision, overwrite = FALSE) {
  ...
  for (y in years) {
    ...
    output       <- list()
    output[[1]]  <- c(rbind(bounds[,1], bounds[,2]))
    output[[2]]  <- (sub.sipnet.output$gpp * 0.001) / timestep.s  # GPP in kgC/m2/s
    ## output[[2]] <- (sub.sipnet.output$npp*0.001) / timestep.s # NPP in kgC/m2/s. Internal SIPNET
    ## calculation
    output[[3]]  <- (sub.sipnet.output$gpp * 0.001) / timestep.s - ((sub.sipnet.output$rAboveground *
                                                                       0.001) / timestep.s + (sub.sipnet.output$rRoot * 0.001) / timestep.s)  # NPP in kgC/m2/s. Post SIPNET calculation
    output[[4]]  <- (sub.sipnet.output$rtot * 0.001) / timestep.s  # Total Respiration in kgC/m2/s
    output[[5]]  <- (sub.sipnet.output$rAboveground * 0.001) / timestep.s + (sub.sipnet.output$rRoot *
                                                                               0.001) / timestep.s  # Autotrophic Respiration in kgC/m2/s
    output[[6]]  <- ((sub.sipnet.output$rSoil - sub.sipnet.output$rRoot) * 0.001) / timestep.s  # Heterotrophic Respiration in kgC/m2/s
    output[[7]]  <- (sub.sipnet.output$rSoil * 0.001) / timestep.s  # Soil Respiration in kgC/m2/s
    ...
    output[[11]] <- (sub.sipnet.output$plantWoodC * 0.001) + (sub.sipnet.output$plantLeafC * 0.001) + 
      (sub.sipnet.output$coarseRootC + sub.sipnet.output$fineRootC) * 0.001 # Total living C kgC/m2
    output[[12]] <- (sub.sipnet.output$soil * 0.001) + (sub.sipnet.output$litter * 0.001)  # Total soil C kgC/m2
    if (revision == "r136") {
      output[[13]] <- (sub.sipnet.output$evapotranspiration * 10 * PEcAn.data.atmosphere::get.lv()) / timestep.s  # Qle W/m2
    } else {
      ...
      output[[13]] <- (sub.sipnet.output$npp * 10 * PEcAn.data.atmosphere::get.lv()) / timestep.s  # Qle W/m2
    }
    ...
    nc_var[[6]]  <- PEcAn.utils::to_ncvar("HeteroResp", dims)
    nc_var[[7]]  <- ncdf4::ncvar_def("SoilResp", units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
                          longname = "Soil Respiration") #need to figure out standard variable for this output
    nc_var[[8]]  <- PEcAn.utils::to_ncvar("NEE", dims)
    # nc_var[[7]] <- mstmipvar('CarbPools', lat, lon, t, NA)
    nc_var[[9]] <- PEcAn.utils::to_ncvar("AbvGrndWood", dims)
    nc_var[[10]] <- PEcAn.utils::to_ncvar("leaf_carbon_content", dims)
    nc_var[[11]] <- PEcAn.utils::to_ncvar("TotLivBiom", dims)
    nc_var[[12]] <- PEcAn.utils::to_ncvar("TotSoilCarb", dims)
    nc_var[[13]] <- PEcAn.utils::to_ncvar("Qle", dims)
    nc_var[[14]] <- PEcAn.utils::to_ncvar("Transp", dims)
    nc_var[[15]] <- PEcAn.utils::to_ncvar("SoilMoist", dims)
    nc_var[[16]] <- PEcAn.utils::to_ncvar("SoilMoistFrac", dims)
    nc_var[[17]] <- PEcAn.utils::to_ncvar("SWE", dims)
    nc_var[[18]] <- PEcAn.utils::to_ncvar("litter_carbon_content", dims)
