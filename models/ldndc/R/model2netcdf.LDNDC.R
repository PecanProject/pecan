...
model2netcdf.LDNDC <- function(outdir, sitelat, sitelon, start_date, end_date, delete.raw = FALSE) {
  ...
  if (all(c("physiology-subdaily.txt") %in% Subdailyfiles)) {
    ...
    physiology <- subset(read.csv(file.path(output_dir, "physiology-subdaily.txt"), header = T, sep = "\t"),
      select = c(
        "datetime", "species", "lai", "dC_co2_upt.kgCm.2.", "dC_maintenance_resp.kgCm.2.",
    ...
    harvest <- subset(read.csv(file.path(output_dir, "report-harvest.txt"), header = T, sep = "\t"),
      select = c(
        "datetime", "dC_fru_export.kgCha.1.", "dC_fol_export.kgCha.1.", "dC_frt_export.kgCha.1.",
    ...
    cut <- subset(read.csv(paste(output_dir, "report-cut.txt", sep = "/"), header = T, sep = "\t"),
      select = c(
        "datetime", "dC_fru_export.kgCha.1.", "dC_fol_export.kgCha.1.", "dC_dfol_export.kgCha.1.",
  ...
  harvest <- rbind(harvest, cut) %>%
    dplyr::group_by(.data$datetime) %>%
    dplyr::summarise(harvest_carbon_flux = sum(.data$total) / 10000) %>%
    as.data.frame()
  ...
  ldndc.out <- ldndc.raw.out %>%
    ...
    dplyr::select(
      "Year", "Day", "Step", "lai", "dC_maintenance_resp.kgCm.2.", "dC_transport_resp.kgCm.2.",
  ...
  for (y in year_seq) {
    ...
    nc_var[[9]] <- ncdf4::ncvar_def("AGB",
      units = "kg C m-2", dim = dims, missval = -999,
      longname = "above ground biomass"
    )
    nc_var[[10]] <- ncdf4::ncvar_def("below_ground_carbon_content",
      units = "kg C m-2", dim = dims, missval = -999,
