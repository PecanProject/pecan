  ...
  if (!is.null(settings$run$inputs$fielddata)) {
    ...
    if (!is.null(events_file$rotation)) {
      ...
      for (uic in seq_along(usmdirs)) {
        ...
        uname <- paste0(p1, p2)
        usmdirs[uic] <- paste0(
          file.path(settings$host$rundir, run.id, uname), "_",
  ...
  if (!is.null(settings$run$inputs$fielddata)) {
    ...
    for (usmi in seq_along(usmdirs)) {
      ...
      if ("harvest" %in% events_sub$mgmt_operations_event) {
        # param names
        h_param_names <- c(
          "julfauche", # date of each cut for forage crops, julian.d
          ...
          "restit",
          "mscoupemini"
        ) # amount of mineral N added by fertiliser application at each cut of a forage crop, kg.ha-1
      ...
      if ("organic_material" %in% events_sub$mgmt_operations_event |
        ...
        # param names
        f_param_names <- c(
          "julapN", # date of fertilization, julian.d
          "absolute_value/%"
        ) # cut height for forage crops, m
