...
run_BASGRA <- function(run_met, run_params, site_harvest, site_fertilize, start_date, end_date, outdir,
  ...
  if (length(outputUnits) != NOUT) {
    PEcAn.logger::logger.severe("#outputNames != #outputUnits")
  }
  ...
  for (y in years) {
    ...
    # Additional C fluxes
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "FLITTC_LEAF"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "FLITTC_ROOT"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "FHARVC"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "NEE"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "FRUNOFFC"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    outlist[[length(outlist) + 1]] <- PEcAn.utils::ud_convert(
      output[thisyear, outputNames == "FSOILAMDC"],
      "g m-2", "kg m-2"
    ) / sec_in_day
    ...
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("soil_organic_nitrogen_content",
      units = "kg N m-2", dim = dims, missval = -999,
    ...
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("SoilMoistFrac",
      units = "m3 m-3", dim = dims, missval = -999,
      longname = "Average Layer Fraction of Saturation"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("leaf_litter_carbon_flux",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "Flux of carbon from leaf litter to soil pools"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("fine_root_litter_carbon_flux",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "Flux of carbon from fine root litter to soil pools"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("harvest_carbon_flux",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "Flux of carbon removed by harvest"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("NEE_alt",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "Alternative NEE"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("FRUNOFFC",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "C in runoff"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("FSOILAMDC",
      units = "kg C m-2 s-1", dim = dims,
      missval = -999, longname = "Flux of carbon input in soil amendments"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("TEMPR30",
      units = "degC", dim = dims,
      missval = -999, longname = "Smoothed air temperature"
    )
    nc_var[[length(nc_var) + 1]] <- ncdf4::ncvar_def("PRECIP30",
      units = "mm/day", dim = dims,
