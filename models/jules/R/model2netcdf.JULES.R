...
model2netcdf.JULES <- function(outdir) {
  ...
  for (fname in files) {
    ...
    base_dims <- nc$var[["GPP"]]$dim[[1]]$name # This isn't the best example, except that GPP is the default with read.output (and it is not reported by surface type)
    for(i in seq_along(vars)){
      var <- vars[i]
      nonstd_var <- nrow(PEcAn.utils::standard_vars[which(PEcAn.utils::standard_vars$Variable.Name == var),]) == 0  
      dims <- nc$var[[var]]$dim[[1]]$name
