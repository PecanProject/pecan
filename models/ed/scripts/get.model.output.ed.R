...
get.model.output.ed <- function() {
  ...
  if (settings$host$name == "localhost") {
    ...
    lapply(c(
      paste(settings$pecanDir, c(
        "R/utils.R", "R/model.specific.R",
        "rscripts/read.output.R"
      ), sep = ""),
      paste(settings$outdir, "samples.Rdata", sep = "")
    ), send.files)
    ...
    system(paste(
      "ssh -T", settings$host$name, "'",
      "cd", settings$host$outdir, "; R --vanilla < read.output.R'"
    ))

    rsync(
      from = paste(settings$host$name, ":", settings$host$outdir, "ensemble.Rdata", sep = ""),
      to = settings$outdir
    )
    rsync(
      from = paste(settings$host$name, ":", settings$host$outdir, "sensitivity.Rdata", sep = ""),
      to = settings$outdir
    )
  }
}
