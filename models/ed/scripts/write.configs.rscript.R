  ...
  if (host$name == "localhost") {
    ...
    if (length(files) > 0) {
      ...
      system(paste("ssh -T ", host$name,
        " 'for f in ", paste(todelete, collapse = " "), "; do rm $f; done'",
        sep = ""
      ))
    }
  }
...
if ("sensitivity.analysis" %in% names(settings)) {
  ...
  if (is.null(settings$sensitivity.analysis)) {
    ...
    sa.samples <- get.sa.sample.list(
      trait.samples,
      env.samples,
      quantiles
    )
    write.sa.configs(
      settings$pfts, sa.samples,
...
if ("ensemble" %in% names(settings) && settings$ensemble$size > 0) {
  ...
  write.ensemble.configs(
    settings$pfts, ensemble.samples,
    host, outdir, settings
  )
}
...
if (host$name == "localhost") {
  if (!host$outdir == outdir) {
    ...
    file.copy(
      from = paste(outdir, "samples.Rdata", sep = ""),
  ...
  system(paste(
    "rsync -routi ", paste(outdir, "samples.Rdata", sep = ""),
