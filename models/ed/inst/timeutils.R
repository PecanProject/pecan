...
#------------------------------------------------------------------------------------------#
alltimes <- function(
    datin, lon, lat, ed21 = TRUE, zeronight = FALSE, meanval = FALSE, imetavg = 1,
    nmean = 120, ...) {
  ...
  zenith <- ed.zen(
    when = datout$when, lon = lon, lat = lat, ed21 = ed21,
...
pretty.time <- function(when, n = 10, ...) {
  ...
  if (wstep.1st == 0) {
    ...
    wlaba <- chron(
      dates = paste(nummonths(whena), numdays(whena), numyears(whena), sep = "/"),
    ...
    yearz <- numyears(whenz) + as.integer(monz == 1)
    wlabz <- chron(
      dates = paste(monz, dayz, yearz, sep = "/"),
    ...
    vlabels <- paste(substring(100 + hours(vlevels), 2, 3),
      substring(100 + minutes(vlevels), 2, 3),
      substring(100 + seconds(vlevels), 2, 3),
      sep = ":"
    )
    padj <- rep(0, times = length(vlabels))
    #----- First time of the day. -------------------------------------------------------#
