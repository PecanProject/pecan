...
read_restart.ED2 <- function(outdir,
  ...
  histout <- read_S_files(
    sfile = basename(histfile),
  ...
  for (var_name in var.names) {
    ...
    if (var_name == "AGB") {
      forecast_tmp <- switch(perpft + 1,
        sum(histout$AGB, na.rm = TRUE),
        histout$AGB
      ) # kgC/m2
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2", "Mg/ha") # conv to MgC/ha
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "AGB",
        paste0("AGB.", pft_names)
      )
    }
    ...
    if (var_name == "AGB.pft") {
      ...
      names(forecast_tmp) <- switch(perpft + 1,
        "AGB",
        paste0(pft_names)
      )
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2", "Mg/ha") # conv to MgC/ha
    ...
    if (var_name == "TotLivBiom") {
      forecast_tmp <- switch(perpft + 1,
        sum(histout$TotLivBiom, na.rm = TRUE),
        histout$TotLivBiom
      ) # kgC/m2
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2", "Mg/ha") # conv to MgC/ha
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "TotLivBiom",
        paste0("TotLivBiom.", pft_names)
      )
    }

    if (var_name == "AbvGrndWood") {
      forecast_tmp <- switch(perpft + 1,
        sum(histout$AbvGrndWood, na.rm = TRUE),
        histout$AbvGrndWood
      ) # kgC/m2
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2", "Mg/ha") # conv to MgC/ha
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "AbvGrndWood",
        paste0("AbvGrndWood.", pft_names)
      )
    }

    if (var_name == "leaf_carbon_content") {
      forecast_tmp <- switch(perpft + 1,
        sum(histout$leaf_carbon_content, na.rm = TRUE),
        histout$leaf_carbon_content
      ) # kgC/m2
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2", "Mg/ha") # conv to MgC/ha
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "leaf_carbon_content",
        paste0("leaf_carbon_content.", pft_names)
      )
    }

    if (var_name == "storage_carbon_content") {
      forecast[[length(forecast) + 1]] <- switch(perpft + 1,
        sum(histout$storage_carbon_content, na.rm = TRUE),
        histout$storage_carbon_content
      ) # kgC/m2
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "storage_carbon_content",
        paste0("storage_carbon_content.", pft_names)
      )
    }
    ...
    if (var_name == "GWBI") {
      forecast_tmp <- switch(perpft + 1,
        sum(histout$GWBI, na.rm = TRUE),
        histout$GWBI
      ) # kgC/m2/yr
      forecast[[length(forecast) + 1]] <- PEcAn.utils::ud_convert(forecast_tmp, "kg/m^2/yr", "Mg/ha/yr") # conv to MgC/ha/yr
      names(forecast)[length(forecast)] <- switch(perpft + 1,
        "GWBI",
        paste0("GWBI.", pft_names)
      )
    }
