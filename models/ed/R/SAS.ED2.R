...
SAS.ED2 <- function(dir.analy, dir.histo, outdir, lat, lon, blckyr,
  ...
  colnames(pss.big) <- c(
    "time", "patch", "trk", "age", "area", "water", "fsc", "stsc", "stsl",
    "ssc", "psc", "msn", "fsn"
  )
  ...
  utils::write.table(css.big,
    file = file.path(outdir, paste0(file.prefix, ".css")), row.names = FALSE, append = FALSE,
    col.names = TRUE, quote = FALSE
  )

  utils::write.table(pss.big,
    file = file.path(outdir, paste0(file.prefix, ".pss")), row.names = FALSE, append = FALSE,
    col.names = TRUE, quote = FALSE
  )
  #---------------------------------------
}
