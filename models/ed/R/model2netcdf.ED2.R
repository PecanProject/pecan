...
model2netcdf.ED2 <- function(outdir, sitelat, sitelon, start_date,
  ...
  for (y in start_year:end_year) {
    ...
    for (i in seq_along(out_list)) {
      ...
      fcn <- match.fun(fcnx)
      put_out <- fcn(
        yr = y, nc_var = nc_var, out = out_list[[rflag]],
    ...
    for (i in seq_along(nc_var)) {
      ...
      cat(paste(nc_var[[i]]$name, nc_var[[i]]$longname),
        file = varfile,
        sep = "\n"
      )
    }
...
read_T_files <- function(yr, yfiles, tfiles, outdir, start_date, end_date, ...) {
  ...
  if (!is.null(ED2vc)) {
    ...
    out <- add(
      getHdf5Data(ncT, "FMEAN_GPP_PY") - getHdf5Data(ncT, "FMEAN_PLRESP_PY"),
      9, row, yr
    ) ## NPP
    out <- add(
      getHdf5Data(ncT, "FMEAN_RH_PY") + getHdf5Data(ncT, "FMEAN_PLRESP_PY"),
      10, row, yr
    ) ## TotalResp
    ...
    out <- add(
      getHdf5Data(ncT, "FMEAN_VAPOR_LC_PY") + getHdf5Data(ncT, "FMEAN_VAPOR_WC_PY") +
    ...
    out <- add(-getHdf5Data(ncT, "AVG_GPP") + getHdf5Data(ncT, "AVG_PLANT_RESP") + getHdf5Data(
      ncT,
      "AVG_HTROPH_RESP"
    ), 8, row, yr) ## NEE
    out <- add(
      getHdf5Data(ncT, "AVG_GPP") - getHdf5Data(ncT, "AVG_PLANT_RESP"), 9, row,
      yr
    ) ## NPP
    out <- add(
      getHdf5Data(ncT, "AVG_HTROPH_RESP") + getHdf5Data(ncT, "AVG_PLANT_RESP"),
      10, row, yr
    ) ## TotalResp
    ...
    ##           getHdf5Data(ncT, 'AVG_PAR_DIFFUSE'),22,row, yr) ## Swdown
    out <- add(
      getHdf5Data(ncT, "AVG_PAR_BEAM") + getHdf5Data(ncT, "AVG_PAR_DIFFUSE"),
      22, row, yr
    ) ## Swdown
    ...
    out <- add(
      getHdf5Data(ncT, "AVG_EVAP") + getHdf5Data(ncT, "AVG_TRANSP"), 44, row,
      yr
    ) ## Evap
...
put_T_values <- function(yr, nc_var, out, lat, lon, begins, ends, ...) {
  ...
  t <- ncdf4::ncdim_def(
    name = "time", units = paste0("days since ", yr, "-01-01 00:00:00"),
    vals = tvals,
  ...
  nc_var[[s + 1]] <- ncdf4::ncvar_def("AbvGrndWood",
  ...
  nc_var[[s + 2]] <- ncdf4::ncvar_def("AutoResp",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Autotrophic Respiration"
  )
  nc_var[[s + 3]] <- ncdf4::ncvar_def("CarbPools",
    units = "kg C m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Size of each carbon pool"
  )
  nc_var[[s + 4]] <- ncdf4::ncvar_def("CO2CAS",
    units = "ppmv", dim = list(lon, lat, t), missval = -999,
    longname = "CO2CAS"
  )
  nc_var[[s + 5]] <- ncdf4::ncvar_def("CropYield",
    units = "kg m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Crop Yield"
  )
  out <- conversion(6, yr2s) ## kg C m-2 yr-1 -> kg C m-2 s-1
  nc_var[[s + 6]] <- ncdf4::ncvar_def("GPP",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Gross Primary Productivity"
  )
  out <- conversion(7, yr2s) ## kg C m-2 yr-1 -> kg C m-2 s-1
  nc_var[[s + 7]] <- ncdf4::ncvar_def("HeteroResp",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Heterotrophic Respiration"
  )
  out <- conversion(8, yr2s) ## kg C m-2 yr-1 -> kg C m-2 s-1
  nc_var[[s + 8]] <- ncdf4::ncvar_def("NEE",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Net Ecosystem Exchange"
  )
  out <- conversion(9, yr2s) ## kg C m-2 yr-1 -> kg C m-2 s-1
  nc_var[[s + 9]] <- ncdf4::ncvar_def("NPP",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Net Primary Productivity"
  )
  out <- conversion(10, yr2s) ## kg C m-2 yr-1 -> kg C m-2 s-1
  nc_var[[s + 10]] <- ncdf4::ncvar_def("TotalResp",
    units = "kg C m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Total Respiration"
  )
  nc_var[[s + 11]] <- ncdf4::ncvar_def("TotLivBiom",
    units = "kg C m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Total living biomass"
  )
  nc_var[[s + 12]] <- ncdf4::ncvar_def("TotSoilCarb",
    units = "kg C m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Total Soil Carbon"
  )
  nc_var[[s + 13]] <- ncdf4::ncvar_def("Fdepth",
    units = "m", dim = list(lon, lat, t), missval = -999,
    longname = "Frozen Thickness"
  )
  nc_var[[s + 14]] <- ncdf4::ncvar_def("SnowDepth",
    units = "m", dim = list(lon, lat, t), missval = -999,
    longname = "Total snow depth"
  )
  nc_var[[s + 15]] <- PEcAn.utils::mstmipvar("SnowFrac", lat, lon, t, zg) # not standard
  nc_var[[s + 16]] <- ncdf4::ncvar_def("Tdepth",
    units = "m", dim = list(lon, lat, t), missval = -999,
    longname = "Active Layer Thickness"
  )
  nc_var[[s + 17]] <- ncdf4::ncvar_def("CO2air",
    units = "umol mol-1", dim = list(lon, lat, t), missval = -999,
    longname = "Near surface CO2 concentration"
  )
  nc_var[[s + 18]] <- ncdf4::ncvar_def("LWdown",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Surface incident longwave radiation"
  )
  nc_var[[s + 19]] <- ncdf4::ncvar_def("Psurf",
    units = "Pa", dim = list(lon, lat, t), missval = -999,
    longname = "Surface pressure"
  )
  nc_var[[s + 20]] <- ncdf4::ncvar_def("Qair",
    units = "kg kg-1", dim = list(lon, lat, t), missval = -999,
    longname = "Near surface specific humidity"
  )
  nc_var[[s + 21]] <- ncdf4::ncvar_def("Rainf",
    units = "kg m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Rainfall rate"
  )
  nc_var[[s + 22]] <- ncdf4::ncvar_def("SWdown",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Surface incident shortwave radiation"
  )
  out <- checkTemp(23)
  nc_var[[s + 23]] <- ncdf4::ncvar_def("Tair",
    units = "K", dim = list(lon, lat, t), missval = -999,
    longname = "Near surface air temperature"
  )
  nc_var[[s + 24]] <- ncdf4::ncvar_def("Wind",
    units = "m s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Near surface module of the wind"
  )
  nc_var[[s + 25]] <- ncdf4::ncvar_def("LWnet",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Net Longwave Radiation"
  )
  nc_var[[s + 26]] <- ncdf4::ncvar_def("Qg",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Ground heat"
  )
  nc_var[[s + 27]] <- ncdf4::ncvar_def("Qh",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Sensible heat"
  )
  out <- conversion(28, PEcAn.data.atmosphere::get.lv()) ## kg m-2 s-1 -> W m-2
  nc_var[[s + 28]] <- ncdf4::ncvar_def("Qle",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Latent heat"
  )
  nc_var[[s + 29]] <- ncdf4::ncvar_def("SWnet",
    units = "W m-2", dim = list(lon, lat, t), missval = -999,
    longname = "Net shortwave radiation"
  )
  nc_var[[s + 30]] <- PEcAn.utils::mstmipvar("RootMoist", lat, lon, t, zg) # not standard
  nc_var[[s + 31]] <- ncdf4::ncvar_def("TVeg",
    units = "kg m-2 s-1", dim = list(lon, lat, t), missval = -999,
    longname = "Transpiration"
  )
  nc_var[[s + 32]] <- PEcAn.utils::mstmipvar("WaterTableD", lat, lon, t, zg) # not standard

  nc_var[[s + 33]] <- ncdf4::ncvar_def("fPAR",
    units = "", dim = list(lon, lat, t), missval = -999,
    longname = "Absorbed fraction incoming PAR"
  )
  nc_var[[s + 34]] <- ncdf4::ncvar_def("LAI",
    units = "m2 m-2", dim = list(lon, lat, t), missval = -999,
  ...
  nc_var[[s + 37]] <- ncdf4::ncvar_def("SoilMoist",
    units = "kg m-2", dim = list(lon, lat, zg, t), missval = -999,
    longname = "Average Layer Soil Moisture"
  )
  out <- checkTemp(38)
  nc_var[[s + 38]] <- ncdf4::ncvar_def("SoilTemp",
    units = "K", dim = list(lon, lat, zg, t), missval = -999,
    longname = "Average Layer Soil Temperature"
  )
  nc_var[[s + 39]] <- ncdf4::ncvar_def("SoilWet",
    units = "", dim = list(lon, lat, t), missval = -999,
  ...
  nc_var[[s + (out_length + 1)]] <- ncdf4::ncvar_def(
...
put_E_values <- function(yr, nc_var, out, lat, lon, begins, ends, pft_names, ...) {
  ...
  t <- ncdf4::ncdim_def(
    name = "dtime", units = paste0("days since ", yr, "-01-01 00:00:00"),
  ...
  p <- ncdf4::ncdim_def(
    name = "pft", units = "unitless", vals = pfts,
  ...
  nc_var[[s + 1]] <- ncdf4::ncvar_def("DBH",
    units = "cm", dim = list(lon, lat, t, p), missval = -999,
    longname = "Diameter at breast height"
  )
  nc_var[[s + 2]] <- ncdf4::ncvar_def("DDBH_DT",
    units = "cm yr-1", dim = list(lon, lat, t, p), missval = -999,
    longname = "Rate of change in dbh"
  )
  nc_var[[s + 3]] <- ncdf4::ncvar_def("NPLANT",
    units = "plant m-2", dim = list(lon, lat, t, p), missval = -999,
  ...
  nc_var[[s + 4]] <- ncdf4::ncvar_def("PFT",
    units = "", dim = list(p),
    longname = paste(pft_names, collapse = ",")
  )
  out_length <- length(out)
  out[[out_length + 1]] <- c(rbind(bounds[, 1], bounds[, 2]))
  nc_var[[s + 5]] <- ncdf4::ncvar_def(
    name = "dtime_bounds", units = "",
...
read_S_files <- function(sfile, outdir, pft_names, pecan_names = NULL) {
  ...
  # List of vars to extract includes the requested one, plus others needed below
  add_vars <- c(
    add_vars, "PFT", "AREA", "PACO_N", "NPLANT", "DAGB_DT", "BDEAD", "DBH",
