...
##' @param out_vars_to_pecanify a space separated string of variables to process or NULL.
...
##' @author Tobey Carman
##'
setup.outputs.dvmdostem <- function(out_vars_to_pecanify, dvmdostem_output_spec, run_directory, run_id, appbinary_path) {

  is.not.null <- function(x) !is.null(x) # helper for readability

  # 0) neither path or variables specified
  if (is.null(out_vars_to_pecanify) && is.null(dvmdostem_output_spec)) {
  ...
  if (is.not.null(out_vars_to_pecanify) && is.null(dvmdostem_output_spec)) {
    outspec_path <- file.path(system.file(package = "PEcAn.dvmdostem"), "output_spec.csv")
    outvars2pecanify <- out_vars_to_pecanify
  }
  ...
  if (is.not.null(out_vars_to_pecanify) && is.not.null(dvmdostem_output_spec)) {
    outvars2pecanify <- out_vars_to_pecanify
    outspec_path <- dvmdostem_output_spec
  }
