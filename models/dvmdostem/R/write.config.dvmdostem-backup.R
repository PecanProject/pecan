...
## Junk
#settings <- '/data/sserbin/Modeling/dvmdostem/pecan_runs/pecan.xml'
#settings <- PEcAn.settings::read.settings(settings)
#trait.values <- '/data/sserbin/Modeling/dvmdostem/pecan_runs/run.5/samples.Rdata'
#write.config.dvmdostem(trait.values = trait.values, settings=settings,run.id=1)
##
...
write.config.dvmdostem <- function(defaults = NULL, trait.values, settings, run.id) {
   ...
   logger.debug("trait.values")
   logger.debug(trait.values)
   logger.debug("trait.values[[settings$pfts$pft$name]]")
   logger.debug(trait.values[[settings$pfts$pft$name]])
