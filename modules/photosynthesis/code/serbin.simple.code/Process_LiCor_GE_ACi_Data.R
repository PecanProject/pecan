...
EGstar <- 37830
mm.constants <- list(
  R = R, Oxygen = Oxygen, Kc25 = Kc25, Ekc = Ekc, Ko25 = Ko25, Eko = Eko, Gstar25 = Gstar25,
  EGstar = EGstar
)
...
ge.data.qc <- data.qc(
  data = ge.data, out.dir = out.dir, Cond.cutoff = Cond.cutoff, Ci.cutoff = Ci.cutoff,
...
system.time(for (i in 1:dim(samples)[1]) {
  ...
  if (dim(sub.data)[1] < 3 | chk2 < 3 & chk3 < 3) {
    ...
    if (chk2 >= 3 & chk3 >= 3) {
      f.model <- 1
      fit <- DEoptim(ACi.full, lower = lower.bound, upper = upper.bound, DEoptim.control(
        NP = NP,
      ...
      f.model <- 2
      fit <- DEoptim(ACi.rubisco, lower = lower.bound[1:2], upper = upper.bound[1:2], DEoptim.control(
        NP = NP,
        F = DWF, CR = CR, itermax = max.iters, VTR = RMSE.min, strategy = 2, trace = FALSE,
        storepopfrom = 1, storepopfreq = 5
      ))
      ...
      f.model <- 3
      fit <- DEoptim(ACi.rubp, lower = lower.bound[3:2], upper = upper.bound[3:2], DEoptim.control(
        NP = NP,
        F = DWF, CR = CR, itermax = max.iters, VTR = RMSE.min, strategy = 2, trace = FALSE,
        storepopfrom = 1, storepopfreq = 5
      ))
...
#--------------------------------------------------------------------------------------------------#
### Generate output
output.data <- data.frame(samples, means,
  Vcmax = Vcmax, Jmax = Jmax, Rd = Rd, RMSE.DEoptim = RMSE.DEoptim,
