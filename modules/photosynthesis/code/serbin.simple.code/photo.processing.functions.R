...
plot.ge.fit <- function(type = c("A-Ci", "A-Q"), data, DEoptim.output, params, outdir, file, f.model) {
  ...
  if (type == "A-Ci") {
    ...
    plot(data[, loc1[1]], data[, loc1[2]],
      pch = 21, bg = "grey70", cex = 3, cex.axis = cexaxis, xlim = c(0, range(data[, loc1[1]])[2]),
    ...
    plot(plot.x, data[, loc1[2]],
      pch = 21, bg = "grey70", cex = 3, cex.axis = cexaxis, xlim = c(0, range(plot.x)[2]),
    ...
    plot(data[, loc1[1]], data[, loc1[2]],
      main = paste(file), xlab = "Ci", ylab = "Photo", cex.lab = 2, cex = 1.8,
      xlim = c(1.1, 1500), ylim = c(0, ylim[2] + 3)
    )
    legend("bottomright",
      legend = c(
        paste("Tleaf =", round(Tleaf, 2)), paste("Vcmax =", round(Vcmax.plot, 2)),
    ...
    legend("topleft",
      legend = c("Rubisco", "RuBP", "Photo"), lty = c(2, 2, 1),
    ...
    if (Vcmax.plot != -9999 & Jmax.plot != -9999) {
      lines(plotCi, pmin(
        Vcmax.plot * (plotCi - Gstar) / (plotCi + (Kc * (1 + Oxygen / Ko))),
        (Jmax.plot * (plotCi - Gstar)) / ((4.5 * plotCi) + (10.5 * Gstar))
      ) - Rd.plot, col = "dark grey", lwd = 2.0)
    }
    box(lwd = 2.2)
    dev.off()
  } else if (type == "A-Q") {
