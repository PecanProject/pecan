...
######### Table 1
aspen.alder.willow <- list(AAW = data.frame(
  spcd = c(350, 351, 353, 740, 741, 742, 743, 746),
...
maple.birch <- list(MB = data.frame(
  spcd = c(312, 315, 316, 319, 371, 372, 375, 379),
...
mixed.hardwood <- list(MH = data.frame(
  spcd = c(
    332, 431, 491, 541, 543, 544, 611, 621, 691,
    693, 711, 731, 761, 762, 763, 931, 951, 952,
    972, 970
  ),
  acronym = c(
    "AEOC2", "CACH6", "COFL2", "FRAM2",
...
maple.oak.hickory.beech <- list(MOHB = data.frame(
  spcd = c(
    318, 400, 531, 802, 806, 809,
...
cedar.larch <- list(CL = data.frame(
  spcd = c(81, 42, 68, 71, 73, 70, 212, 241, 242),
  acronym = c(
    "CADE27", "CANO9", "JUVI", "LALA", "LAOC",
...
fir.hemlock <- list(FH = data.frame(
  spcd = c(202, 11, 12, 15, 17, 19, 20, 22, 10, 261, 263, 264),
  acronym = c(
    "PSME", "ABAM", "ABBA", "ABCO", "ABGR",
...
pine <- list(P = data.frame(
  spcd = c(
    101, 105, 108, 136, 106, 116, 117, 133, 119, 122, 125, 126,
    129, 131
  ),
  acronym = c(
    "PIAL", "PIBA2", "PICO", "PIDI3", "PIED", "PIJE",
...
spruce <- list(S = data.frame(
  spcd = c(91, 93, 94, 95, 97, 98, 90),
...
woodland <- list(W = data.frame(
  spcd = c(475, 69, 65, 986, 814, 843),
