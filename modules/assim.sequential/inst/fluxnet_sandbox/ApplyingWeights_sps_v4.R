...
sda_plots_dir <- file.path(paste0(
  "~/Data/Dropbox/MANUSCRIPTS/BNL_TEST/Dokoohaki_etal_NASA_CMS_SDA_part1/Figures/",
  Site_name
))
...
fluxnet_local_file <- osf_retrieve_file(fluxnet_site_data$meta[[1]]$links$download) %>%
  osf_download(
    conflicts = "skip", path = file.path(fluxnet_data_dir),
    verbose = TRUE, progress = TRUE
  )
fluxnet_data_dir_uncomp <- file.path(
  fluxnet_data_dir,
...
# weekly data
fluxnet_ww_data <- read.csv(file = file.path(
  fluxnet_data_dir_uncomp,
  local_flux_files[grep("WW", local_flux_files)]
), header = T)
...
png(
  filename = file.path(fluxnet_data_dir, paste0("FLX_", Site_name, "_FLUXNET2015_SUBSET_WW_1999-2014_1-4.png")),
...
# monthly data
fluxnet_mm_data <- read.csv(file = file.path(
  fluxnet_data_dir_uncomp,
  local_flux_files[grep("_MM_", local_flux_files)]
), header = T)
...
png(
  filename = file.path(fluxnet_data_dir, paste0("FLX_", Site_name, "_FLUXNET2015_SUBSET_MM_1999-2014_1-4.png")),
  width = 2800, height = 1200, res = 200
)
par(mfrow = c(1, 1), mar = c(4.5, 4.7, 0.3, 0.4), oma = c(0.3, 0.9, 0.3, 0.1)) # B, L, T, R
...
ncdf_sda_local_file <- osf_retrieve_file(ncdf_file$meta[[1]]$links$download) %>%
  osf_download(
    conflicts = "skip", path = file.path(sda_results_dir),
    verbose = TRUE, progress = TRUE
  )
...
png(
  filename = file.path(sda_plots_dir, paste0(Site_name, "_raw_SDA_", Var, "_kgC_m2_s1.png")),
  width = 2800, height = 1200, res = 200
)
par(mfrow = c(1, 1), mar = c(4.5, 4.7, 0.3, 0.4), oma = c(0.3, 0.9, 0.3, 0.1)) # B, L, T, R
...
flux.data.df %>%
  ...
  map(~ {
    ...
    ggsave(
      file = file.path(sda_plots_dir, paste0("timeseries_", .x$Site %>% unique(), ".png")),
      width = 14, height = 7
    )
    # browser()
  })
...
for (i in seq_along(year_select)) {
  ...
  p <- ggplot() +
    geom_ribbon(
      data = flux.data.df_sub_yr, aes(x = Week, y = Forecast, ymin = FLL, ymax = FUL),
      alpha = 0.3, colour = "black", fill = "#1B9E77"
    ) +
    theme_bw(base_size = 15) +
    geom_point(
      data = flux.data.df_sub_yr, aes(x = Week, y = Forecast), colour = "#1B9E77",
      alpha = 0.85, size = 5, shape = 18
    ) +
    geom_line(
      data = flux.data.df_sub_yr, aes(x = Week, y = Forecast),
      colour = "#1B9E77"
    ) +
    geom_ribbon(
      data = flux.data.df_sub_yr, aes(x = Week, y = Analysis, ymin = ALL, ymax = AUL),
      alpha = 0.3, colour = "black", fill = "#D95F02"
    ) +
    theme_bw(base_size = 15) +
    geom_point(
      data = flux.data.df_sub_yr, aes(x = Week, y = Analysis), colour = "#D95F02",
      alpha = 0.75, size = 5
    ) +
    geom_line(
      data = flux.data.df_sub_yr, aes(x = Week, y = Analysis),
      colour = "#D95F02"
    ) +
    geom_errorbar(
      data = fluxnet_ww_data_sub_yr,
      aes(x = Week, ymin = get(ymin_var), ymax = get(ymax_var))
    ) +
    geom_line(data = fluxnet_ww_data_sub_yr, aes(x = Week, y = get(varef))) +
    ylab(bquote(.(Var) ~ (gC ~ m^{
      -2
    } ~ d^{
      -1
    }))) +
  ...
  ggsave(p,
    file = file.path(sda_plots_dir, paste0(Site_name, "_SDA_", year_select[i], "_", Var, "_weekly_comparison_v2.png")),
    width = 14, height = 7
  )
