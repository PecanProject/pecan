...
# get MODIS data
bety <- list(
  user = settings$database$bety$user, password = settings$database$bety$password,
...
if (extract_LAI) {
  ...
  modis_data <- foreach(i = 1:nrow(site_coords)) %dopar% PEcAn.data.remote::call_MODIS(
    product = "MOD15A2H",
...
date.obs <- strsplit(names(point_list$median_lai), "_")[3:length(point_list$median_lai)] %>%
  map_chr(~ .x[2]) %>%
  paste0(., "/07/15")
