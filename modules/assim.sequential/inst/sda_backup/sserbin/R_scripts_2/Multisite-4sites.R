...
library(dplyr)
#------------------------------------------ Setup -------------------------------------
setwd("/data/sserbin/Modeling/sipnet/NASA_CMS")
...
site2 <- point_list
site2$median_AGB[[1]] %>%
  filter(Site_ID == "796") -> site2$median_AGB[[1]]


site2$stdv_AGB[[1]] %>%
  filter(Site_ID == "796") -> site2$stdv_AGB[[1]]
...
point_list$median_AGB <- rbind(
  site1$median_AGB[[1]],
  site2$median_AGB[[1]]
) %>% filter(Site_ID %in% site.ids)
point_list$stdv_AGB <- rbind(
  site1$stdv_AGB[[1]],
  site2$stdv_AGB[[1]]
) %>% filter(Site_ID %in% site.ids)
