...
setwd("/data/bmorrison/sda/ecoregion_site_analysis/modis_data")
...
make_grid <- function(x, cell_diameter, cell_area, clip = FALSE) {
  ...
  g <- spsample(ext,
    type = "hexagonal", cellsize = cell_diameter,
    offset = c(0.5, 0.5)
  )
...
save(hex_grid, file = paste("/data/bmorrison/sda/ecoregion_site_analysis/hex_grid_", length(names(hex_grid)), ".Rdata", sep = ""))
load(paste("/data/bmorrison/sda/ecoregion_site_analysis/hex_grid_1164.Rdata", sep = ""))
...
plot(region, col = "grey50", bg = "light blue", axes = TRUE)
plot(hex_grid, border = "orange", add = T)
csamples <- spTransform(samples, CRS = crs(eco))
region <- spTransform(region, CRS = crs(eco))


xy <- as.data.frame(samples)
names(xy) <- c("lon", "lat")
save(xy, file = paste("/data/bmorrison/sda/ecoregion_site_analysis/random_sites_", nrow(xy), ".Rdata", sep = ""))
# extract MODIS data for location

load("/data/bmorrison/sda/ecoregion_site_analysis/random_sites_989.Rdata")
...
#     working = print(paste("working on : ", ((j*10)-9+600), "-", nrow(xy)+600, sep = ""))
#
#   } else {
#     coords = xy[((j*10)-9):(j*10),]
#     working = print(paste("working on : ", ((j*10)-9+600), "-", (j*10+600), sep = ""))
#
#   }
...
files <- list.files(path = "/data/bmorrison/sda/ecoregion_site_analysis/modis_data", pattern = ".csv", include.dirs = T, full.names = T)
xy <- data.frame()
...
sites <- SpatialPointsDataFrame(data = xy, coords = xy, proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs "))
...
for (i in 1:nrow(sites))
{
  working <- print(i)
  site <- sites[i, ]
  t <- compute_annual_lai(data = data, sites = site)
  test <- rbind(test, t)
}
...
v.vgm <- vgm(psill = NA, range = NA, model = "Sph", nugget = 0.9)
