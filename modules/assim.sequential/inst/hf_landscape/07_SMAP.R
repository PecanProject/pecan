...
extract_SMAP_h5 <- function(h5_in, name, x, y) {
  ...
  valid <- apply(Longlat_matrix, 1, function(x) {
    all(!is.na(x))
  })
  smp <- rep(NA, length(x))
  for (i in seq_along(x)) {
    ...
    Distance <- sp::spDistsN1(Longlat_matrix[valid, ],
      as.matrix(data.frame(lon = as.numeric(x[i]), lat = as.numeric(y[i]))),
      longlat = TRUE
    )[-1]
...
if (file.exists(smap_file)) {
  ...
  SMAP_CSV <- matrix(NA, 0, 6) %>%
    `colnames<-`(c("date", "site_id", "lat", "lon", "smp", "sd")) %>%
    as.data.frame()
}
...
for (t in seq_along(status$date)) {
  ...
  sm_raster <- extract_SMAP(
    file = file.path(smap_download_dir, status$file[t]),
