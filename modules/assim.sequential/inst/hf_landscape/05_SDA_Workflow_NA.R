...
## parse start date
option_list <- list(
  optparse::make_option("--start.date",
    default = Sys.Date(),
    type = "character"
  ),
...
site.ids <- papply(set, function(x) (x$run$site$id)) %>%
  unlist() %>%
  as.character()
nsite <- length(site.ids)

NAdata <- data.frame(
  date = c(rep(start.date, nsite), rep(sda.start, nsite)),
...
run_id <- as.data.frame(run_id) %>%
  mutate(folder = prev_run_ids, id = paste0("id", .data$ens)) %>%
  group_by(site)
