...
#setup working space
outdir <- "/projectnb/dietzelab/dietze/hf_landscape_SDA/test01"
...
#SMP
SMP_outdir <- file.path(outdir,"obs")
SMP_search_window <- 30
SMP_timestep <- list(unit="day", num=3)
SMP_export_csv <- TRUE
update_csv <- FALSE
...
for(i in seq_along(site_ID)){
  ...
  veg_config[[i]] = veg2model.SIPNET(outfolder, poolinfo, site_ID[i], 1)
