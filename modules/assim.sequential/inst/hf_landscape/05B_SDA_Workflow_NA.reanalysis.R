...
for (s in seq_along(runDays)) {
  ...
  if (dir.exists(prev)) {
    ...
    if (length(prev.out) > 0) {
      prev.files <- sapply(as.list(prev.out), function(x) {
        length(dir(x, pattern = "*.nc"))
      })
      if (min(prev.files) > 0) {
        #########   RUN FORECAST   ########
...
for (s in seq_along(runDays)) {
  ...
  runs <- sapply(strsplit(this.out, "/"), function(x) {
    x[grep("ENS", x)]
  })
  site_ids <- unique(sapply(strsplit(runs, "-"), function(x) {
    as.numeric(x[3])
  }))
  ens_ids <- unique(sapply(strsplit(runs, "-"), function(x) {
    as.numeric(x[2])
  }))
  ...
  for (i in seq_along(runs)) {
    out[[runs[i]]] <- PEcAn2EFI.ens(
      outdir = file.path(now, "out"),
      run.id = runs[i],
  ...
  out <- out %>%
    relocate(parameter) %>%
    relocate(site_id) %>%
    relocate(time_bounds) %>%
    rename(datetime = time_bounds) %>%
  ...
  out %>%
    dplyr::group_by(reference_datetime) %>%
    arrow::write_dataset(minio_uri(), format = "parquet")
}
