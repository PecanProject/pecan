...
ntrees.save <- agb.pft.save <- array(NA, dim = c(
  9, length(run.id),
  length(1:1950)
))
...
for (i in c(9, 8, 4)) {
  plot(950:1949, quant.keep[[i]][2, ],
    typ = "l", col = rainbow_hcl(9)[i],
  ...
  plot(950:1949, quant.keep.a[[i]][2, ],
    typ = "l", col = rainbow_hcl(9)[i],
...
rownames(pf) <- rownames(q.bar) <- colnames(pf) <- colnames(q.bar) <- c(
  "Maple", "Birch", "Hickory",
...
par(mfrow = c(1, 1))
corrplot(cov2cor(pf),
  type = "upper", tl.srt = 25,
  tl.cex = .8, col = c("#ca0020", "#0571b0"), diag = FALSE,
  order = "original"
)
corrplot(cov2cor(pf + q.bar),
  type = "upper", tl.srt = 25,
  tl.cex = .8, col = c("#ca0020", "#0571b0"), diag = FALSE
)
...
par(mfrow = c(1, 1))
mattext(
  data = wt.props, data_names = as.character(1:nens), colors = rainbow(nens),
...
par(mfrow = c(1, 1))
mattext(param.hist[1, , ],
  data_names = as.character(1:nens), colors = rainbow(nens),
