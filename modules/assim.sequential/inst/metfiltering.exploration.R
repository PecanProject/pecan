...
theme_set(theme_classic() + theme(
  axis.text.x = element_text(angle = 90, hjust = 1, size = fontsize$axis),
...
ggplot(pre.dfm, aes(x = value, y = year)) +
  geom_density_ridges(fill = "dodgerblue1") +
...
ggplot(pre.dfm, aes(x = value, y = year, fill = variable)) +
  ...
  theme(legend.position = "none") +
  coord_flip() +
  stat_density_ridges(bandwidth = 0.0735) +
