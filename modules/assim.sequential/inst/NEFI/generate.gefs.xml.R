...
# Determine the workflow ID by looking in the database
ids <- PEcAn.DB::db.query("SELECT id FROM workflows", con)
ids <- ids[ids<2000000000] # There are several placeholder ids that are much too large in the database, so this screens
                           # those out.  Kind of hack-y, but it works.
workflowid <- as.character(max(ids) + 1)
...
# Register new workflow id with the database
hostname <- PEcAn.remote::fqdn()
query <- paste0("INSERT INTO workflows (site_id, model_id, notes, folder, hostname, start_date,", 
                "end_date, params, advanced_edit, started_at, created_at) ", 
                "values (", settings$run$site$id, ", ", settings$model$id, ", ", "''", ", '', '", hostname, "', '",
                format(start_date, "%Y/%m/%d %H:%M:%S"), "', '", format(end_date, "%Y/%m/%d %H:%M:%S"), "', ", "''", ", ", "true", ", NOW(), NOW())")
PEcAn.DB::db.query(query, con = con)
