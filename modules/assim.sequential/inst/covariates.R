...
srad <- terra::rast(list.files(
  path = "/projectnb/dietzelab/jploshay/pecan_copy/jploshay/10m_srad",
  pattern = ".tif$",
  all.files = T,
  full.names = T
))
...
## Vapor Pressure (kPa)
vapr <- terra::rast(list.files(
  path = "/projectnb/dietzelab/jploshay/pecan_copy/jploshay/10m_vapr",
  pattern = ".tif$",
  all.files = T,
  full.names = T
))
...
tavg <- terra::rast(list.files(
  path = "/projectnb/dietzelab/jploshay/pecan_copy/jploshay/avg_temp_prep/WorldClim",
  pattern = ".tif$",
  all.files = T,
  full.names = T
))
...
prec <- terra::rast(list.files(
  path = "/projectnb/dietzelab/jploshay/pecan_copy/jploshay/total_prec",
  pattern = ".tif$",
  all.files = T,
  full.names = T
))
...
GLanCE_extract <- function(pattern, path) {
  files <- list.files(
    path = "/projectnb/dietzelab/dietze/glance2012/e4ftl01.cr.usgs.gov/MEASURES/GLanCE30.001/2012.07.01", # make this path default
    all.files = T,
    full.names = T,
    pattern
  )
