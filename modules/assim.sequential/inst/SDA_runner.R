...
for (i in 1:4) {
  ...
  if (is.character(try(PEcAnAssimSequential::sda.enkf.multisite(
    settings = settings,
    obs.mean = obs.mean,
    obs.cov = obs.cov,
    Q = NULL,
  ...
  file.rename(
    "/projectnb/dietzelab/dongchen/All_NEON_SDA/NEON42/SDA",
    file.path(
      "/projectnb/dietzelab/dongchen/All_NEON_SDA/NEON42",
