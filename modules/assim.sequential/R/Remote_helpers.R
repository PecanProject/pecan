...
SDA_remote_launcher <- function(settingPath,
  ...
  out <- remote.execute.R(
    script = paste0("dir.create(\"/", settings$host$folder, "//", folder_name, "\")"),
    host = my_host,
    user = my_host$user,
    scratchdir = "."
  )
  ...
  if (is.MultiSettings(settings)) {
    input.paths <- settings %>%
      map(~ .x[["run"]]) %>%
      map(~ .x[["inputs"]] %>% map(~ .x[["path"]])) %>%
      unlist()
  } else {
    input.paths <- settings$run$inputs %>%
      map(~ .x[["path"]]) %>%
      unlist()
  }
  ...
  if (!any(missing.inputs)) {
    # creating a folder on remote
    out <- remote.execute.R(
      script = paste0("dir.create(\"/", settings$host$folder, "//", folder_name, "//inputs", "\")"),
      host = my_host,
      user = my_host$user,
      scratchdir = "."
    )
      ...
      tryCatch(
        {
          ...
          # replace the path
          settings <<- rapply(settings, function(x) {
            ifelse(x == missing.input,
  ...
  out <- remote.execute.R(
    script = paste0("dir.create(\"/", settings$host$folder, "//", folder_name, "//scratch", "\")"),
    host = my_host,
    user = my_host$user,
    scratchdir = "."
  )
