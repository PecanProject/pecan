...
sda.enkf.multisite <- function(settings, obs.mean, obs.cov, Q = NULL, restart=F, 
  ...
  for(t in seq_len(nt)){
    ...
    ### save outputs                                                      ###
    ###-------------------------------------------------------------------###---- 
    save(settings, t, FORECAST, ANALYSIS, enkf.params, new.state, new.params, out.configs, ensemble.samples, inputs, file = file.path(settings$outdir,"SDA", "sda.output.Rdata"))
