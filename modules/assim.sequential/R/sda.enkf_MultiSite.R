...
sda.enkf.multisite <- function(settings, obs.mean, obs.cov, Q = NULL, restart=F, 
  ...
  for(t in seq_len(nt)){
    ...
    if (t%%2==0 | t==nt)  post.analysis.multisite.ggplot(site.locs, t, obs.times, obs.mean, obs.cov, obs, X, FORECAST, ANALYSIS ,plot.title=control$plot.title, facetg=control$facet.plots, readsFF=readsFF)
  } ### end loop over time
