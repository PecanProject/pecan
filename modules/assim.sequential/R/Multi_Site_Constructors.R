...
Construct.H.multisite <- function(site.ids, var.names, obs.t.mean) {
  ...
  nobs <- obs.t.mean %>%
    map_dbl(~ length(.x)) %>%
    max() # this gives me the max number of obs at sites
  nobstotal <- obs.t.mean %>%
    purrr::flatten() %>%
    length() # this gives me the total number of obs
  ...
  {
    site <- site.ids[i]
    choose <- sapply(names(obs.t.mean[[site]]), agrep,
      x = var.names,
      max = 1, USE.NAMES = FALSE
    ) %>% unlist()
