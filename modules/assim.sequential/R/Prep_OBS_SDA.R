...
Prep_OBS_SDA <- function(settings, out_dir, AGB_dir, Search_Window = 30) {
  ...
  for (i in 1:round.length) {
    ...
    temp_site_info <- furrr::future_pmap(list(site_info, start, end), function(X, start, end) {
      X[start:end]
    })

    # download LAI data and LAI std
    lai_data <- rbind(PEcAn.data.remote::call_MODIS(
      outdir = NULL, var = "LAI", site_info = site_info, product_dates = c(start_YEARDOY, end_YEARDOY),
      run_parallel = TRUE, ncores = ncores, product = "MOD15A2H", band = "Lai_500m",
      package_method = "MODISTools", QC_filter = TRUE, progress = FALSE
    ), lai_data)

    lai_sd <- rbind(PEcAn.data.remote::call_MODIS(
      outdir = NULL, var = "LAI", site_info = site_info, product_dates = c(start_YEARDOY, end_YEARDOY),
