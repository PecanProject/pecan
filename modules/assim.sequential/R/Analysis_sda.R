...
GEF <- function(settings, Forecast, Observed, H, extraArg, nitr = 50000, nburnin = 10000, ...) {
  ...
  for (i in 1:length(input.vars)) {
    interval[grep(
      x = rownames(interval),
      pattern = input.vars[i]
    ), ] <- matrix(c(
      as.numeric(settings$state.data.assimilation$inputs[[i]]$min_value), # needs to be inputs because sometimes the observation is on a different scale than the state variable
      as.numeric(settings$state.data.assimilation$inputs[[i]]$max_value)
    ),
    length(grep(x = rownames(interval), pattern = input.vars[i])), 2,
    byrow = TRUE
    )
  }
  ...
  if (t == 1 | recompile) {
    ...
    inits.pred <<- list(
      q = diag(length(mu.f)) * (length(mu.f) + 1),
