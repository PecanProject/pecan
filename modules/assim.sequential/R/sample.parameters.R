...
sample.parameters <- function(ne, settings, con) {
  ...
  if (is.null(settings$assim.sequential$prior)) {
    pft.id <- PEcAn.DB::db.query(
      paste0("SELECT id from pfts where name = '", settings$pfts$pft$name, "'"),
      con
    )
  ...
  prior.db <- PEcAn.DB::db.query(paste0(
    "SELECT * from dbfiles where container_type = 'Posterior' and container_id = ",
    settings$assim.sequential$prior
  ), con)
  ...
  for (i in seq_len(nvar)) {
    if (post.distns$distn[i] == "exp") {
      ...
      prior[, i] <- eval(parse(text = paste0(
        "r", post.distns$distn[i],
