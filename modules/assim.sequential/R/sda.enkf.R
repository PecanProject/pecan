...
sda.enkf.original <- function(settings, obs.mean, obs.cov, IC = NULL, Q = NULL, adjustment = TRUE, restart = NULL) {
  ...
  if (!is.null(con)) {
    ...
    ensemble.id <- db.query(
      paste("SELECT id FROM ensembles WHERE created_at='", now, "'", sep = ""),
      con
    )[["id"]]
  } else {
    ensemble.id <- -1
  ...
  for (i in seq_len(nens)) {
    ...
    if (!is.null(con)) {
      ...
      run.id[[i]] <- db.query(paste0(
        "INSERT INTO runs (model_id, site_id, start_time, finish_time, outdir, created_at, ensemble_id,",
        ...
        now, "', ", ensemble.id, ", '", paramlist, "') RETURNING id"
      ), con)
    } else {
  ...
  for (t in seq_len(nt)) { #
      ...
      if (FALSE) { # interactive() & t > 1
        ...
        for (i in sample(x = 1:ncol(X), size = 2)) {
          ...
          if (i <= ncol(Ybar)) {
            ...
            lines(as.Date(obs.times[t1:t]), as.numeric(Ybar[, i]),
              type = "l",
              col = "darkgreen", lwd = 2
            )
          }
    ...
    if (interactive() & t > 1) { #
      ...
      for (i in 1:ncol(FORECAST[[t]])) { #
        ...
        Xci <- plyr::laply(FORECAST[t1:t], function(x) {
          quantile(x[, i], c(0.025, 0.975), na.rm = TRUE)
        })
