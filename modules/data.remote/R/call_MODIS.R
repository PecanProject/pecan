...
#################### FUNCTION PARAMETER PRECHECKS #################### 
    ...
    if (QC_filter == T)
      {
        ...
        for (i in 1:nrow(output))
         {
            convert = paste(binaryLogic::as.binary(as.integer(ou   script.path <- file.path(system.file("extract_modis_data.py", package = "PEcAn.data.remote"))
    ...
    # extract the data
    output <- extract_modis_data(product = product, band = band, lat = lat, lon = lon, start_date = start