  ...
  if (!is.null(outdir)) {
    if (file.exists(file.path(outdir, "AGB.csv")) && length(buffer) == 0 && as.logical(skip_buffer)) {
      ...
      AGB_Output <- matrix(NA, length(site_info$site_id), 2 * length(time_points) + 1) %>%
        ...
        as.data.frame() # we need: site_id, agb, sd, target time point.
      ...
      AGB.list <- time_points %>% furrr::future_map(function(t) {
        ...
        for (id in site_info$site_id) {
          ...
          if (length(site_AGB$agb[which(site_AGB$date == lubridate::year(t))]) == 1) {
            out.t <- rbind(out.t, list(
              mean = site_AGB$agb[which(site_AGB$date == lubridate::year(t))],
    ...
    if (is.null(buffer) | as.logical(skip_buffer)) {
      ...
      AGB_Output <- l %>%
        furrr::future_map(function(ll) {
          ...
          med_agb_data <- PEcAn.data.remote::extract.LandTrendr.AGB(
            site_info = site_info,
            dataset = "median",
            fun = "mean",
          ...
          sdev_agb_data <- PEcAn.data.remote::extract.LandTrendr.AGB(
            site_info = site_info,
            dataset = "stdv",
            fun = "mean",
            data_dir = ll$AGB_indir,
      ...
      # extracting AGB data
      med <- PEcAn.data.remote::extract.LandTrendr.AGB(
        ...
        dplyr::select(-2) %>%
        `colnames<-`(c("site_id", paste0(time_points, "_AbvGrndWood")))
      sdev <- PEcAn.data.remote::extract.LandTrendr.AGB(
        site_info = new_site_info,
        dataset = "stdv",
        buffer = buffer,
        fun = "mean",
