...
download.LandTrendr.AGB <- function(outdir, target_dataset = "biomass", product_dates = NULL,
  ...
  remote_filenames <- Map(function(p) {
    RCurl::getURL(p,
      ftp.use.epsv = FALSE,
  ...
  if (length(files_to_download_final) < 1) {
    ...
    if (run_parallel) {
      ...
      foreach::foreach(i = 1:length(files_to_download_final)) %dopar%
        try(PEcAn.utils::download.file(download_urls_final[i], file.path(
          outdir,
      ...
      Map(function(u, d) PEcAn.utils::download.file(u, d), download_urls_final, file.path(
        outdir,
