...
working_path <- "/home/bhardima/pecan/modules/data.remote/palsar_scenes/Link_to_cheas/geo_corrected_single_gamma/HV"
setwd(working_path)
...
filelist <- as.vector(list.files(path=getwd(), pattern=".tif" ,recursive=F))

for (i in 1:length(filelist)){ 
HH_inpath <-file.path(getwd(),filelist[i])
HV_inpath <-file.path(getwd(),filelist[i])
