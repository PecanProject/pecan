...
run_mslsp_prep_static <- function(year, overwrite = FALSE) {
  ...
  sge_path <- mslsp_sge_tiles_path(year)
  if (overwrite || !file.exists(sge_path)) {
    write_mslsp_sge_tiles(year, prep)
  }
  invisible(prep)
}
...
# Tiles to schedule on SGE: canonical tileids.txt order, restricted to tiles with
# ag parcels for this year (from prep cache). Not all HLS tiles have ag land.
mslsp_sge_tiles_path <- function(year) {
  file.path(mslsp_out_root, sprintf("year=%d", as.integer(year)), "sge_tiles.txt")
}

mslsp_sge_tiles <- function(prep) {
  canonical <- read_mslsp_tile_list()
  with_parcels <- names(prep$tile_to_parcel_ids)
  canonical[canonical %in% with_parcels]
}

write_mslsp_sge_tiles <- function(year, prep) {
  tiles <- mslsp_sge_tiles(prep)
  path <- mslsp_sge_tiles_path(year)
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  writeLines(tiles, path)
  n_canonical <- length(read_mslsp_tile_list())
  msg <- paste0(
    "[MSLSP prep] sge_tiles written: ", path,
...
read_mslsp_sge_tiles <- function(year) {
  path <- mslsp_sge_tiles_path(year)
  if (!file.exists(path)) {
    stop(
      "SGE tile list not found: ", path,
...
run_mslsp_extract_sge_tile <- function(year, overwrite = FALSE) {
  year <- as.integer(year)
  task_id <- suppressWarnings(as.integer(Sys.getenv("SGE_TASK_ID", "")))
  if (is.na(task_id) || task_id < 1L) {
    stop("SGE_TASK_ID must be set for tile-array extraction")
  }
  tiles <- read_mslsp_sge_tiles(year)
  if (task_id > length(tiles)) {
    message("[MSLSP] SGE task ", task_id, " > ", length(tiles), " scheduled tiles — nothing to do")
    return(invisible(NULL))
  }
