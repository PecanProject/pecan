...
load_events_helpers <- function() {
  ...
  if (length(file_arg) == 0L) {
    stop("load_events_helpers() must be called from make_events_statewide.R (Rscript --file=...).")
  }
  root <- dirname(normalizePath(sub("^--file=", "", file_arg[1L]), mustWork = FALSE))
  lib <- file.path(root, "R")
  ...
  source(file.path(lib, "tillage_metrics.R"))
