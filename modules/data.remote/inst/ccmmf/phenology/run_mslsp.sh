...
#   --task-tile      array mode: extract one tile (TASK_ID -> tiles_to_run.txt line)
...
export PHENOLOGY_ROOT="${PHENOLOGY_ROOT:-$SCRIPT_DIR}"
EXTRACT_ROOT="$PHENOLOGY_ROOT/extract"
...
log "MSLSP: years=$(IFS=,; echo "${YEARS[*]}") prep=$DO_PREP extract=$DO_EXTRACT combine=$DO_COMBINE task_tile=$DO_TASK_TILE overwrite=$OVERWRITE"
