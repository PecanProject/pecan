...
#
# Requires: source your setup_env.sh first (CCMMF_MANAGEMENT, CCMMF_MATCHED_DIR, ...).
set -euo pipefail
...
: "${CCMMF_MANAGEMENT:?Set CCMMF_MANAGEMENT (source setup_env.sh)}"
...
Rscript "$ROOT/gapfill/fit_phenology_gapfill_models.R"
Rscript "$ROOT/gapfill/apply_phenology_gapfill.R" "${YEARS[@]}"
