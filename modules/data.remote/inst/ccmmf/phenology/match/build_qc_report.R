...
cat("- **Matching logic** (see `match_landiq_mslsp.R`): Non-woody seasons match MSLSP by ADOY inside [OGI, OGMn]; tie-break nearest Peak then cycle 1. Woody seasons take the strongest remaining MSLSP cycle and do not use ADOY (`woody_strongest_cycle`). Season priority: season 2 (main) first when CLASS present; season 1 for MULTIUSE D/M; then 3/4. Young woody (YP / SPECOND=Y) is after mature woody on the same parcel-year. An unused extra MSLSP cycle does not veto an in-window non-woody match in `match_outcome`.\n")
cat("- **When ADOY is missing (non-woody):** Tie-break by season priority and mslsp_cycle (`no_adoy_recorded`). Using ADOY_EMRG as fallback is a possible next step (not in current workflow).\n")
...
cat("- **Woody parcels:** Assigned to the strongest MSLSP cycle; LandIQ ADOY is not used. Mature woody is assigned before YP / SPECOND=Y on the same parcel-year. Field-year label: `matched_woody_strongest_cycle`.\n")
cat("- **Cycle/season mismatches:** `mismatch_2cycles_1season` remains when the assigned non-woody season is not in-window (or has no ADOY). An extra unused cycle no longer overrides `matched_adoy_validated`.\n")
