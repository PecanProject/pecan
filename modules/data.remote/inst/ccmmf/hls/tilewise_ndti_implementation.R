# NDTI product for the tilewise framework (HLS bands, Fmask, per-parcel NDTI).
# Weighted stats: n_eff = w_valid^2/sum_w2; SE_weighted = ndti_sd/sqrt(n_eff).
#
# Main inputs: HLSL/HLSS roots, LandIQ paths, parcel-tile map (env).
# Main outputs: tilepieces and monthly Parquet under tillage/ndti_v4.1.
# How to run: sourced from tilewise_ndti_driver.R.
# Workflow: monitoring workflow NDTI branch.
...
ndti_parcel_tilemap <- Sys.getenv(
  "NDTI_PARCEL_TILEMAP",
  file.path(ndti_management, "hls_parcel_tile_map_v4.1_years=2016-2024.rds")
)
...
load_parcel_tilemap <- function(year = NULL) {
  if (!file.exists(ndti_parcel_tilemap)) return(NULL)
  dt <- as.data.table(readRDS(ndti_parcel_tilemap))
  if ("year" %in% names(dt)) {
    if (is.null(year)) return(NULL)
    dt <- dt[year == as.integer(year)][, year := NULL]
  }
  if (!all(c("parcel_id", "tileIDs") %in% names(dt))) return(NULL)
  dt[, parcel_id := as.character(parcel_id)]
  dt <- dt[, .(tileIDs = paste(sort(unique(unlist(
    strsplit(paste(tileIDs, collapse = ","), ",", fixed = TRUE)
  ))), collapse = ",")), by = parcel_id]
  setkey(dt, parcel_id)
  dt
}
