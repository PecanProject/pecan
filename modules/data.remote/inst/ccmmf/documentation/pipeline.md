...
**Stakeholders:** start at [documentation/README.md](README.md) for training sessions.
...
> `CCMMF_MANAGEMENT=$CCMMF_ROOT/management`. Stakeholders must override.
...
| `CCMMF_LANDIQ_GAPFILL_PRODUCT` | Gap-filled product (v4.1.2); downstream input after §5 |
...
> harmonized dir (same `crops_all_years.parq` + `parcels-consolidated.gpkg`). After §5,
> point `CCMMF_LANDIQ_V4` at `$CCMMF_LANDIQ_GAPFILL_PRODUCT` so §6–§10 consume the
> filled table.
...
Use when CADWR releases a new annual map. Skip if `TARGET_YEAR` is already in `crops_all_years.parq` from S3.
...
$CCMMF_ROOT/data_raw/cadwr_land_use/landiq_shapefiles/
  i15_Crop_Mapping_<TARGET_YEAR>_*/   # release name varies
```

Compare the legend for `TARGET_YEAR`; update `LandIQ_cropCode_lookup_table.csv` if new codes appear (`documentation/*_DWR_Legend.pdf`).

### Harmonize into v4.1

[cadwr-landuse](../../usr/ashiklom/cadwr-landuse/) iteratively overlays annual shapefiles. Adding a year regenerates **both** geometry and the long crop table.

```bash
cd "$CCMMF_ROOT/usr/ashiklom/cadwr-landuse"
# Ensure TARGET_YEAR tile exists under _results/v4.1/01-tiles-by-year/.../<year>.parq
qsub scripts/scc-harmonize.sh _results/v4.1
```

Copy or symlink outputs to `$CCMMF_LANDIQ_V4`.
...
Raw shapefile drop for a new year:

```text
$CCMMF_ROOT/data_raw/cadwr_land_use/landiq_shapefiles/i15_Crop_Mapping_<YEAR>_*/
```

(cadwr-landuse scripts must also list the new year path — they are hardcoded through 2023
today.)
...
[sessions/02-phenology.md](sessions/02-phenology.md) §2.4.
...
Fills missing planting/harvest dates (MSLSP → `lm(ADOY × CLASS)` → crop-class mean).
...
Match (§9) and lookups (§10) must be done first. Prefer running after §11 if you want
...
| ☐ | Harmonize `TARGET_YEAR` (wire path in cadwr-landuse) | year in crops parquet |
