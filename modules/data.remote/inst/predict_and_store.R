#Goes through all counties and predicts up until end_year using the optimized matrices.
#Stores county-separated LandIQ-style datasets inside BAU / NBS_Targets subfolders.
...
##-------scenario setup-------
#Crop classes are predicted from county_optimized_matrices/<county>_crop_matrix.csv.
#Tillage states are assigned from county_tillage_targets/<scenario_safe>/crop_by_till_targets_*.csv.

run_scenarios = c("BAU Targets", "NBS Targets")

prediction_root_dir = "county_landiq_predictions"
crop_matrix_dir = "county_optimized_matrices"
tillage_target_root = "county_tillage_targets"
...
if (length(missing_mats) > 0) {message("Counties in all_data without crop matrices: ", length(missing_mats))}
...
run_one_prediction_scenario = function(run_scenario) {
  ...
  future_crop = predict_grouped_markov_to_targets(
    year_states = all_data, transition_mats = crop_mats, 
  ...
  if ("till_state_2023" %in% names(future_landiq)) {
    future_landiq[, till_state_2023 := NULL]
  }
