...
#'
#' @examples
#' harv.var <- runSMAPvalidation(
#'   "1000004945", as.Date("2016-01-02"), as.Date("2016-07-16"),
#'   download_SMAP(
#'     "1000004945", "2016-01-02", "2016-07-16",
...
runSMAPvalidation <- function(betySiteId, start, end, smap_data = NULL, ensemble_dir) {
  ...
  moistureFrame <- extractVariableENS(
    betySiteId = betySiteId, predictVar = "SoilMoist",
  ...
  p1 <- ggplot(moistureFrame, aes(x = time, y = SoilMoist)) +
    geom_hex() +
    theme_bw() +
    ...
    labs(colour = "") +
    ylab("Surface Soil Moisture (ssm)") +
    xlab("Date of Prediction") +
    labs(colour = "Legend")
  print(p1)

  ## Min/Max Values with 95% Quantile Range ## Still has messed up color legend
  p2 <- ggplot(grouped_ENS, aes(x = time, y = SoilMoist)) +
    theme_bw() +
  ...
  if (!is.null(smap_data)) {
    ...
    plot(smap_data$Date, smap_data$ssm,
      main = "SMAP ssm (Surface Soil Moisture) vs Time",
    ...
    p4 <- ggplot(moistureFrame, aes(x = time, y = SoilMoist)) +
      geom_hex() +
      theme_bw() +
      geom_line(data = mean_per_ENS, aes(x = time, y = mean, color = "Mean of Model Data")) +
      ...
      theme(plot.title = element_text(hjust = 0.5)) +
      xlab("Date of Prediction/Collection") +
      ylab("Surface Soil Moisture (ssm)") +
      labs(colour = "Legend") +
      guides(fill = guide_legend(title = "Frequency of Model Data"))
    print(p4)
    ...
    plot(df$vec_mean, smap_data$ssm,
      main = "Actual SMAP vs Mean Model Data\nw/ Linear Regression Line",
      xlab = "Mean of Model Data", ylab = "Actual SMAP (ssm)"
    ) + abline(mod)
    p6 <- recordPlot()
    ...
    plot(df$vec_time, mod$residuals,
      main = "Residuals vs. Time", xlab = "Date of Collection",
      ylab = "Residuals of SMAP and Model Data"
    ) + abline(h = 0, lty = 2)
    p7 <- recordPlot()
    ...
    names(vtr) <- c(
      "betySiteId", "Plot1", "Plot2", "Plot3", "Plot4", "Plot5", "Plot6", "Plot7",
