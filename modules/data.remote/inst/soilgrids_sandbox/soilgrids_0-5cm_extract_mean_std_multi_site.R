...
# the function
soc_extract <- function (site_list=NULL, verbose=TRUE) {
  ...
  internal_site_info <- data.frame(site_list$id, site_list$sitename, site_list$lat, site_list$lon, site_list$time_zone)

  i <- 1 # setup counter
  #create a variable to store mean and quantile of SOC for each soil depth
  socquant <- matrix(NA, nrow = 6, ncol = length(internal_site_info$site_list.lon) * 4)
  siteids <- matrix(NA, nrow = 6, ncol = length(internal_site_info$site_list.id) * 4)
  lonlat <- cbind(internal_site_info$site_list.lon, internal_site_info$site_list.lat)
  ...
  for (dep in seq_along(depths)) {
    ...
    siteids[i,] <- rep(as.numeric(internal_site_info$site_list.id), 4)
    
    rownames(socquant) <- c("0-5", "5-15", "15-30", "30-60", "60-100", "100-200")
    colnames(socquant) <- c(rep("Mean", length(internal_site_info$site_list.lon)), 
                            rep("0.05", length(internal_site_info$site_list.lon)),
                            rep("0.5", length(internal_site_info$site_list.lon)),
                            rep("0.95", length(internal_site_info$site_list.lon)))
    rownames(siteids) <- c("0-5", "5-15", "15-30", "30-60", "60-100", "100-200")
    colnames(siteids) <- c(rep("Mean", length(internal_site_info$site_list.lon)), 
                          rep("0.05", length(internal_site_info$site_list.lon)), 
                          rep("0.5", length(internal_site_info$site_list.lon)), 
                          rep("0.95", length(internal_site_info$site_list.lon)))
    i <- i + 1
  ...
  mean_site <- matrix(mean, length(internal_site_info$site_list.lon), 6)
  rownames(mean_site) <- as.numeric(internal_site_info$site_list.id)
  colnames(mean_site) <- c("0-5cm",
                           ...
                           "60-100cm",
                           "100-200cm")
  mean_site.2 <- data.frame(site_id=internal_site_info$site_list.id, 
                            lat=internal_site_info$site_list.lat, 
                            lon=internal_site_info$site_list.lon, 
                            mean_site)
  colnames(mean_site.2)[4:9] <- c("0-5cm",
                                   ...
                                   "60-100cm",
                                   "100-200cm")
  std_site <- matrix(std, length(internal_site_info$site_list.lon), 6)
  rownames(std_site) <- as.numeric(internal_site_info$site_list.id)
  colnames(std_site) <- c("0-5cm",
                          ...
                          "60-100cm",
                          "100-200cm")
  std_site.2 <- data.frame(site_id=internal_site_info$site_list.id, 
                            lat=internal_site_info$site_list.lat, 
                            lon=internal_site_info$site_list.lon, 
                           std_site)
...
# extract SOC for each BETYdb SiteID
#result_soc <- soc_extract(site_list=NULL)

site_list <- qry_results.2
verbose <- TRUE
system.time(result_soc <- soc_extract(site_list=site_list, verbose=verbose))
result_soc
