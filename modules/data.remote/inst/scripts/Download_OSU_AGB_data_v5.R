...
foreach::foreach(i=1:length(med_urls)) %dopar% try(PEcAn.utils::download_file(med_urls[i], file.path(target_directory,med_files[i])))
rm(i)

# get stdv data
stdv_urls <- paste(URL,dataset_version,target_dataset,"stdv",stdv_files,sep="/")
PEcAn.logger::logger.info("*** Downloading AGB stdv data")
foreach::foreach(j=1:length(stdv_urls)) %dopar% try(PEcAn.utils::download_file(stdv_urls[j], file.path(target_directory,stdv_files[j])))
rm(j)
#--------------------------------------------------------------------------------------------------#
