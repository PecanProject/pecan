...
if (plot_results) {
  ...
  ggplot2::ggplot(data = formatted_data, aes(x = Year, y = AGB_med)) +
    geom_point(size = 2) +
    geom_errorbar(aes(x = Year, ymin = AGB_med - AGB_stdv, ymax = AGB_med + AGB_stdv), width = 0.25) +
    facet_wrap(~site_ID) +
    theme(
      axis.text = element_text(size = 18), legend.text = element_text(size = 18), axis.title.y = element_text(size = 18, face = "bold"),
