...
for (i in 1:length(pol_bands)) {
  for (j in 1:length(buff)) {
    ...
    plot(x, y,
      xlab = expression(sqrt(biomass)), ylab = pol_bands[i], main = buff[i], # something wrong here with main
      xlim = c(min(x), max(x)),
      ylim = c(min(y), max(y)),
      pch = 16, col = "#CCCCCC"
    )
