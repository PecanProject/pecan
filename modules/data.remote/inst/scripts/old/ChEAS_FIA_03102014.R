...
if (fia == 1) { # EXTRACTS FROM FIA COORDINATES
  ...
  if (kml == 1) {
    writeOGR(spdf.latlon, layer = 1, "WI_FIA.kml", driver = "KML") # export as kml (this puts in in the Home folder)
  }
...
plot(format(dat48$scndate, "%Y"), dat48$HH.sigma.48,
  col = "grey", cex = 0.5,
...
par(new = FALSE, mfrow = c(1, 1))
plot(format(dat48$scndate, "%Y"), dat48$HV.sigma.48,
  col = "grey", cex = 0.5,
...
for (i in dat48$plot) {
  lines(cbind(c(2007, 2008, 2009, 2010), date.plot.HVmean[dat48$plot[i], ]), col = i)
  par(new = T)
}
