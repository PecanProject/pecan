...
EDR <- function(paths, RT.matrix, par.wl, nir.wl, datetime, trait.values = list(), 
  ...
  # Extract paths TODO: Provide option to just a results path with implied file
  # structure (ED2IN, config.xml, history)
  ...
  if (change.history.time) {
    history.full.prefix <- EDR.preprocess.history(history.path, output.path, 
                                                  datetime, history.prefix)
  } else {
  ...
  if (!is.na(ed2in.path)) {
    EDR.preprocess.ed2in(ed2in.path, output.path, new.config.path, datetime, 
                         history.full.prefix)
  }
  ...
  if (clean) {
    delete.files <- file.remove(file.path(output.path, 
                                          c("lengths.dat", "reflect_par.dat", 
                                            "reflect_nir.dat", "trans_par.dat", "trans_nir.dat")))
    # NOTE that currently, not all files are deleted (e.g. history file, copied
    # ED2IN)
    if (!delete.files) {
...
#'      plot(albedo, type='l')
#' }
EDR.prospect <- function(prospect.param, prospect.version = 5, paths, 
                         par.wl, nir.wl, datetime, ...) {
