sampler_resp <- nimbleFunction(
                               ...
                               run = function(){
                                       ...
                                       for(sp in 1:nspec){
                                               theta <- zeroswl
                                               Refl <- zeroswl
                                               k <- (1.0/model$N[sp]) * (model$Cab[sp] * Cab_abs +
                                                                                 model$Cw[sp] * Cw_abs +
                                                                                 model$Cm[sp] * Cm_abs)
                                               tau <- zeroswl
                                               ...
                                               nmR = taoa * tao90 * (b90^(model$N[sp]-1.0) - b90^(1.0-model$N[sp]))
                                               dmRT = a90*b90^(model$N[sp]-1.0) - b90^(1.0-model$N[sp])/a90 - rho90 * (b90^(model$N[sp]-1.0) - b90^(1.0-model$N[sp]))
                                               Refl = rhoa + nmR / dmRT

                                               ## Gibbs Sampling
                                               specerror[,sp] <- Refl - observed[,sp]
                                       }
