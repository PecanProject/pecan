...
invert.custom <- function(observed, invert.options, quiet = FALSE) {
  ...
  need.invert.options <- c("inits", "ngibbs", "prior.function", "param.mins", 
  ...
  if (any(!overlap.invert.options)) {
    error.msg <- paste("Missing the following invert.options:", 
                       paste(need.invert.options[!overlap.invert.options], collapse = " "), "Try modifying a default.invert.options() object", sep = "\n")
    stop(error.msg)
  }
  ...
  for (ng in seq_len(ngibbs)) {
    ...
    if (ng%%adapt < 1) {
      if (ar < 2) {
        ...
        stdev   <- apply(results[region, seq_len(npars)], 2, sd)
