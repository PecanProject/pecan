...
prospect_LL <- nimbleFunction(
                setup = function(model, constants) {
                        ...
                        sa = sin(alpha)
                        b1V = sqrt((sa*sa - np/2) * (sa*sa - np/2) + kt) ## cannot reuse b1 that was a scalar previously as a vector now
                        b2 = sa*sa - np/2
                        bV = b1V - b2
                },
                run = function(){
                        ...
                        k = (2.0/model$N) * (model$Cab * Cab_abs + model$Cw * Cw_abs + model$Cm * Cm_abs)
...
prospectSpec <- configureMCMC(prospect, print=FALSE)
