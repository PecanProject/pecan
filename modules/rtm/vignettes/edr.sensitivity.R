...
base.output.dir <- file.path(
  "/projectnb", "dietzelab", "pecan.data",
  "output", "ashiklom"
)
ed2in.path <- file.path(
  base.output.dir, pecan.workflow.id,
  "run", ed.run.id, "ED2IN"
)
...
sensitivity.seqs <- list(
  ...
  ed.b1ht = c(
    seq(0.03, 0.5, length.out = n.sens / 2),
