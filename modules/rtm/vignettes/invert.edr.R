...
invert.options$model <- function(params) {
  ...
  paths <- list(
    ed2in = "/path/to/ED2IN", # Points directly at FILE
    history = "/path/to/history/"
  ) # Points to DIRECTORY where history files are stored

  prospect.params <- params[1:5]
  trait.values <- as.list(params[-(1:5)])
  out <- EDR.prospect(
    prospect.param = prospect.params,
    prospect.version = 5,
