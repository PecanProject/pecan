...
paths <- list(
  ed2in = file.path(data_dir, "ED2IN"),
  history = file.path(data_dir, "outputs")
)

output_dir <- tempdir()
file.copy(
  from = "~/Projects/ED2/EDR/build/ed_2.1-opt",
...
spectra_list <- list(
  temperate.Early_Hardwood = prospect(c(1.4, 40, 0.01, 0.01), 4, TRUE),
  temperate.Late_Hardwood = prospect(c(1.5, 25, 0.01, 0.012), 4, TRUE)
)

trait.values <- list(
  temperate.Early_Hardwood = list(orient_factor = 0.5),
