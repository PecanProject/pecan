...
Before moving on to the `BayesianTools` case, here is what your `pecan.PDA.xml` would look like for a case where you constrain parameters of more than one model PFT with more than one Ameriflux data stream: 
```
...
 <assim.batch>
  ...
...
The rest of the `<assim.batch>` block is the same. Below is an example for DREAMzs (recommended) algorithm. To make sure everything works fine, you may choose to initially set `<iter>` to some small number like 100:
```
 <assim.batch>
   <method>bayesian.tools</method>
   <bt.settings>
