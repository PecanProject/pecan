...
* `external.priors` This is a list of pecan probability distribution dataframes to be used as PDA priors, e.g.:
```
[[1]]
...
* `external.knots` This is a list of dataframes containing emulator training points. Only pass it if you want to build the emulator on specific knots that you want to have control over, otherwise knots are generated from the PDA priors using Latin Hypercube Sampling. Each sublist should be a `nknot x nPFTparam` dataframe, e.g. if you are building amulator with 250 knots and your PFT has 30 parameters, this will be a 250 x 30 dataframe where each row is a parameter vector. Therefore only the columns of parameters that you are targeting in PDA should have varying parameter values. E.g. if you are calibrating the `psnTOpt` parameter, it would look like this:
```
> external.knots
$temperate.deciduous
