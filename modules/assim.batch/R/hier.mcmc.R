...
hier.mcmc <- function(settings, gp.stack, nstack = NULL, nmcmc, rng_orig,
  ...
  for (g in 1:nmcmc) {
    ...
    if ((g > 2) && ((g - 1) %% settings$assim.batch$jump$adapt == 0)) {
      ...
      jcov.list <- lapply(seq_len(nsites), function(v) {
        pda.adjust.jumps.bs(
          settings, jcov.arr[, , v], musite.accept.count[v],
  ...
  return(list(
    mu_site_samp = mu_site_samp, mu_global_samp = mu_global_samp, sigma_global_samp = sigma_global_samp,
