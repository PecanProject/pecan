...
pda.get.model.output <- function(settings, run.id, bety, inputs, external.formats = NULL) {
  ...
  for (k in 1:n.input) {
    ...
    if (!is.null(external.formats)) {
      ...
      format <- PEcAn.DB::query.format.vars(
        bety = bety,
    ...
    # read model output
    model.raw <- as.data.frame(PEcAn.utils::read.output(run.id,
      outdir = file.path(settings$modeloutdir, run.id),
    ...
    model.secs <- PEcAn.utils::ud_convert(model$time, "days", "seconds")
