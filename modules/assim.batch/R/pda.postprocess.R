...
pda.postprocess <- function(settings, con, mcmc.param.list, pname, prior, prior.ind, sffx = NULL) {
  ...
  for (i in seq_along(settings$pfts)) {
    ...
    ## Save params
    filename.mcmc <- file.path(
      settings$pfts[[i]]$outdir,
      paste0(
        "mcmc.pda.",
    ...
    if (!is.null(con)) {
      ...
      posteriorid <- PEcAn.DB::db.query(paste0(
        "INSERT INTO posteriors (pft_id) VALUES (",
        pft.id, ") RETURNING id"
      ), con)
    ...
    post.distns <- PEcAn.MA::approx.posterior(
      trait.mcmc = params.subset[[i]],
      priors = prior[[i]],
      outdir = settings$pfts[[i]]$outdir,
      filename.flag = paste0(
        ".pda.", settings$pfts[[i]]$name, "_",
    ...
    filename <- file.path(
      settings$pfts[[i]]$outdir,
      paste0(
        "post.distns.pda.", settings$pfts[[i]]$name, "_",
...
pda.plot.params <- function(settings, mcmc.param.list, prior.ind, par.file.name = NULL, sffx) {
  ...
  for (i in seq_along(prior.ind)) {
    ...
    if (length(params.subset[[i]]) > 1) {
      ...
      utils::capture.output(coda::gelman.diag(params.subset[[i]], autoburnin = FALSE),
        file = filename.mcmc.temp,
        append = TRUE
      )
    }
...
write_sf_posterior <- function(sf.samp.list, sf.prior, sf.samp.filename) {
  ...
  sf.post.distns <- PEcAn.MA::approx.posterior(
    trait.mcmc = sf.subset.list[[1]], priors = sf.prior,
