...
pda.emulator <- function(settings, external.data = NULL, external.priors = NULL,
  ...
  if (!remote) {
    ...
    pda.restart.file <- file.path(settings_outdir, paste0(
      "history.pda",
    ...
    pda.restart.file <- paste0(
      settings_outdir, "history.pda",
      settings$assim.batch$ensemble.id, ".Rdata"
    )
  }
  ...
  knots.list <- lapply(
    ...
    function(x) {
      pda.generate.knots(
        settings$assim.batch$n.knot, sf, probs.sf,
        n.param.all[x],
        prior.ind.orig[[x]],
        prior.fn[[x]],
        pname[[x]]
  ...
  if (run.round & is.null(external.knots)) {
    ...
    sampled_knots <- sample_MCMC(
      settings$assim.batch$mcmc.path, n.param.orig, prior.ind.orig,
    ...
    for (i in seq_along(settings$pfts)) {
      knots.list[[i]]$params <- rbind(
        knots.params[[i]][mix.knots, ],
        knots.params.temp[[i]]
      )
  ...
  if (run.normal | run.round) {
    ...
    run.ids <- pda.init.run(settings, con, my.write.config, workflow.id, knots.params,
      ...
      run.names = paste0(
        settings$assim.batch$ensemble.id, ".knot.",
  ...
  if (run.normal | run.round) {
    ...
    for (c in seq_len(settings$assim.batch$chain)) {
      jmp.list[[c]] <- sapply(
        prior.fn.all$qprior,
        function(x) 0.1 * diff(eval(x, list(p = c(0.05, 0.95))))
      )[prior.ind.all]
  ...
  for (c in seq_len(settings$assim.batch$chain)) {
    ...
    for (i in seq_along(prior.ind.all.ns)) {
      ...
      if (any(grepl(sf.check, sf))) {
        m[, i] <- eval(
          prior.fn.all$qprior[prior.ind.all.ns][[i]],
  ...
  settings$assim.batch$emulator.path <- file.path(
    settings_outdir,
    paste0(
      "emulator.pda",
  ...
  settings$assim.batch$ss.path <- file.path(
    settings_outdir,
    paste0(
      "ss.pda",
      settings$assim.batch$ensemble.id,
  ...
  settings$assim.batch$mcmc.path <- file.path(
    settings_outdir,
    paste0(
      "mcmc.list.pda",
      settings$assim.batch$ensemble.id,
  ...
  settings$assim.batch$resume.path <- file.path(
    settings_outdir,
    paste0(
      "resume.pda",
      settings$assim.batch$ensemble.id,
  ...
  if (any(unlist(any.mgauss) == "multipGauss")) {
    settings$assim.batch$bias.path <- file.path(
      settings_outdir,
      paste0(
        "bias.pda",
        settings$assim.batch$ensemble.id,
  ...
  if (length(mcmc.param.list) > length(settings$pfts)) {
    ...
    for (c in seq_len(settings$assim.batch$chain)) {
      mcmc.param.list[[length(mcmc.param.list)]][[c]] <- cbind(
        mcmc.param.list[[length(mcmc.param.list)]][[c]],
        mcmc.out[[c]]$mcmc.par
      )
    }
