...
pda.load.priors <- function(settings, con, extension.check = FALSE) {
  ...
  if (is.null(settings$assim.batch$prior$prior.id)) {
    ...
    for (i in seq_along(settings$pfts)) {
      pft.id <- PEcAn.DB::db.query(
        paste0(
          "SELECT pfts.id FROM pfts, modeltypes WHERE pfts.name='",
      ...
      prior.db <- PEcAn.DB::db.query(paste0(
        "SELECT * from dbfiles where container_type = 'Posterior' and container_id IN (",
        paste(priors$id, collapse = ","), ")"
      ), con)
  ...
  params.no.priors <- which(is.na(match(
    unlist(settings$assim.batch$param.names),
...
pda.create.ensemble <- function(settings, con, workflow.id) {
  if (!is.null(con)) {
    ...
    ensemble.id <- PEcAn.DB::db.query(paste("INSERT INTO ensembles (runtype, workflow_id) values ('",
      ensemble.type, "', ", workflow.id, ") RETURNING id",
      sep = ""
    ), con)
  } else {
    ensemble.id <- NA
...
pda.define.prior.fn <- function(prior) {
  ...
  return(list(
    dprior = dprior, rprior = rprior, qprior = qprior,
...
pda.adjust.jumps <- function(settings, jmp.list, accept.rate, pnames = NULL) {
  PEcAn.logger::logger.info(paste0(
    "Acceptance rates were (", paste(pnames, collapse = ", "), ") = (",
...
pda.adjust.jumps.bs <- function(settings, jcov, accept.count, params.recent) {
  ...
  PEcAn.logger::logger.info(paste0(
    "New jump variance diagonals are (",
    paste(round(diag(jcov), 3), collapse = ", "), ")"
  ))
  return(jcov)
} # pda.adjust.jumps.bs
...
return_multi_site_objects <- function(multi.settings) {
  ...
  knots.list <- lapply(
    ...
    function(x) {
      pda.generate.knots(
        as.numeric(settings$assim.batch$n.knot), NULL, NULL,
        n.param.all[x],
        prior.ind.orig[[x]],
        prior.fn[[x]],
        pname[[x]]
  ...
  if (!is.null(settings$assim.batch$round_counter)) {
    ...
    for (i in seq_along(multi.settings)) {
      ...
      sampled_knots <- sample_MCMC(
        file.path(settings$outdir, basename(settings$assim.batch$mcmc.path)),
    ...
    collect_site_knots <- collect_site_knots[sample(
      1:nrow(collect_site_knots),
      2 * as.numeric(settings$assim.batch$n.knot)
    ), ]

    # bring the previous set in
    need_obj <- load_pda_history(
      workdir = settings$outdir,
...
prepare_pda_remote <- function(settings, site = 1, multi_site_objects) {
  ...
  cat(paste0(
    "mv ", multi_site_objects$ensembleidlist[site], "/pecan.pda",
    multi_site_objects$ensembleidlist[site], ".xml ", remote_dir
  ), file = local_sub_file, append = TRUE)
  ...
  if (!is.null(settings$assim.batch$round_counter)) {
    external_data_line <- paste0("load(\"", file.path(
      remote_dir,
      paste0(
        "external.",
...
sync_pda_remote <- function(multi.settings, ensembleidlist, register = FALSE) {
  ...
  for (ms in seq_along(multi.settings)) {
    tmp_settings <- PEcAn.settings::read.settings(paste0(
      multi.settings[[ms]]$outdir, "/pecan.pda",
