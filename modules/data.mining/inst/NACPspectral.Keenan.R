...
if (FALSE) {
  ...
  model.set <- c(sort(c(
    ...
    "LPJ", "BIOME_BGC", "SSiB2", "TRIPLEX", "EPIC"
  )), "MEAN")
  ...
  for (model.dir in vars) {
    ...
    for (i in seq_along(site.files)) {
      ## for(i in c(5,25,38,45)){ for(i in c(1,5,7,8,9,25,26,30,38,45)){
      ...
      for (j in 3:ncol(dat)) {
        ...
        save(Nperiod, period, day, POWER, file = paste0("NACPspec.Keenan.", site, ".Rdata"))
...
for (sitenum in c(1, 5, 7, 8, 9, 25, 26, 38, 45)) {
  ...
  load(paste0(prefix, "pseudo.Rdata"))
  ...
  for (j in seq_along(dat)) {
    ...
    save(period, POWER, file = paste0("NACPspecNULL.Keenan.", site.name, ".Rdata"))
