...
for (s in sites) {
  ...
  for (m in seq_len(Nmodel)) {
    ...
    if (sum(Power, na.rm = TRUE) > 0) {
      ...
      pow.bin <- c(
        sum(Pglobe[subday.bin]), sum(Pglobe[day.bin]), sum(Pglobe[synop.bin]),
        sum(Pglobe[year.bin]),
      ...
      pow.binN <- c(
        sum(Pnorm[subday.bin]), sum(Pnorm[day.bin]), sum(Pnorm[synop.bin]),
        sum(Pnorm[year.bin]),
        sum(Pnorm[inter.bin])
      )
  ...
  cross.site[[s]] <- list(
    avg = avg.spec / mcount, norm = norm.spec / mcount, geom = exp(geom.spec / mcount),
...
for (m in seq_len(Nmodel)) {
  ...
  if (nsite > 0) {
    ...
    if (FALSE) {
      ...
      plot(Period, P.bar, ylim = c(1, maxP), log = "xy", type = "n", ylab = "Power", main = paste(
        "RAW",
        model.set[m]
      ), xlab = "period (days)")
    ...
    ### Normalized Spectra
    plot(Period, P.norm, ylim = range(1e-07, 1), log = "xy", type = "n", ylab = "Power", main = paste(
      "NORMALISED",
      model.set[m]
    ), xlab = "period (days)")
    for (i in seq_along(sites)) {
