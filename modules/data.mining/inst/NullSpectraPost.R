...
## THIS PART OF THE CODE COMBINES SPECTRA FROM DIFFERENT FILES GENERATED BY THE MAIN CODE
...
for (sitenum in c(5, 7, 8, 9, 25, 38, 45)) {
    # 1,26
    ...
    ## for(nstart in c(1,101,201,301,401,501,601,701,801,901)){
    for (nstart in seq(1, 951, by = 50)) {
        ## for(nstart in c(751,801,851,901,951)){
        print(c(sitenum, nstart))
        load(paste0("Rdata/", site.name, ".", nstart, ".specCI.Rdata"))
        
        if (is.null(PspecG)) {
            PspecG <- matrix(NA, length(Period), 1000)
        }
        PspecG[, seq(nstart, length = stepsize, by = 1)] <- Pspec[seq_along(Period), seq(nstart, length = stepsize, 
            by = 1)]
    ...
    if (FALSE) {
        plot(Period, pbar, log = "xy", ylim = c(10000, max(pCI)), type = "l", ylab = "Power", xlab = "Period (days)", 
            main = "US-Ho1 Null Spectra", lwd = 2, cex.lab = 1.5, cex.axis = 1.2, cex.main = 1.5)
    ...
    save(pSD, pbar, pCI, PspecG, Period, file = paste0(site.name, ".specCI.Rdata"))
