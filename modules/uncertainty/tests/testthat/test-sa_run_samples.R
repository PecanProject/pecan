...
test_that("the median run is named for what it is", {
  ids <- sa_run_descriptions(sa_design_fixture(), site_id = "772", pft_names = "pft1")

  expect_equal(ids$id[1], "SA-median-772")
  expect_match(ids$paramlist[1], "quantile=MEDIAN,trait=all")
})
...
test_that("a moved run is named for the trait and quantile it moves", {
  ...
  expect_equal(ids$id[2], "SA-pft1-SLA-0.159-772")
  expect_equal(ids$paramlist[2], "quantile=15.9,trait=SLA,pft=pft1")
})
...
test_that("one id is built per design row", {
  ...
  expect_equal(nrow(ids), nrow(design))
})
...
test_that("the lookup finds each run by its trait and quantile", {
  ...
  expect_equal(table$pft1["15.9", "SLA"], ids$id[2])
  expect_equal(table$pft1["84.1", "Vcmax"], ids$id[5])
})
...
test_that("each PFT gets its own lookup", {
  ...
  ids <- sa_run_descriptions(design, site_id = "772", pft_names = c("pft1", "pft2"))
