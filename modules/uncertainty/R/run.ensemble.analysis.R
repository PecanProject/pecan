#' Run ensemble analysis and generate summary plots
#' 
#' Generates distribution and optional time-series plots for ensemble
#' model outputs. Reads processed ensemble output files and produces
#' summary PDFs in \code{settings$outdir}.
#' 
#' @param settings PEcAn \code{Settings} object
...
#' @param variable variable name(s) to process, taken from settings if not specified
#'   taken as a vector in case of multiple variables
#' @param start.year First year of analysis, taken from settings if not specified
#' @param end.year Last year of analysis, taken from settings if not specified
#' @param ... additional arguments passed to [ensemble.ts()]
#'
#' @details
#' Produces histograms, boxplots, and optionally time-series plots with
#' ensemble mean and 95% confidence intervals. Requires ensemble output
#' files generated by \code{\link{get.results}}
#' 
#' @return nothing, creates ensemble plots as ensemble.analysis.pdf.
#'   If \code{plot.timeseries = TRUE}, creates ensemble.ts.pdf and 
#'   also saves ts analysis object as \code{.Rdata} file
#' @export
#' @seealso \code{\link{get.results}}, \code{\link{read.ensemble.ts}}, \code{\link{ensemble.ts}} 
...
#'
#' Loads ensemble member outputs for a specified variable and time range,
#' and returns a matrix of ensemble members by time.#' 
#' 
...
#'
#' @details
#' Reads model output files for each ensemble member, applies any
#' requested derived variable expressions, and saves the resulting
#' time-series object to disk (as \code{.Rdata} file).
#' 
#' @return named list of matrices (ensemble members × time)
#'
#' @export
#'
...
#'
#' @param ensemble.ts ensemble timeseries to be plotted, as returned by [\code{\link{read.ensemble.ts}}]
#' @param observations observed data to plot over timeseries predictions
#' @param window Integer, number of timepoints to average across
#' @param ... additional arguments, currently ignored
#'
#' @return Invisibly returns a list containing ensemble mean and confidence intervals,
#'   generates an ensemble time-series plot
#'
#' @export
#' @seealso \code{\link{read.ensemble.ts}}
#' @author Michael Dietze, Ryan Kelly
