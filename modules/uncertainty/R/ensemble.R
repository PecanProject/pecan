...
write.ensemble.configs <- function(defaults, ensemble.samples, settings, model, 
  ...
  if (is.null(restart)){
    ...
    # Let's find the PFT based on site location, if it was found I will subset the ensemble.samples otherwise were not affecting anything
    Pft_Site<- tbl(con, 'sites_cultivars')%>%
