...
get.gi.phii <- function(splinefuns, trait.samples, maxn = NULL) {
  ...
  if (class(trait.samples) != "matrix") {
    stop(paste("variance.decomposition currently does not handle trait.samples of class", 
               class(trait.samples), 
...
spline.ensemble <- function(gi.phii, median) {
  ...
  ## Equation 3 1. calculate residuals (g_i(phi_i,j) - g_i(median))
  residuals <- gi.phii - median
  ## 2. sum residuals by row, then truncate at 0:
  spline.estimate <- sapply(rowSums(residuals), function(x) max(0, x + median))
...
vd.variance <- function(gi.phii) {
  ...
  return(list(var = sum(var.phii), sd = sqrt(sum(sd.var.phii ^ 2))))
} # vd.variance
