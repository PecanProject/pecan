...
run.sensitivity.analysis <- function(settings, plot = TRUE, ensemble.id = NULL, 
  ...
  if ("sensitivity.analysis" %in% names(settings)) {
    ...
    ### Load samples Have to load samples.Rdata for the traits. But can overwrite the
    ### run ids if a sensitivity analysis ensemble id provided. samples.Rdata always
    ### has only the most recent ensembles for both ensemble and sensitivity runs.
    fname <- file.path(settings$outdir, "samples.Rdata")
    if (!file.exists(fname)) {
      logger.severe("No samples.Rdata file found!")
    }
    load(fname)
    
    # Can specify ensemble ids manually. If not, look in settings. If none there,
    # will use the most recent, which was loaded with samples.Rdata
    ...
    for (pft in settings$pfts) {
      ...
      ## settings$outdir, pft.name=pft$name)
      
      C.units <- grepl("^Celsius$", trait.lookup(traits)$units, ignore.case = TRUE)
      if (any(C.units)) {
        for (x in which(C.units)){
          trait.samples[[pft$name]][[x]] <- trait.samples[[pft$name]][[x]] + 273.15
        } 
      }
      
      ## only perform sensitivity analysis on traits where no more than 2 results are
      ## missing
      good.saruns <- sapply(sensitivity.output[[pft$name]], 
                            function(x) sum(is.na(x)) <= 2)
      if (!all(good.saruns)) {
        # if any bad saruns, reduce list of traits and print warning
      ...
      ### Gather SA results
      sensitivity.results[[pft$name]] <- 
        sensitivity.analysis(trait.samples = trait.samples[[pft$name]][traits], 
      ...
      if (plot) {
        ...
        sensitivity.plots <- 
          plot.sensitivities(sensitivity.results[[pft$name]]$sensitivity.output, 
                             linesize = 1, dotsize = 3)
        ...
        fname <- sensitivity.filename(settings, 
