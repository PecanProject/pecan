...
define_benchmark <- function(settings, bety) {
  ...
  if (is.null(bm.settings$reference_run_id)) {
    if (!is.null(bm.settings$ensemble_id)) {
      ...
      bm_ens <- tbl(bety, "benchmarks_ensembles") %>%
        rename(bm_ensemble_id = id) %>%
        filter(ensemble_id == bm.settings$ensemble_id) %>%
        collect()

      if (length(bm_ens) == 0) {
        # Get workflow id from ensemble id
        ens_wf <- tbl(bety, "ensembles") %>%
          filter(id == bm.settings$ensemble_id) %>%
          rename(ensemble_id = id) %>%
          left_join(., tbl(bety, "workflows") %>% rename(workflow_id = id), by = "workflow_id") %>%
          collect()
        BRR <- create_BRR(ens_wf, con = bety$con, user_id = settings$info$userid)
      } else if (dim(bm_ens)[1] == 1) {
        BRR <- tbl(bety, "reference_runs") %>%
          filter(id == bm_ens$reference_run_id) %>%
          rename(reference_run_id = id) %>%
          collect()
      ...
      logger.error("Cannot find or create benchmark reference run")
    }
  } else {
    logger.debug("Reference run already created")
  }
  ...
  for (i in which.bm) {
    ...
    bmBRR <- tbl(bety, "benchmarks_benchmarks_reference_runs") %>%
      ...
      filter(reference_run_id == bm.settings$reference_run_id) %>%
      collect()
