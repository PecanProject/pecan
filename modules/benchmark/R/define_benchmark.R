...
define_benchmark <- function(settings, bety){
  ...
  if (is.null(bm.settings$reference_run_id)) {
    if (!is.null(bm.settings$ensemble_id)) {
      ...
      bm_ens <- dplyr::tbl(bety,"benchmarks_ensembles") %>%
        dplyr::rename(bm_ensemble_id = .data$id) %>%
        dplyr::filter(.data$ensemble_id == bm.settings$ensemble_id) %>%
        dplyr::collect()

      if (length(bm_ens) == 0) {
        # Get workflow id from ensemble id
        ens_wf <- dplyr::tbl(bety, 'ensembles') %>%
          dplyr::filter(.data$id == bm.settings$ensemble_id) %>%
          dplyr::rename(ensemble_id = .data$id) %>%
          dplyr::left_join(
            dplyr::tbl(bety, "workflows") %>% dplyr::rename(workflow_id = .data$id),
            by = "workflow_id"
          ) %>%
          dplyr::collect()
        BRR <- create_BRR(ens_wf, con = bety, user_id = settings$info$userid)
      } else if(dim(bm_ens)[1] == 1) {
        BRR <- dplyr::tbl(bety,"reference_runs") %>%
          dplyr::filter(.data$id == bm_ens$reference_run_id) %>%
          dplyr::rename(reference_run_id = .data$id) %>%
          dplyr::collect()
      } else if(dim(bm_ens)[1] > 1) { # There shouldn't be more than one reference run per run
        PEcAn.logger::logger.error(
          "There is more than one reference run in the database for this ensemble id. Review for duplicates."
        )
      }
      ...
      PEcAn.logger::logger.error("Cannot find or create benchmark reference run")
    }
  } else {
    PEcAn.logger::logger.debug("Reference run already created")
  }
  ...
  for (i in which.bm) {
    ...
    if (!is.null(benchmark$metrics)) {
      ...
      metric_ids <- dplyr::tbl(bety, 'metrics') %>% dplyr::pull(.data$id)
    }
    ...
    bm <- dplyr::tbl(bety, 'benchmarks') %>%
      dplyr::filter(.data$input_id == benchmark$input_id) %>%
      dplyr::filter(.data$variable_id == benchmark$variable_id) %>%
    ...
    bmBRR <- dplyr::tbl(bety, 'benchmarks_benchmarks_reference_runs') %>%
      dplyr::filter(.data$benchmark_id == bm$id) %>%
      dplyr::filter(.data$reference_run_id == bm.settings$reference_run_id) %>%
      dplyr::collect()
    ...
    for (k in seq_along(metric_ids)) {
      bmmetric <- dplyr::tbl(bety, 'benchmarks_metrics') %>%
        dplyr::filter(.data$benchmark_id == bm$id) %>%
        dplyr::filter(.data$metric_id == metric_ids[[k]]) %>%
        dplyr::collect()
