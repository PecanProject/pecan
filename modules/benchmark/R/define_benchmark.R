...
define_benchmark <- function(settings, bety) {
  ...
  if (is.null(bm.settings$reference_run_id)) {
    if (!is.null(bm.settings$ensemble_id)) {
      ...
      bm_ens <- dplyr::tbl(bety, "benchmarks_ensembles") %>%
        rename(bm_ensemble_id = .data$id) %>%
        filter(.data$ensemble_id == bm.settings$ensemble_id) %>%
        collect()

      if (length(bm_ens) == 0) {
        # Get workflow id from ensemble id
        ens_wf <- dplyr::tbl(bety, "ensembles") %>%
          filter(.data$id == bm.settings$ensemble_id) %>%
          rename(ensemble_id = .data$id) %>%
          dplyr::left_join(tbl(bety, "workflows") %>% rename(workflow_id = .data$id), by = "workflow_id") %>%
          collect()
        BRR <- create_BRR(ens_wf, con = bety, user_id = settings$info$userid)
      } else if (dim(bm_ens)[1] == 1) {
        BRR <- tbl(bety, "reference_runs") %>%
          filter(.data$id == bm_ens$reference_run_id) %>%
          rename(reference_run_id = .data$id) %>%
          collect()
  ...
  for (i in which.bm) {
    ...
    if (dim(bm)[1] == 0) {
      cmd <- sprintf(
        paste0(
          "INSERT INTO benchmarks (input_id, variable_id, site_id, user_id)",
      ...
      PEcAn.logger::logger.debug(sprintf(
        "Benchmark %.0f for input %.0f variable %.0f created",
        bm$id, bm$input_id, bm$variable_id
      ))
    } else if (dim(bm)[1] > 1) {
      PEcAn.logger::logger.error(sprintf(
        "DUPLICATE records exist for input %.0f variable %.0f",
        as.numeric(benchmark$input_id), benchmark$variable_id
      ))
    } else {
      PEcAn.logger::logger.debug(sprintf(
        "Benchmark %.0f for input %.0f variable %.0f exists",
        bm$id, bm$input_id, bm$variable_id
      ))
    }
    ...
    bmBRR <- tbl(bety, "benchmarks_benchmarks_reference_runs") %>%
      ...
      filter(.data$reference_run_id == bm.settings$reference_run_id) %>%
      collect()

    if (dim(bmBRR)[1] == 0) {
      cmd <- sprintf(
        paste0(
          "INSERT INTO benchmarks_benchmarks_reference_runs",
    ...
    for (k in seq_along(metric_ids)) {
      bmmetric <- tbl(bety, "benchmarks_metrics") %>%
        filter(.data$benchmark_id == bm$id) %>%
        filter(.data$metric_id == metric_ids[[k]]) %>%
        collect()

      if (dim(bmmetric)[1] == 0) {
        cmd <- sprintf(
          paste0("INSERT INTO benchmarks_metrics (benchmark_id, metric_id) VALUES (%s, %s)"),
          bm$id, metric_ids[[k]]
        )
        PEcAn.DB::db.query(cmd, bety)
