...
align_data <- function(model.calc, obvs.calc, var, align_method = "match_timestep") {
  ...
  date_subsets[["m"]] <- model.calc %>% 
    dplyr::filter(rng_dat[1] <= .data$round.posix) %>%
    dplyr::filter(rng_dat[2] >= .data$round.posix)
  ...
  date_subsets[[compare$type[fine]]] <- date_subsets[[compare$type[fine]]] %>% 
    dplyr::filter(coarse_range_check[1] <= .data$round.posix) %>%
    dplyr::filter(coarse_range_check[2] >= .data$round.posix)
