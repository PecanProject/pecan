...
calc_benchmark <- function(settings, bety, start_year = NA, end_year = NA) {
  ...
  if ("benchmarking" %in% names(settings)) {
    ...
    ensemble <- tbl(bety, "ensembles") %>%
      filter(.data$workflow_id == settings$workflow$id) %>%
      collect()
    ...
    bms <- tbl(bety, "benchmarks") %>%
      dplyr::rename(benchmark_id = "id") %>%
    ...
    for (input.id in unique(bms$input_id)) {
      ...
      read.model <- PEcAn.utils::read.output(
        runid = basename(model_run),
        outdir = model_run,
        ...
        for (metric.id in metrics$id) {
          ...
          if (dim(score.entry)[1] == 0) {
            PEcAn.DB::db.query(paste0(
              ...
              "('", score, "',", bm.ensemble$id, ", ", bm$id, ",", metric.id, ")"
            ), bety)
      ...
      result.out <- list(
        bench.results = do.call(rbind, results.list),
        data.path = data.path,
