...
calc.metrics <- function(data.path, format, model_run, metrics, start_year=NA, end_year=NA, site=NA){
  ...
  model <- read.output(model_run$id, model_run$outdir, start.year=NA, end.year=NA, var=format$mstmip_name)
  # Names need to be mstmip variable names -- need to figure out conversion from bety names. To be done with the lookup table. 
  ...
  # returns observation list object with variable values (including time), variable name and units
