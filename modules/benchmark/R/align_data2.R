align_data <- function(model.calc, obvs.calc, var, start_year, end_year, align_method = "match_timestep") {
  ...
  if (mode.o != mode.m) {
    ...
    out1 <- model_sub[, as.character(var), drop = FALSE]
    colnames(out1) <- paste0(as.character(var), ".m")
    out2 <- obvs_sub[, as.character(var), drop = FALSE] #This also has NEE data
    colnames(out2) <- paste0(as.character(var), ".o")
