...
* Datasets
  ...
  *  `ca_n_application_rate` dataset with recommended N application rates (g N/m2) for 41 California crops from CDFA-FREP and UC ANR sources.
  *  `ca_compost_amendment` dataset with C:N ratios, nitrogen, and PAN (g/m2) for 32 organic amendment materials, plus a `material_class` column mapping each material to the CalRecycle taxonomy (14 CCR section 17852).
* Functions
