...
} ## simple function to convert data to numeric
...
for (i in seq_along(paleon.sitegroups)) {
  ...
  for (j in seq_len(nrow(group))) {
    ...
    if (length(id) == 0) {
      ...
      elev <- 0
      InsertString <- paste0(
        "INSERT INTO sites(sitename,country,geometry,user_id) VALUES(",
        "'", sitename, "', ",
        "'", country, "', ",
      ...
      if (new.id %in% pecan.sgs$site_id) {
        ...
        InsertSite <- paste0(
          "INSERT INTO sitegroups_sites(sitegroup_id,site_id) VALUES(",
          pecan.sitegroup$id, ", ",
          new.id, ")"
        )
...
for (i in seq_along(paleon.sitegroups)) {
  ...
  for (j in seq_len(nrow(pecan.sgs))) {
    ...
    site.info <- rbind(site.info, data.frame(
      id = site_id,
  ...
  for (j in seq_len(nrow(pecan.sgs))) {
    ...
    if (all(years %in% local.years)) {
      ...
      if (nrow(input.check) == 0 | length(grep("PalEON", input.check$name)) == 0) {
        ## create new input
        cmd <- paste0(
          "INSERT INTO inputs (site_id, format_id, start_date, end_date, name, parent_id) VALUES (",
      ...
      dbfile.insert(
        in.path = remote.dir, in.prefix = "", type = "Input", id = site.input.id,
