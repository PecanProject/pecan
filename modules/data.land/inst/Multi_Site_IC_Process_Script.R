...
for (i in 1:length(settings)) {
  ...
  veg_info <- try(extract_NEON_veg(
    lat = as.numeric(temp_settings$run$site$lat),
  ...
  # write into log file
  if (sum(Ind) != 3) {
    log_file <- c(log_file, paste0(
      "Site: ", temp_settings$run$site$id, ". No ", paste(Var[which(!(Var %in% bin_var))]),
  ...
  # sample_ic function
  sample_ic_results <- PEcAn.data.land::sample_ic(
