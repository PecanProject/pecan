...
jags.comb <- NULL
for(i in 21:24){
  load(paste0("C:/Users/mekevans/Documents/old_user/Documents/CDrive/Bayes/DemogRangeMod/ProofOfConcept/treerings/FIAmetadata/ArizonaData/MergedDatabase/New/ESAoutput/IGF.7.",i,".RData"))
...
save(jags.comb,file="IGF.7.ESA.RData")
...
tmax <- mean(time_data$tmax.JanA, na.rm=TRUE)
...
stdBeta[2] <- mean(betas[,"betaSDI_tmax.JanA"])/SDI/tmax
stdBeta[3] <- mean(betas[,"betaSICOND"])/SI
stdBeta[4] <- mean(betas[,"betaSICOND_SDI"])/SI/SDI
stdBeta[5] <- mean(betas[,"betaSICOND_tmax.JanA"])/SI/tmax
...
stdBeta[10] <- mean(betas[,"betaX_tmax.JanA"])/x/tmax
stdBeta[11] <- mean(betas[,"betatmax.JanA"])/tmax
...
## Size from model 7 (tmax as predictor)
...
for(k in seq_along(i)){
  j <- i[k]
  incX[k,] <- B0[j] + betas[j,"betaSDI"]*SDI + betas[j,"betaSDI_tmax.JanA"]*SDI*tmax + 
    betas[j,"betaSICOND"]*SI + betas[j,"betaSICOND_SDI"]*SDI*SI + 
    betas[j,"betaSICOND_tmax.JanA"]*SI*tmax + betas[j,"betaX"]*xseq + betas[j,"betaX2"]*xseq*xseq +
    betas[j,"betaX_SDI"]*xseq*SDI + betas[j,"betaX_SICOND"]*xseq*SI + betas[j,"betaX_tmax.JanA"]*xseq*tmax +
    betas[j,"betatmax.JanA"]*tmax
}
