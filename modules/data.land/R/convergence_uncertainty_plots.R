...
#####################################################################################
file.base.name <- "Full.model.stg1.distrubtime_v2."
output.base.name <- "Full.model.stg1.distrubtime_v2"
stage2 <- TRUE
...
for(i in 238:288){ # note this model stopped early b/c convergence
...
png(height = 10, width = 8, units = "in", res = 200, paste0(workingdir,output.base.name,"_ACFplots.png")
    ...
    for(k in seq_along(i)){
      ...
      incP_tlo[k,] <- B0[j] + betas[j,"betaSDI"]*SDI + betas[j,"betaSDI_wintP.wateryr"]*SDI*wintPseq+ 
        betas[j,"betaSICOND"]*SI + betas[j,"betaSICOND_SDI"]*SDI*SI + 
        betas[j,"betaSICOND_wintP.wateryr"]*SI*wintPseq+ betas[j,"betaX"]*x[1] + 
        betas[j,"betaX_SDI"]*x[1]*SDI + betas[j,"betaX_SICOND"]*x[1]*SI + betas[j,"betaX_wintP.wateryr"]*x[1]*wintPseq+
        betas[j,"betawintP.wateryr"]*wintPseq+
        # temperature
        betas[j,"betaSICOND_tmax.fallspr"]*SI*sprfallTmaxhl[1] + betas[j,"betaX_tmax.fallspr"]*x[1]*sprfallTmaxhl[1] +
        ...
        # time since fire
        betas[j,"betaSICOND_TimeSinceFIRE"]*SI*timesinceFire + betas[j,"betaX_TimeSinceFIRE"]*x[1]*timesinceFire +
        betas[j,"betaTimeSinceFIRE"]*timesinceFire + betas[j,"betaSDI_TimeSinceFIRE"]*SDI*timesinceFire +
      ...
      incP_thi[k,] <- B0[j] + betas[j,"betaSDI"]*SDI + betas[j,"betaSDI_wintP.wateryr"]*SDI*wintPseq+ 
        ...
        betas[j,"betawintP.wateryr"]*wintPseq+
        # temperature
        betas[j,"betaSICOND_tmax.fallspr"]*SI*sprfallTmaxhl[2] + betas[j,"betaX_tmax.fallspr"]*x[1]*sprfallTmaxhl[2] +
    ...
    Fire.SI.Effect %<a-% {
      ...
      legend("bottomright",legend=c("low SI","mean","high SI"),col=c("blue",1,2),lwd=3)
    }
