...
match_pft <- function(bety_species_id, pfts, query = NULL, con = NULL, allow_missing = FALSE) {
  ...
  if (!is.null(con)) {
    for (pft in pfts) {
      if (is.null(query)) {
        query <- paste0(
          "SELECT bp.id as bety_pft_id, bp.name as pft, bs.id as bety_species_id, bs.scientificname as latin FROM pfts as bp INNER JOIN ",
    ...
    tmp <- lapply(seq_along(bety_list), function(x) {
      data.frame(
        pft = rep(names(bety_list)[x], length(bety_list[[x]])),
