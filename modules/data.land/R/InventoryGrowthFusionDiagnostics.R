...
InventoryGrowthFusionDiagnostics <- function(jags.out, combined = NULL) {
  ...
  ## process model
  vars <- (1:ncol(out))[-c(
    which(substr(colnames(out), 1, 1) == "x"),
  ...
  var.out <- coda::as.mcmc.list(lapply(jags.out, function(x) {
    x[, vars]
  }))
