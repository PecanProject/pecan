...
soil_params <- function(soil_type = NULL, sand = NULL, silt = NULL, clay = NULL, bulk = NULL) {
  ...
  if (sum(is.null(sand), is.null(silt), is.null(clay)) > 1) {
    ...
    if (any(c(sand, silt, clay) > 2)) {
      ...
      sand <- if (is.null(sand)) {
        NULL
      } else {
        sand / 100
      }
      silt <- if (is.null(silt)) {
        NULL
      } else {
        silt / 100
      }
      clay <- if (is.null(clay)) {
        NULL
      } else {
        clay / 100
      }
  ...
  for (z in which(!(mysoil$soil_n %in% c(12, 13)))) {
    ...
    mysoil$soil_hydraulic_conductivity_at_saturation[z] <- udunits2::ud.convert(
      10.^(-0.60 + 1.26 * mysoil$fraction_of_sand_in_soil[z] - 0.64 * mysoil$fraction_of_clay_in_soil[z]),
