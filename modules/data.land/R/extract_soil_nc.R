...
extract_soil_gssurgo <- function(outdir, lat, lon, size = 1, radius = 500, depths = c(0.15, 0.30, 0.60)) {
  ...
  if ("GML" %in% rgdal::ogrDrivers()$name) {
    suppressMessages({
      ...
      mukey_area <- data.frame(
        Area = raster::area(x = as(site_area, "Spatial")),
  ...
  soilprop <- gSSURGO.Query(mukeys, c(
  ...
  # unit conversion
  soilprop.new [, c(
    "fraction_of_sand_in_soil", "fraction_of_silt_in_soil", "fraction_of_clay_in_soil",
    "soil_depth"
  )] <- soilprop.new [, c(
    "fraction_of_sand_in_soil", "fraction_of_silt_in_soil",
    "fraction_of_clay_in_soil", "soil_depth"
  )] / 100
          ...
          tryCatch(
            {
              ...
              simulated.soil <- simulated.soil %>%
                as.data.frame() %>%
                mutate(
                  DepthL = rep(DepthL.Data[1, 6], size),
