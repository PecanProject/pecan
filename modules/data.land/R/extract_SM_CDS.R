  ...
  for (i in seq_along(time.points)) {
    ...
    if (length(dates.ind.download) > 0) {
      PEcAn.logger::logger.info(paste(
        "The nc files for the following dates were not found.",
    ...
    sm <- ncfile %>%
      furrr::future_map(function(nc) {
        list(
          sm = raster::brick(nc, varname = "sm") %>%
    ...
    for (j in seq_along(site_info$site_id)) {
      ...
      results <- rbind(results, list(
        date = as.character(time.points[i]),
