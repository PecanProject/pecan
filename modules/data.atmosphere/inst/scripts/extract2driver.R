...
readMetTars <- function(tarfiles, indices,
  ...
  foo <- sapply(
    tarfiles,
    function(tarfile) {
      readMetTar(tarfile, indices, nearestCells, weights)
    }
  )
  if (!is.list(foo)) {
    print(foo)
...
writeHdf5 <- function(file, metdata, potential, downscale.radiation = function(x) {
                        x
                      }) {
  as.met.array <- function(x) {
    array(x, dim = c(1, 1, length(x)))
  }
...
smoothedRadiation <- function(a, month, year, timelag, new.timestep, old.timestep) { ## radiation
  ...
  rin <- potentialRadiation(
    rep(startday:startday, each = secsInDay / new.timestep),
    rep(
      seq(new.timestep, secsInDay, new.timestep),
      monthlength(mo, year)
    ), LAT, timelag * 24
  )
...
for (year in unique(years)) {
  ...
  for (month in c("08", "09", "10", "11", "12")) {
    ...
    surfaceMetData <- readMetTars(
      tarfiles[surfaceTars],
      list(
        cfrzr = 33, cicep = 32, crain = 34, csnow = 31, dlwrf = 42,
    ...
    fluxMetData <- readMetTars(
      tarfiles[fluxTars],
      list(
        tmp10 = 38, tmp30 = 44, ugrd10 = 35, ugrd30 = 41, vgrd10 = 36,
    ...
    out.file <- paste(narrdir, outdir, "/",
      outdir, "_", yearnum,
      monthnames[monthnum], ".h5",
      sep = ""
    )
    print(out.file)
    writeHdf5(out.file, monthMetData, potential,
      downscale.radiation = function(x) {
        x
      }
    )
