...
if (cf == TRUE){
  ...
  cf.id <- PEcAn.DB::convert.input(input.id,outfolder,formatname,mimetype,site.id,start_date,end_date,pkg,fcn,write,username,con=con,raw.host=raw.host)
}
...
#   
#   
#   perm.id <- PEcAn.DB::convert.input(input.id,outfolder,formatname,mimetype,site.id,start_date,end_date,pkg,fcn,write,username,con=con)
# }
...
if (extract == TRUE){
  ...
  extract.id <- PEcAn.DB::convert.input(input.id,outfolder,formatname,mimetype,site.id,start_year,end_year,pkg,fcn,write,username,con=con,newsite = newsite,raw.host=raw.host,write=TRUE)
}
...
if(nchar(model) >2){
  ...
  model.id <- PEcAn.DB::convert.input(input.id,outfolder,mod.formatname,mod.mimetype,newsite,start_year,end_year,pkg,fcn,write,username,con=con,lst=lst,overwrite=overwrite,raw.host=raw.host,write=TRUE)
