...
if(nchar(model) >2){
  ...
  model.id <- convert.input(input.id,outfolder,mod.formatname,mod.mimetype,site.id,start_date,end_date,pkg,fcn,write,username,con,lst=lst,overwrite=overwrite)
}
