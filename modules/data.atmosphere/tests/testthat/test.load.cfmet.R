...
daily.cf <- load.cfmet(
  met.nc = daily.nc, lat = 39.75, lon = -87.25,
...
test_that("data extracted from test pecan-cf met files is valid", {
  ...
  expect_true(all(c(
    "index", "date", "doy", "year", "month", "day", "hour",
...
test_that("load.cfmet throws error if start/end date out of range", {
  expect_error(
    load.cfmet(
      met.nc = subdaily.nc, lat = 39, lon = -88,
