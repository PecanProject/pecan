...
x <- weather[,list(year, day,
                   ...
                   Tmax = qctemp(tempmax),
                   Tmin = qctemp(tempmin),
                   solarR   = qcsolar(24 * ud.convert(fixsolar(solar),
                       "watt day meter-2",
                       "megajoule meter-2")), ## W/m2/h to MJ /m2/d
                   WS  = qcwind(sqrt(uwind^2 + vwind^2)),
                   ## precip from kg/m2/s (kg/m2 == mm) to inch/day
                   precip = qcprecip(ud.convert(precip, "mm s-1", "mm day-1")),
                   shum,
                   rh)]
