...
#----------------------------------------------------------------------
nc.merge <- function(
    outfolder, in.path, in.prefix, start_date, end_date,
  ...
  vars.info <- data.frame(CF.name = c(
  ...
  train_df$date <- seq.POSIXt(
    from = start_time, by = 60 * 60 / stepby[[1]],
  ...
  if (upscale == FALSE) {
    ...
    dat.train <- stats::aggregate(
      train_df[, names(train_df)[!names(train_df) %in%
        c("year", "doy", "hour")]],
      by = train_df[time.vars], FUN = mean,
      na.rm = FALSE
    )
  ...
  time <- ncdf4::ncdim_def(
    name = "time", units = paste0("days since ", start_year, "-01-01T00:00:00Z"),
  ...
  for (j in seq_along(vars.info$CF.name)) {
    var.list[[j]] <- ncdf4::ncvar_def(
      name = as.character(vars.info$CF.name[j]),
