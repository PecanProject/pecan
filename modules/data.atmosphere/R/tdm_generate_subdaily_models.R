...
#' @export
#'
gen.subdaily.models <- function(
    outfolder, path.train, yrs.train, direction.filter = "forward", in.prefix,
  ...
  train.day <- stats::aggregate(dat.train[, c(
  ...
  names(train.day)[3:9] <- c(
    "air_temperature_mean.day", "precipitation_flux.day",
  ...
  lag.day$lag.air_temperature_min <- stats::aggregate(dat.train[, c("air_temperature")],
    by = dat.train[, c("year", "doy", "sim.day")], FUN = min
  )[, "x"] # Add in a lag for the next day's min temp
  lag.day$lag.air_temperature_max <- stats::aggregate(dat.train[, c("air_temperature")],
    by = dat.train[, c("year", "doy", "sim.day")], FUN = max
  )[, "x"] # Add in a lag for the next day's min temp
  lag.day$sim.day <- lag.day$sim.day + met.lag #


  dat.train <- merge(dat.train, lag.day[, c(
    "sim.day", vars.lag, "lag.air_temperature_min",
    "lag.air_temperature_max"
  )], all.x = T)
  ...
  # ----------
  vars.day <- c(
    "air_temperature_mean.day", "air_temperature_max.day",
  ...
  vars.next <- c(
    "next.air_temperature_mean", "next.air_temperature_max",
  ...
  temporal.downscale.functions(
    dat.train = dat.train, n.beta = n.beta, day.window = day.window,
