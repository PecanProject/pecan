...
download.MsTMIP_NARR <- function(outfolder, start_date, end_date, site_id, lat.in, lon.in,
  ...
  var <- data.frame(
    DAP.name = c("air_2m", "dswrf", "dlwrf", "wnd_10m", "apcp", "shum_2m", "rhum_2m"),
    CF.name = c(
  ...
  for (i in seq_len(rows)) {
    ...
    for (j in seq_len(nrow(var))) {
      if (var$DAP.name[j] == "dswrf") {
        (dap_file <- paste0(
          "http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/mstmip_driver_na_qd_dswrf_",
          year, "_v1.nc4"
        ))
      } else {
      ...
      dat.list[[j]] <- ncdf4::ncvar_get(
        dap, as.character(DAPvar[j]),
