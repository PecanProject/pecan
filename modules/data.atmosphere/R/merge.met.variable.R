...
merge_met_variable <- function(in.path, in.prefix, start_date, end_date, merge.file,
  ...
  merge.time.std <- PEcAn.utils::ud_convert(
    merge.time,
  ...
  for (year in start_year:end_year) {
    ...
    target.time.std <- PEcAn.utils::ud_convert(
      target.time,
    ...
    merge.interp <- stats::approx(merge.sub$time, merge.sub$data,
      xout = target.time.std,
      rule = 2, method = "linear", ties = mean
    )

    ## insert new variable
    var.merge <- ncdf4::ncvar_def(
      name = merge.vars[1], units = merge.attr$units, dim = nc$dim$time,
