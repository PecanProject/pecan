...
met_temporal_downscale.Gaussian_ensemble <- function(in.path, in.prefix, outfolder,
  ...
  var <- data.frame(
    ...
    units <- c(
      "Kelvin", "Kelvin", "Kelvin", "W/m2", "Pascal", "W/m2", "m/s",
      "m/s", "g/g", "kg/m2/s"
    )
  )
  ...
  for (e in seq_len(n_ens)) {
    ...
    wnd <- c(
      "specific_humidity", "eastward_wind", "northward_wind", "surface_downwelling_longwave_flux_in_air",
      "air_pressure"
    )
    for (u in wnd) {
      train_vec <- vector()
    ...
    ephemeris <- function(lat, lon, date, span = 1, tz = "UTC") {
      ...
      sunrise <- maptools::sunriset(lon.lat, sequence,
        direction = "sunrise",
        POSIXct.out = TRUE
      )
      sunset <- maptools::sunriset(lon.lat, sequence,
        direction = "sunset",
        POSIXct.out = TRUE
      )
      ...
      data.frame(
        date = as.Date(sunrise$time), sunrise = as.numeric(format(sunrise$time, "%H%M")),
    ...
    if (swdn_method == "sine") {
      eph <- ephemeris(lat_train, lon_train,
        date = paste0(eph_year, "-01-01", tz = "UTC"),
        span = sp
      )
    ...
    if (swdn_method == "Waichler") {
      ...
      Bm <- c(
        0.2089, 0.2857, 0.2689, 0.2137, 0.1925, 0.2209, 0.2527, 0.2495,
    ...
    train.list <- list()
    lat <- ncdf4::ncdim_def(
      name = "latitude", units = "degree_north", vals = lat_train,
      create_dimvar = TRUE
    )
    lon <- ncdf4::ncdim_def(
      name = "longitude", units = "degree_east", vals = lon_train,
      create_dimvar = TRUE
    )
    ...
    loc.file <- file.path(outfolder, paste0(
      source_name, ".dwnsc.gauss.ens",
      e, ".", year, ".nc"
    ))
