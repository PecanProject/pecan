...
download.GLDAS <- function(outfolder, start_date, end_date, site_id, lat.in, lon.in,
  ...
  if (start_year < GLDAS_start) {
    PEcAn.logger::logger.severe(sprintf(
      "Input year range (%d:%d) exceeds the GLDAS range (%d:present)",
      start_year, end_year,
      GLDAS_start
    ))
  }
  ...
  var <- data.frame(
    DAP.name = c(
      "Near_surface_air_temperature", "Surface_incident_longwave_radiation",
      ...
      for (h in seq_len(nrow(dap.log))[-1]) {
        ...
        for (v in seq_len(nrow(var))) {
          dap_query <- paste(dap_query,
            paste0(var$DAP.name[v], "[0:1:0]", "[", lat.use, "][", lon.use, "]"),
            sep = ","
          )
        }
