...
<<<<<<< HEAD
  ...
  if ("specific_humidity" %in% colnames(cfmet) & (!"relative_humidity" %in% colnames(cfmet))) {
    cfmet$relative_humidity <- cfmet[, list(qair2rh(qair = specific_humidity, temp = ud.convert(air_temperature, 
                                                                                                "Kelvin", "Celsius"), press = air_pressure))]
  }
...
<<<<<<< HEAD
  new.date <- subdailymet[, list(hour = 0:(23/output.dt)/output.dt), by = c("year", "month", "day", 
                                                                            "doy")]
  ...
  solarMJ <- ud.convert(subdailymet$surface_downwelling_shortwave_flux_in_air, paste0("W ", tint, 
                                                                                      "h"), "MJ")
  ...
  for (var in c("air_pressure", "specific_humidity", "precipitation_flux", "air_temperature", "surface_downwelling_shortwave_flux_in_air", 
                "ppfd", "relative_humidity")) {
...
cfmet.downscale.daily <- function(dailymet, output.dt = 1, lat) {
  ...
  a <- qair[, list(year, doy, tmin, tmax, air_temperature, qmin, qmax, pressure = ud.convert(air_pressure, 
  ...
  ans <- data.table(time, downwelling_photosynthetic_photon_flux = SolarR, air_temperature = ud.convert(Temp, 
