...
extract.local.NLDAS <- function(outfolder, in.path, start_date, end_date, lat.in, lon.in,
  ...
  rows <- length(ylist)
  results <- data.frame(
    file = character(rows), host = character(rows),
  ...
  var <- data.frame(
    NLDAS.name = c("air_temperature", "surface_downwelling_longwave_flux_in_air", "air_pressure", "downwelling_shortwave_flux_in_air", "eastward_wind", "northward_wind", "specific_humidity", "precipitation_amount"),
