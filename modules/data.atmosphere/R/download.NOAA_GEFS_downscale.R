...
download.NOAA_GEFS_downscale <- function(outfolder, lat.in, lon.in, sitename, start_date = Sys.time(), end_date = (as.POSIXct(start_date, tz = "UTC") + lubridate::days(16)),
  ...
  start_date <- as.POSIXct(paste0(
    lubridate::year(start_date), "-", lubridate::month(start_date), "-", lubridate::day(start_date), " ",
    substring(forecast_hour, 1, 2), ":00:00"
  ), tz = "UTC")
  ...
  if (as.POSIXct(Sys.time(), tz = "UTC") < start_date || start_date < NOAA_GEFS_Start_Date) {
    PEcAn.logger::logger.severe(sprintf(
      "Start date (%s) exceeds the NOAA GEFS range (%s to %s).",
      start_date,
  ...
  noaa_var_names <- c(
    "Temperature_height_above_ground_ens", "Pressure_surface_ens", "Relative_humidity_height_above_ground_ens", "Downward_Long-Wave_Radp_Flux_surface_6_Hour_Average_ens",
  ...
  cf_var_names <- c(
    "air_temperature", "air_pressure", "specific_humidity", "surface_downwelling_longwave_flux_in_air",
  ...
  nonSW.flux.hrly <- forecasts %>%
    ...
    PEcAn.data.atmosphere::downscale_repeat_6hr_to_hrly() %>%
  ...
  joined <- dplyr::inner_join(joined, ShortWave.ds, by = c("NOAA.member", "timestamp")) %>%
    dplyr::distinct() %>%
    dplyr::mutate(
      surface_downwelling_shortwave_flux_in_air = dplyr::if_else(surface_downwelling_shortwave_flux_in_air < 0, 0, surface_downwelling_shortwave_flux_in_air),
