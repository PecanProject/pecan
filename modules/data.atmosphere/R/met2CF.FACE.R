    ...
    if(!file.exists(outfolder)){ #file.copy(f,f.cf)
      ...
      for (k in 1:l){   
        if(vars[k] %in% nc.vars){#nc <- tncar_rename(nc,vars[k],nvars[k])
          ...
          units <- ncatt_get(nc, varid=vars[k], attname='units', verbose=verbose)$value
            
          var <- ncvar_def(name=nvars[k], units=units, dim=dim, verbose=verbose)
          nc2 <- ncvar_add(nc=nc2, v=var, verbose=verbose)
...
>>>>>>> c182a66982ee43c59b3335eaaca2904cf92c80c6
}
