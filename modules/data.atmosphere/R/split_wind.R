...
split_wind <- function(in.path, in.prefix, start_date, end_date,
  ...
  for (year in start_year:end_year) {
    ...
    ## insert east
    eastward <- ncdf4::ncvar_def(
      name = "eastward_wind", units = wind_speed.attr$units, dim = dims,
    ...
    ## insert north
    northward <- ncdf4::ncvar_def(
      name = "northward_wind", units = wind_speed.attr$units, dim = dims,
      missval = wind_speed.attr$`_FillValue`, verbose = verbose
    )
