...
debias.met.regression <- function(train.data, source.data, n.ens, vars.debias = NULL, CRUNCEP = FALSE,
  ...
  for (v in vars.debias) {
    ...
    # -----
    met.train <- data.frame(
      year = train.data$time$Year,
    ...
    for (ens in 1:n.ens) {
      ...
      if (pair.anoms == TRUE) {
        ...
        dat.anom <- merge(
          met.src[met.src$ind == ind & met.src$year %in% yrs.overlap, c("year", "doy", "ind", "X", "anom.raw")],
      ...
      while (n.new > 0 & sane.attempt <= sanity.tries) {
        ...
        if (pair.anoms == F & (v %in% c("air_temperature_maximum", "air_temperature_minimum"))) {
          ...
          if (length(cols.redo) > 1) {
            sim1b[, cols.redo] <- apply(sim1b[, cols.redo], 2, FUN = function(x) {
              x + anom.detrend
            }) # Get the range around that medium-frequency trend
          } else {
        ...
        if (v %in% c("air_temperature", "air_temperature_maximum", "air_temperature_minimum")) {
          ...
          cols.redo <- which(apply(sim1, 2, function(x) {
  ...
  nc.info <- data.frame(
    CF.name = c(
      "air_temperature", "air_temperature_minimum", "air_temperature_maximum", "precipitation_flux",
    ...
    longname = c(
      "2 meter air temperature", "2 meter minimum air temperature", "2 meter maximum air temperature",
