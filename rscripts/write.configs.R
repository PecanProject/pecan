...
for (i in seq(pft.names)){
  ...
  ## config header
  if('config.header' %in% settings){
    config.header <- settings$config.header
  }else{
    ## THIS FCN IS AN ED2-SPECIFIC KLUGE
    config.header <- pecan.config.constants(pft.name) 
  }
  ...
  run.ids[[pft.name]] <- write.ensemble.configs(config.header, ensemble.samples[[pft.name]], outdirs[i])
  ...
  if('sensitivity.analysis' %in% names(settings)) {
    ...
    run.ids[[pft.name]] <- append(run.ids[[pft.name]], write.sa.configs(config.header, sa.samples[[pft.name]], outdirs[i]))
  } 
}
