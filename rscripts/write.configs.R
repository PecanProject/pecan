...
#Make outdirectory
ssh(host$name, 'mkdir ', host$outdir, get.run.time())
print('Write configs written. Use the following to refer to output generated by these config files: ')
print(get.run.time())

save(ensemble.samples, trait.samples, sa.samples, file = paste(outdir, 'samples.Rdata', sep=''))
rsync(paste(outdir, 'samples.Rdata', sep=''),
      paste(host$name, ':', host$outdir, get.run.time(), sep=''))
