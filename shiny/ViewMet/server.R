...
server <- function(input, output, session) {
    ...
    ignoreInit = TRUE,
    {
      ...
      if (is.na(as.numeric(site))) {
        ...
        if (all(dim(dbfiles) == 0)) {
          ...
          types <- unique(dbfiles$file_path) %>%
            basename() %>%
          ...
          for (i in seq_along(paths)) {
            new.files <- dir(dirname(paths[i]),
              pattern = basename(paths[i]),
              full.names = TRUE
            )
  ...
  load.model.data <- eventReactive(input$load_data, {
    ...
    for (i in seq_along(rv$load.paths)) {
      ...
      inputid <- tbl(bety, "dbfiles") %>%
        filter(file_path == fpath) %>%
        pull(container_id) %>%
        unique() %>%
        .[1]
      formatid <- tbl(bety, "inputs") %>%
        filter(id == inputid) %>%
        pull(format_id)
      siteid <- tbl(bety, "inputs") %>%
        filter(id == inputid) %>%
        pull(site_id)
      ...
      dat <- try(load_data(
        data.path = rv$load.paths[i],
        format = format, site = site,
      ))

      if (inherits(dat, "data.frame")) {
        dat$met <- rv$load.paths[i] %>%
          dirname() %>%
          basename() %>%
          gsub(pattern = "\\_site_.*", replacement = "", x = .)
        data[[i]] <- dat
  ...
  observeEvent(input$plot_data, {
    ...
    p_overlay <- ggplot(rv$plot.data) +
      geom_line(aes(x = date, y = var, color = met)) +
      ylab(input$var) +
      ggtitle(input$var)

    p_facet <- ggplot(rv$plot.data) +
      geom_line(aes(x = date, y = var, color = met), size = 1) +
      ylab(input$var) +
      ggtitle(input$var) +
      facet_grid(met ~ .)
