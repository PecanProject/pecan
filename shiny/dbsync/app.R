...
check_servers <- function(servers, progress) {
  ...
  urls <- sapply(check_servers, function(x) {
    sub("bety.tar.gz", "version.txt", x)
  })
  lapply(urls, function(x) {
    curl::curl_fetch_multi(x, done = server_version, fail = failure)
  })
  ...
  urls <- sapply(check_servers, function(x) {
    sub("bety.tar.gz", "sync.log", x)
  })
  lapply(urls, function(x) {
    curl::curl_fetch_multi(x, done = server_log, fail = failure)
  })
