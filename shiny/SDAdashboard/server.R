    ...
    tryCatch(
      {
        ...
        if (values$MultiSite) {
          values$Multisite.info <- list(
            Sites = values$dead_data$FORECAST[[1]] %>% attr("Site") %>% unique(),
      ...
      bs_accordion(id = "meet_the_beatles") %>%
        bs_append(
          ...
          content = tagList(fluidRow(
            column(
              12, leafletOutput("mymap", height = "800px", width = "100%"),
              ...
              absolutePanel(
                ...
                column(
                  ...
                  prettyCheckboxGroup(
                    ...
                    choices = list(
                      "Forecast Spatial Correlation" = "spcor",
                      "Raster Map" = "rastMap"
                    ),
                    inline = TRUE,
                    status = "success",
  ...
  observeEvent(input$mymap_marker_click, {
    ...
    showModal(
      modalDialog(
        ...
        fluidRow(
          column(
            12, h3(paste0("SiteID:", site_id, " - You can either choose a time or click on Play !")),
    ...
    output$network <- renderVisNetwork({
      ...
      visNetwork(nodes = data$nodes, edges = data$edges) %>%
        ...
        visGroups() %>%
        visOptions(
          selectedBy = list(variable = "group", selected = site_id), #
          collapse = TRUE,
  ...
  observe({
    ...
    if ("spcor" %in% input$AllmapOpt) {
      ...
      p1 <- map_df(alllines$groupt, function(x) {
        values$dead_data$site.locs[which(row.names(values$dead_data$site.locs) == x), c(1, 2)] %>%
          t() %>%
          as.data.frame()
      })
      ...
      leafletProxy("mymap", data = gcIntermediate(p1,
        ...
        # clearShapes()%>%
        addPolylines(
          popup = paste0("<h4>", alllines$from, " & ", alllines$to, "<br>", tmp.covs %>% round(2), "</h4>"),
  ...
  observeEvent(c(input$boverlay, input$tileop), {
    ...
    if (input$boverlay == "None") {
      ...
      aoi_boundary_HARV %>%
        ...
        clearShapes() %>%
        addPolygons(
          stroke = FALSE, fillOpacity = input$tileop, smoothFactor = 0.5,
