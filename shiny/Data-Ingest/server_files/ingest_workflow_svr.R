...
observeEvent(input$nextFromInput, {
  ...
  if (input$InputSiteName == "") {
    ...
    inputsList$siteID <<- sites %>%
      dplyr::filter(sitename %in% input$InputSiteName) %>%
      pull(id)
  }
  ...
  if (input$InputParentName == "") {
    ...
    inputsList$parentID <<- inputs %>%
      dplyr::filter(name %in% input$InputParentName) %>%
      pull(id)
  }
...
output$input_record_df <- renderPrint({
  Shared.data$input_record_df
})

######### Formats Svr #############
output$autoname <- renderPrint({
  Shared.data$selected_row
}) # _local
...
observeEvent(input$register_variable, {
  ...
  output$format_vars_df <- DT::renderDT(datatable(
    {
      Shared.data$format_vars_df
    },
    escape = FALSE,
    selection = "single",
    options = list(ordering = F, dom = "tp")
  ))
})
  ...
  tryCatch(
    {
      ...
      Shared.data$input_record_df <- PEcAn.DB::dbfile.input.insert(
        in.path = inputsList$Path,
