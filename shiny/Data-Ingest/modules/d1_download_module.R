d1DownloadUI <- function(id) {
  ...
  box(
    width = 4, title = h2("Import From DataONE"), solidHeader = TRUE, status = "success",
    textInput(
      ns("id"),
...
d1Download <- function(input, output, session) {
  ...
  output$rowSelection <- renderPrint({
  ...
  observeEvent(input$D1FinishButton, {
    ...
    output$D1dbfilesPath <- renderText({
      paste0(PEcAn_path, d1_dirname)
    }) # Print path to data
  })
}
