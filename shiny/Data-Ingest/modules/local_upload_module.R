localUploadUI <- function(id) {
  ...
  box(
    width = 4, title = h2("Upload Local Files"), solidHeader = T, status = "success",
...
localUpload <- function(input, output, session) {
  ...
  output$dtfiles <- DT::renderDT(
    {
      Shared.data$local_files
    },
    selection = "single",
  ...
  output$test <- renderPrint({
    Shared.data$selected_row_local
  })
  ...
  observeEvent(input$LocalFinishButton, {
    ...
    output$LocaldbfilesPath <- renderText({
      paste0(PEcAn_path, local_dirname)
    }) # Print path to dbfiles
  })
}
