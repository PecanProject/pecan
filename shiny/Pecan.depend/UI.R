dashboardPage(
  ...
  dashboardSidebar(sidebarMenu(
    ...
    conditionalPanel(
      condition = "input.Tabsetpan === 'Packages'",
    ...
    conditionalPanel(
      condition = "input.Tabsetpan === 'Functions'",
          ...
          box(
            fluidRow(
              column(
                width = 4,
                pickerInput(
                  inputId = "packageslist", "Package(s) :", choices = installed.packages()[, 1] %>% grep("^PEcAn", ., value = T, ignore.case = T), multiple = TRUE,
                  options = list(
                    `actions-box` = TRUE,
                    `live-search` = TRUE
                  )
