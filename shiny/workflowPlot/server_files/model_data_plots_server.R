...
observeEvent(input$var_name_modeldata, {
  model.df <- load.model()
  default.unit <- model.df %>%
    filter(var_name == input$var_name_modeldata) %>%
    pull(ylab) %>%
    unique()
...
observeEvent(input$units_modeldata, {
  if (PEcAn.utils::unit_is_parseable(input$units_modeldata)) {
    ...
    if (units::ud_are_convertible(default.unit, input$units_modeldata)) {
      output$unit_text2 <- renderText({
        "Conversion possible"
      })
    } else {
      output$unit_text2 <- renderText({
        "Units are parsible but conversion is not possible"
      })
    }
  } else {
    output$unit_text2 <- renderText({
      "Units are not parsible, please type units in udunits2 compatible format"
    })
  }
})
      ...
      tryCatch(
        {
          withProgress(
            ...
            detail = "This may take a while...",
            {
              ...
              output$modelDataPlotscatter <- renderHighchart({
                ...
                highchart() %>%
                  ...
                  hc_add_theme(hc_theme_elementary(
                    yAxis = list(
                      title = list(style = list(color = "#373b42", fontSize = 15)),
              ...
              plot_type <- switch(input$plotType_model,
                point = "scatter",
                line = "line"
              )
              ...
              ply <- highchart() %>%
                hc_add_series(model.xts,
                  name = "model", type = plot_type,
                  regression = TRUE,
                  regressionSettings = list(type = "loess", loessSmooth = smooth_param)
                ) %>%
                hc_add_series(observasions.xts,
                  name = "observations", type = plot_type,
                  regression = TRUE,
                  regressionSettings = list(type = "loess", loessSmooth = smooth_param)
                ) %>%
