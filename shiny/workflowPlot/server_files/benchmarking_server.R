  ...
  tryCatch(
    {
      ...
      if (nrow(ids_DF) == 1) {
        ...
        ens_id <- dplyr::tbl(dbConnect$bety, "runs") %>%
          dplyr::filter(id == ids_DF$runID) %>%
          dplyr::pull(ensemble_id)
        ens_wf <- dplyr::tbl(dbConnect$bety, "ensembles") %>%
          dplyr::filter(id == ens_id) %>%
          dplyr::rename(ensemble_id = id) %>%
          dplyr::left_join(., tbl(dbConnect$bety, "workflows") %>% dplyr::rename(workflow_id = id), by = "workflow_id") %>%
          dplyr::collect()
...
observeEvent(
  ...
  {
    output$brr_message <- renderText({
      bm$brr_message
    })
    output$button_BRR <- renderUI({
      if (bm$button_BRR) {
        actionButton("create_bm", "Create Benchmarking Reference Run")
      }
    })
  }
)
  ...
  tryCatch(
    {
      ...
      bm$metrics <- dplyr::tbl(dbConnect$bety, "metrics") %>%
        dplyr::select(one_of("id", "name", "description")) %>%
        collect()
      ...
      if (is.null(format$time.row) & is.na(format$var$column_number)) {
        ...
        bm$vars <- dplyr::inner_join(
          data.frame(
            read_name = names(bm$model_vars),
            pecan_name = bm$model_vars, stringsAsFactors = FALSE
          ),
          format$vars[
            -grep("%", format$vars$storage_type),
...
observeEvent(bm$ready, {
  if (bm$ready > 0) {
    ...
    bench.out <- grep("benchmarking.output.Rdata",
      dir(bm.path, full.names = TRUE),
      value = TRUE
    ) # Look for benchmarking directory
      ...
      if (bm$ready > 0) {
        fluidRow(
          ...
          column(
            2,
            shinyjs::disabled(
              actionButton("calc_bm_button", "Calculate",
  ...
  tryCatch(
    {
      ...
      withProgress(
        ...
        value = 0,
        {
          ...
          basePath <- dplyr::tbl(dbConnect$bety, "workflows") %>%
            dplyr::filter(id %in% bm$ens_wf$workflow_id) %>%
            dplyr::pull(folder)
