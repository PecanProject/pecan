      ...
      withProgress(
        ...
        value = 0,
        {
          ...
          for (i in seq(nrow(ids_DF))) {
            ...
            wf.folder <- workflow(dbConnect$bety, ids_DF$wID[i]) %>%
              collect() %>%
              pull(folder)
          ...
          select.data <- select.df %>%
            ...
            dplyr::select(
              runtype, workflow.id, ensemble.id, pft.name, quantile, trait, run.id,
