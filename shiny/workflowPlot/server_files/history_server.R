...
cmd <- paste0(
  "SELECT workflows.id, workflows.folder, workflows.start_date, workflows.end_date, workflows.started_at, workflows.finished_at, attributes.value,",
  ...
  tryCatch(
    {
      ...
      output$historyfiles <- DT::renderDT(
        DT::datatable(history %>%
          ...
          mutate(
            id = paste0("<button id=\"workflowbtn_", id, " \" type=\"button\" class=\"btn btn-primary btn-sm btn-block action-button workflowclass\" width=\"100%\">", id, "</button>"),
    ...
    error = function(e) {
      toastr_error(
        title = "Error in History Runs Page", message = ""
        # , conditionMessage(e)
      )
    }
  )
