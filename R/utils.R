...
get.units <- function(traits = NULL) {
  ...
  if(nrow(units) <= 0){
    warning(paste('No unit within database for variable(s): ', 
            paste(traits, collapse=',')))
    return(data.frame(name=traits, units=''))
  }
  ans <- merge(data.frame(name = traits), units, by = 'name', 
      all.x=TRUE, sort =FALSE)
  ans$units[is.na(ans$units)] <- ''
  return(ans)
}
...
#' Returns a user friendly version of trait names for use in filenames, figures, etc. 
#'
...
trait.dictionary <- function(traits = NULL) {
  defs<-data.frame(stringsAsFactors = FALSE,
      id = c("plant_min_temp", "c2n_leaf", "dark_respiration_factor", "f_labile", "growth_resp_factor", "leaf_turnover_rate", "leaf_width", "mort2", "nonlocal_dispersal", "fineroot2leaf", "quantum_efficiency", "root_respiration_rate", "root_turnover_rate", "SLA", "stomatal_slope", "Vcmax", "Vm_low_temp", "water_conductance","cuticular_cond","seedling_mortality","r_fract","storage_turnover_rate", "T", "agf_bs"),
      ...
      units = c("Celsius", "ratio", "fractio", "fraction", "fraction", "yr-1", "mm", "yr-1", "fraction", "ratio", "fraction", "umol CO2 kg-1 s-1", "yr-1", "m2 kg-1", "ratio", "umol CO2 m-2 s-1", "Celsius", "mm", "umol H2O m-2 s-1", "fraction", "fraction", "yr-1", "mm  H2O m-1s-1", "fraction")
  )
  ...
  if(is.null(traits)) {
    ...
    trait.defs <- data.frame(id=traits, figid=traits, units='', stringsAsFactors = FALSE)
    rownames(trait.defs) <- traits
    trait.defs[traits[traits %in% defs$id],] <- defs[traits[traits %in% defs$id],]
  }
  return(trait.defs)
}
