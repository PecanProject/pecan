...
add_icon <- function(id = NULL, x = 0, y = 0) {
  ...
  dims <- dim(icon)
  logo <- rasterGrob(icon, unit(x, "npc"), unit(y, "npc"), unit(dims[1], "points"), 
                     unit(dims[2], "points"), just = c("left", "bottom"))
  grid.draw(logo)
  
  lab <- textGrob(label = paste("PEcAn", id), x = unit(x, "npc") + 
                    unit(dims[1], "points"), y = unit(y, "npc"), just = c("left", "bottom"))
  grid.draw(lab)
} # add_icon
