...
efi.data.process <- function(args) {
  ...
  if (!is.null(start_date)) {
    workflows <- PEcAn.DB::db.query(paste0(
      "SELECT * FROM workflows WHERE start_date='", format(start_date, "%Y-%m-%d %H:%M:%S"),
      "' ORDER BY id"
    ), con)
  } else {
  ...
  if (length(SDA_check) > 0) {
    workflows <- workflows[-SDA_check, ]
  }

  index <- grepl(basename(args[3]), workflows$folder)

  if (length(index) == 0) {
    workflow <- workflows[which(workflows$folder == ""), ]
  }
  ...
  for (i in 2:length(pecan_out_dirs)) {
    ...
    if (length(datafiles) == 1) {
      ...
      if (i == 2) {
        ...
        soilmoist <- cbind(soilmoist, soil)
      }
  ...
  needf <- neemat %>%
    as_tibble() %>%
    mutate(
      date = as.Date(sec, origin = origin),
