...
submit.workflow.list <- function(workflowList, userDetails) {
  ...
  outdir <- paste0(
    Sys.getenv("DATA_DIR", "/data/"), "workflows/PEcAn_",
    workflow_id_str
  )
...
insert.workflow <- function(workflowList) {
  ...
  PEcAn.DB::db.query(
    "UPDATE workflows SET folder = $1 WHERE id = $2", con,
    values = list(
...
insert.attribute <- function(workflowList) {
  ...
  properties <- list(
    ...
    email = if (is.na(workflowList$info$userid) || workflowList$info$userid == -1) {
      ""
    } else {
      dplyr::tbl(global_db_pool, "users") %>%
        filter(id == bit64::as.integer64(workflowList$info$userid)) %>%
        pull(email)
    },
