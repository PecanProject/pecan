...
paviyield <- read.csv('/home/carya/pecan/qaqc/inst/Compare/wang2010data.csv', sep = ';', header = F)
...
area.uncert <- function(x,y,iflog=T,quantile=c(0.05,0.95),fold=7) {
  ...
  out <- vector()
  for (t in 1:fold) {
    out[t] <- area(x[-get(paste("idx",t,sep=""))],y[-get(paste("idy",t,sep=""))],iflog,quantile)
  }
  return(sd(out))
}
```
...
measure <- function(ref,model,rname=c("ref"),mname=c("model"),normalize=FALSE,iflog=T,quantile=c(0.05,0.95),fold=c(7)){
  ref <- na.omit(ref)
  model <- na.omit(model)
  ...
  measures <-c(sd.f,sd.r,R,E,S,area(ref,model,iflog,quantile),area.uncert(ref,model,iflog,quantile,fold))
